...
| `harness audit [--since 1h] [--policy <name>] [--outcome <outcome>] [--session <id>] [--json]` | Replay recorded policy decisions from the evidence-ledger, plus (agent-tasks `5ad63b01`) an `approvals` section listing the raw understanding-gate ledger facts (`understanding-approved:<sid>`, its `:forced:<field>` variant, `understanding-auto-approved:<sid>`) in the same `--since` window, filtered by `--session`; empty is omitted in text, always present in `--json`. |
