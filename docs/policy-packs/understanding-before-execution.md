...
- Seven hooks in the harness-managed `settings.json` by default:
  ...
  - `PostToolUse` marker-expiry: `harness pack hook post-tool-use` clears the approval marker (and expires the persisted report) after a configurable task-boundary tool fires (default: agent-tasks `task_finish` / `task_abandon` / `pull_requests_merge` / `tasks_transition` / `task_merge`), or after a Bash call whose command matches `approval_lifecycle.expire_on_bash_match`; the emitted `matcher` widens to include `Bash` whenever that list is configured (task bea04a03) so a real `gh pr merge` / `git push` actually reaches the hook.
    The approval expires on the same boundary as the active-claim marker below (task `5018c0c4`, operator decision: align). For a listed tool that is an agent-tasks claim lifecycle verb (`ACTIVE_CLAIM_TOOL_NAMES`), `matchPostToolUseBoundary` asks the one decider, `claimEffectForAgentTasksTool`, with the event's `tool_response`; a "claim kept" answer is not a boundary. So a `task_finish` whose result lands the task in `review` keeps the session and task approval markers (the approval is task-scoped and the claim is kept; scope growth from review feedback is the stay-in-scope hook's job), while `task_finish` to `done`, `task_abandon`, `task_merge` and a `tasks_transition` to `done` expire them. An absent or unreadable `task_finish` result takes the decider's fail-safe `release` answer and expires (the safe side of the gate). Tools the decider does not track (`pull_requests_merge`, `task_submit_pr` when listed) and a `task_start` an operator lists explicitly (an `acquire`, not a keep) expire on list membership as before. `task_merge` joined the default list so the review-then-`task_merge` path still expires the approval when the task reaches `done`; re-run `harness apply` to install the widened matcher. Both the Claude hook and the Codex `codex-post-tool-use` sibling pass `tool_response`.
