...
#### Install default, upgrade path, and the bypass-without-opt-in doctor finding

`harness init` ships the `auto_approve` block ACTIVE, not commented out, in every template that enables this pack (`full`, `solo`, `team`; `minimal` carries no `policy_packs` at all) and in `harness init --interactive`'s Custom composer whenever `understanding-before-execution` is selected there: `when: [bypassPermissions]`, `harnesses: [claude-code]`, `require_report: true`, preceded by a comment explaining what it does, that `bypassPermissions` is the operator's own launch choice, and that deleting the block restores the manual prompt. The rationale (docs/decisions/2026-08-27-ug-auto-mode-approval.md, "Amendment: install default"): under `bypassPermissions` the permission system is off by the operator's own choice at launch (`--dangerously-skip-permissions` / `--permission-mode bypassPermissions`); a human approval prompt in that mode is exactly the friction the mode exists to remove. `require_report: true` is unchanged (the only schema-valid value), and every template carrying this block also ships `mode: grill_me`, so the report the auto path consumes was actually checked, not merely present. The opt-in stays a visible, deletable config key, and an existing install is never changed silently: it only gets this block through the explicit upgrade verb below.

For a manifest that predates this default, `harness pack upgrade understanding-before-execution` inserts the same block into the pack's `config:` mapping, text-level (never a full YAML parse-and-reserialize), so every other line, including hand edits `pack add` / `pack reseed` never touch, stays byte-for-byte untouched. It is idempotent: if `auto_approve:` already exists anywhere inside the pack's `config:` mapping, the command is a no-op and reports the manifest unchanged. It refuses (non-zero exit, a clear message) rather than guess when it cannot unambiguously locate the pack block or its `config:` key. `--dry-run` prints the unified diff without writing.

`harness doctor` reports a warning when, among the newest `--recent-sessions` observations, the PreToolUse hook itself recorded `permission_mode: bypassPermissions` for some session AND `auto_approve` does not cover it (missing entirely, `bypassPermissions` absent from `when`, or `claude-code` absent from `harnesses`). The observation the finding reads comes from the hook process, not from a ledger row or report field the agent writes through its own API: the same PreToolUse hook that attempts the opt-in auto-approval already sees `permission_mode` on its payload, so it also persists a small per-session record (session id, last-seen mode, timestamp; no payload content) under `harness.generated/.permission-mode-observations/`, independent of whether `auto_approve` is even configured. That keeps the finding off the evidence-ledger and persisted-report trust boundary the gate itself enforces elsewhere, but the record is NOT signed the way an approval marker is: it is evidence that the hook was invoked with a given payload, not proof that a session actually ran under that mode, and an already-approved session (or anything else able to invoke `harness pack hook pre-tool-use` directly) can write, or forge, one. This is exactly why the finding stays advisory (`⚠`) only and never gates a tool call or mints an approval; the operator who reads it decides what it means. Signing the observation was considered and rejected, not deferred: a signature would only prove the hook signed the record, not that the named session actually ran under that mode, and it would open a second, unconditional signing surface for the marker-signing key (not a path to mint an approval marker, which stays namespaced by markerId); see docs/decisions/2026-08-27-ug-auto-mode-approval.md's "Amendment: permission-mode observation stays unsigned" for the full reasoning and the reopen criterion. The finding names the exact snippet to add and the upgrade command that inserts it. `harness gc` sweeps `harness.generated/.permission-mode-observations/` under its `permission-mode-observation` category, deleting a file once its own mtime is past the retention window, same as the approval-marker sweep (see `docs/CLI.md`'s `gc` row).
...
1. **Filesystem approval marker** at `harness.generated/.approvals/<sessionId>` (or `task-<taskId>`). Owned by harness; **the only source the gate accepts**. Written by `harness approve understanding` from the operator's shell. Edit / Write / Bash are all gated by this same PreToolUse hook, and no configured MCP exposes filesystem writes, so the marker is reachable only from operator-launched processes. This is what closes the self-approval boundary (agent-tasks/88ca4bb3). **As of harness/f9485cc7 this invariant is also cryptographically enforced, not merely assumed** (see "Marker signing" below): the marker carries an HMAC signature, so even if a future MCP tool silently reopened a local filesystem-write path the blocker's `Edit|Write|Bash` matcher doesn't cover, a marker written through it still fails the gate without the operator-side signing key. **The session marker is bound to the task it was granted for** (task `5018c0c4`, operator decision): every session marker records, as a signed `claimTaskId` field, the task id of the active claim (`harness.generated/active-claim`) at the moment it is written (`null` when no task is claimed), and the gate accepts it only while the active claim still names that same id (`checkSessionApprovalMarker` in `src/policy-packs/builtin/understanding-before-execution/task-markers.ts`). A different task, or no claim where one was recorded (or the reverse), blocks exactly like a missing marker, with the reason `session approval for <sessionId> belongs to another task: it was granted for <task>, the active claim is now <task>`, and never reads as an expired approval (so the recovery-git-commit exemption stays closed). This holds however the previous task ended, including a merge in the UI or on GitHub or a webhook moving it to `done`, where no PostToolUse hook in the session sees the completion. Resuming the same task id (a `task_start` on it again) makes the marker count again. A session marker written by an earlier release carries no binding and is refused (`carries no task binding`); one fresh `harness approve understanding` re-binds it. The task-scoped `task-<id>` marker is unchanged and carries no binding field.
...
The approval marker has no TTL of its own: once `harness approve understanding` writes `harness.generated/.approvals/<sessionId>`, the gate is satisfied for the lifetime of that session id while the active claim names the task the approval was granted for (task binding, see "Approval state"), until a lifecycle trigger below re-arms it. The contract is "one Understanding Report approved per session and task, not per tool call"; a time-bounded shape similar to `within: 1h` on other policies is a possible v2 feature, not v1 behaviour.
...
### Lifecycle re-arm triggers and in-flight subagents (task `496660c5`)

**What re-arms the gate.** Four independent triggers can force a session back through the gate, and any one of them firing is enough, they are not additive requirements:

1. **`approval_lifecycle.expire_on_tool_match`**, a configured MCP tool name completes (PostToolUse), clearing the marker (agent-tasks task-boundary tools by default). For an agent-tasks claim lifecycle verb the boundary follows the active-claim marker (task `5018c0c4`): a `task_finish` that lands the task in `review` keeps the work claim and so keeps the approval, see "What the pack ships" below.
2. **`approval_lifecycle.expire_on_bash_match`**, a Bash call matches one of the configured regex patterns (gh-CLI / pure-Bash task boundaries).
3. **`approval_lifecycle.max_age`**, the marker's signed `approvedAt` ages past the configured duration; a safety net independent of the other two, and, since task `496660c5`, the one TTL that still applies even under `mode: session` (see below).
4. **Operator removal of the marker**, `rm harness.generated/.approvals/<sessionId>` (or the task-scoped marker), by hand or via `harness approve understanding --session <new>` writing a fresh one for a different id.
5. **A different active claim** (task `5018c0c4`): the session marker counts only while the active claim names the task it was granted for, so a `task_start` on another task stops it without deleting anything (see "Approval state"). A task-scoped marker for the new task, pre-approved with `--task`, still opens the gate.

`{ mode: "session" }` opts a session out of triggers 1 and 2 only, the tool/bash boundary expiry, reverting to the legacy "one approval per session" contract for a single task. It does **not** opt out of `max_age`, nor of the task binding (trigger 5): under `mode: session` too, a session approval stops counting when the agent claims a different task. a `{ mode: "session", max_age: "4h" }` config still expires the marker on a fixed TTL even though no tool or Bash call ever clears it early. `parseApprovalLifecycle` parses `max_age` on both branches through the same helper, so a malformed `max_age` warns identically either way (task `496660c5`, `CHANGELOG.md` and the ADR amendment "in-flight subagent records close the mid-session re-arm gap" in `docs/decisions/2026-08-27-ug-auto-mode-approval.md`).

**What an in-flight subagent inherits, and what it never gets.** An Agent-tool subagent shares its parent session's `session_id`, it has no session of its own to approve, and no way to call `harness approve understanding` itself. Without a copy of the parent's approval, a re-arm trigger firing on the parent mid-task (most commonly `max_age`, or an agent-tasks boundary tool the orchestrator calls between subagent dispatches) strands every in-flight subagent: the marker it would otherwise rely on is gone, but the subagent did nothing wrong and has no path to re-approve.

Claude Code's `SubagentStart` hook (`harness pack hook subagent-start`) closes this by writing a signed, HMAC-protected **in-flight record** at `<generatedDir>/.inflight/<sessionId>/<agentId>`, but only at the moment the subagent starts, and only when the parent's own `checkOperatorApprovalMarkers` check matches at that instant (task-scoped marker first, then session marker, both under the same `approval_lifecycle`). A record is a copy of authority the parent already held, never a fresh grant: a parent with no valid approval at spawn time gets no record written for the subagent it spawns.

- **What it inherits:** the parent's approval, frozen at spawn time, presented on the subagent's behalf by the `PreToolUse` blocker (see below) until `SubagentStop` clears the record or the 24-hour staleness window (`DEFAULT_INFLIGHT_STALE_AFTER_MS`) passes, whichever comes first, an unusually long-running subagent still eventually needs its own re-approval path.
- **What it never gets:**
  - **A record after a re-arm.** A subagent started *after* the parent's marker was re-armed (deleted, expired, or freshly reapproved under a new session) gets no record for the old approval, there is nothing left to copy at that point. This is the load-bearing property that closes the bypass: a re-armed orchestrator cannot launder new work through freshly-spawned subagents, because each subagent's record is pinned to the approval state at its own start time, not to whatever the parent holds later.
  - **Consultation on a main-line call.** `PreToolUse` only consults `.inflight/` when the payload names a non-empty `agent_id`; a main-line call from the orchestrator itself never carries one, so no in-flight record, however fresh, can ever open the gate for a main-line call.
  - **A record on Codex.** `SubagentStart`/`SubagentStop` are Claude-Code-only events; Codex has no analogous concept and no record path. The Codex `PreToolUse` hook is unchanged and stays fail-closed for every call.

`PreToolUse` (`src/cli/pack/hook-pre-tool-use.ts`) consults the in-flight record only after the operator-marker check has already missed, matching against the record recomputed from the requested `(session_id, agent_id)`, never from anything stored in the record body, which closes a copy-onto-a-different-id forgery the signature alone would not catch. A match allows with `source: "inflight"`; a record that exists but fails verification (forged, or its body disagrees with its own path) is treated the same as a forged operator marker, never silently retried as an auto-approval; a genuine miss blocks with the suffix `subagent <aid>: no in-flight approval record (the parent session held no valid approval when this subagent started, or the record was removed); stop cleanly and report to the orchestrator instead of retrying.`, naming explicitly that only the parent's next `subagent-start` call, never the subagent itself, can produce a new record. A record that exists but is older than the staleness window blocks with `subagent <aid>: the in-flight record for this subagent is older than the staleness window`, and one that failed verification with `subagent <aid>: the in-flight record for this subagent failed verification`, each followed by the same stop-and-report instruction. When the parent is a delegated `claude -p` child whose report scan timed out, the delegation retry instruction takes precedence in the agent-facing deny text and the subagent sentence stays in the audit reason only, so the agent receives exactly one instruction.

**Doctor and gc.** `harness doctor` reports `in-flight subagent records on disk: N (M stale)` from `<generatedDir>/.inflight/` (silent when the directory is absent, mirroring the `.delegations/` and `.approvals/` sections). `harness gc` sweeps records older than 24 hours or dated implausibly far in the future (more than 5 minutes of clock skew), the same fixed window `verifyInflightRecord` itself uses, not `--retention-days`-tunable.

**Recommended lifecycle for orchestrator-led sessions.** An orchestrator that dispatches Agent-tool subagents across a batch of tasks should configure `approval_lifecycle: { mode: session, max_age: "4h" }` (or a duration matched to the batch's expected wall-clock length): `mode: session` keeps an approval valid without re-arming on every agent-tasks boundary tool call, while `max_age` still forces periodic re-approval so a session cannot stay approved indefinitely. Since the task binding (task `5018c0c4`), the session marker covers only the task claimed when it was granted, so an orchestrator that claims each task of the batch itself pairs `mode: session` with the batch `--task` pre-approval already described above (`harness approve understanding --task a b c`), which writes one task-scoped marker per task in a single operator action; a batch run where no agent-tasks claim is held at all keeps one session approval across the batch as before. The active-claim file is one per `harness.generated/`, shared by every session and subagent on the machine, so a claim taken by any of them moves the binding check for all of them. Either way is a valid choice, but silently exempting subagent sessions from re-approval altogether, or defaulting every install to arm-at-start-only, were both considered and rejected (see the ADR amendment in `docs/decisions/2026-08-27-ug-auto-mode-approval.md`).
...
This emits `harness.generated/codex/config.toml` (instead of `settings.json`) with harness-managed Codex hook groups such as `[[hooks.UserPromptSubmit]]`, `[[hooks.Stop]]`, `[[hooks.PreToolUse]]`, and `[[hooks.PostToolUse]]`. Each group contains a nested command hook entry (`hooks = [{ type = "command", command = "...", timeout = 5 }]`); the PreToolUse blocker uses the expanded matcher `apply_patch|Bash|shell|exec_command|functions.exec_command`. With `--install`, harness replaces only the marked harness-managed block under `~/.codex/config.toml`; harness owns hook wiring only, not the operator-owned model/auth/sandbox config, and (task 6a037359) it never deletes a foreign (non harness-owned) table such as Codex's own `[hooks.state]`, `[marketplaces.*]`, or `[plugins.*]` tables: it now stops at the first foreign table it encounters rather than trusting a found END marker's position unconditionally, and refuses instead of guessing if it finds a foreign table wedged inside its own managed block. `--dry-run` and the install summary name every hook id being removed and every foreign section it preserves; a namespace where more than one table collapsed into a single preserved entry (e.g. Codex's own `[hooks.state."<key>"]` sub-tables) prints that entry with its table count, `[hooks.state] (3 tables)`, rather than the bare header alone.

**Hook trust and re-approval.** Codex tracks per-hook trust in `[hooks.state]`, keyed `<config path>:<event>:<group index>:<handler index>`, with a `trusted_hash` computed over a normalized hook identity (event, matcher, and handler config), not the raw source text; a hash mismatch, or no recorded hash at all, means Codex will not run that hook until the operator re-approves it (source: `codex-rs/hooks/src/engine/discovery.rs`'s `hook_hash` / `hook_trust_status` and `codex-rs/hooks/src/lib.rs`'s `hook_key`, openai/codex commit `30fc6864cc1318121eca1843c217fe00ce1212f1`). Harness never writes or forges `trusted_hash` entries; installing no longer deletes the table that holds them, but a hook whose command, matcher, or table position changes on an install gets a new hash under the same key (a command, matcher, or timeout change) or moves to a new key (the key embeds the table's ordinal index for that event, so a position change moves it) and needs a fresh Codex-side re-approval, same as any operator hand-edit would. Re-run the install and re-approve whenever a policy pack's hook list, or an individual hook's command or matcher, changes. Concretely: a command, matcher, or timeout change (also an `async`/`status_message` change) produces a new hash under the SAME key; a change to the hook's position in its event's list instead MOVES which key the entry lives under; either way the affected hook reads `Modified` or `Untrusted` and Codex will not run it until re-approved. A `[hooks.state]` entry can also carry a positional `enabled` override for that key, so adding or removing a hook earlier in an event's list shifts every later hook's ordinal index and can land an operator's own `enabled = false` override onto a DIFFERENT hook than the one they disabled (for example, a harness guard hook). Review Codex's hook list after any install that changes the hook roster for an event (a hook added, removed, or reordered), not only after one that changes a single hook's own command or matcher. Separately, under Codex 0.57.0+ a re-install removes the codex "stay in scope" hook block unless `stay_in_scope` is enabled in the manifest; expected, not a regression, the same class as the `post-tool-use` disappearance under `approval_lifecycle: { mode: session }` described below.
...
- Seven hooks in the harness-managed `settings.json` by default:
  ...
  - `PostToolUse` marker-expiry: `harness pack hook post-tool-use` clears the approval marker (and expires the persisted report) after a task-boundary tool listed in `approval_lifecycle.expire_on_tool_match` fires (every `harness init` scaffold lists agent-tasks `task_finish` / `task_abandon` / `task_merge` / `pull_requests_merge` / `tasks_transition`), or after a Bash call whose command matches `approval_lifecycle.expire_on_bash_match`; the emitted `matcher` widens to include `Bash` whenever that list is configured (task bea04a03) so a real `gh pr merge` / `git push` actually reaches the hook.
    The approval expires on the same boundary as the active-claim marker below (task `5018c0c4`, operator decision: align). For a listed tool that is an agent-tasks claim lifecycle verb (`ACTIVE_CLAIM_TOOL_NAMES`), `matchPostToolUseBoundary` asks the one decider, `claimEffectForAgentTasksTool`, with the event's `tool_response`; a "claim kept" answer is not a boundary. So a `task_finish` whose result lands the task in `review` keeps the session and task approval markers (the claim is kept, and both markers count only for that claimed task: the task marker by its key, the session marker by its signed task binding, so review fixes on the same task need no fresh report; scope growth from review feedback is the stay-in-scope hook's job), while `task_finish` to `done`, `task_abandon`, `task_merge` and a `tasks_transition` to `done` expire them. An absent or unreadable `task_finish` result takes the decider's fail-safe `release` answer and expires (the safe side of the gate). Tools the decider does not track (`pull_requests_merge`, `task_submit_pr` when listed) and a `task_start` an operator lists explicitly (an `acquire`, not a keep) expire on list membership as before. The runtime boundary list comes from the configured `expire_on_tool_match` alone (`parseApprovalLifecycle` returns an empty list when the key is absent, so an absent list means no tool boundary at runtime); `DEFAULT_BOUNDARY_TOOL_NAMES` only shapes the emitted PostToolUse matcher when no explicit list is configured. Every `harness init` scaffold (full, team, the interactive composer) and `docs/examples/full-manifest.yaml` therefore list `task_merge` explicitly, so the review-then-`task_merge` path expires the approval in-session when the task reaches `done`; an operator's own list needs `mcp__agent-tasks__task_merge` added by hand. A completion outside the session (a UI merge) is covered by the session marker's task binding instead. Both the Claude hook and the Codex `codex-post-tool-use` sibling pass `tool_response`.
  - `PostToolUse` active-claim tracker: `harness pack hook track-active-claim` writes `harness.generated/active-claim` on `task_start` (harness/494fd1e5). The emitted matcher (`ACTIVE_CLAIM_TOOL_NAMES`, `src/runtime/task-providers/agent-tasks.ts`) covers `task_start` / `task_finish` / `task_abandon` / `tasks_transition` / `task_merge`; `task_merge` was added by task `c86e3c4a` (previously untracked here, so a real `task_merge` tool call never dispatched to this hook at all). Clearing on `task_finish` is decided from the RESULT, not the verb, in ONE place, `claimEffectForAgentTasksTool` (same file): a `task_finish` whose resulting `task.status` is `review` KEEPS the marker (agent-tasks's own v2 semantics keep the work claim going to `review`); `done`, any other resolvable status, and an unreadable result, all release. Recovery when a marker was correctly kept on a review-state task: `task_start` on that SAME task id (no code change needed here, `writeActiveClaim` already overwrites unconditionally) rewrites the marker the same way any other `task_start` does, so `harness approve understanding` auto-resolves it again without `--task`.
    - **The tool RESULT arrives in several shapes, all unwrapped defensively (task c86e3c4a round 2).** Round 1 read only the mcp-server receipt object itself, `{ ok, task: { id, status } }` (or the `include:["task"]` full-task variant, which nests the same way); this was INERT in production, because Claude Code's real PostToolUse `tool_response` for an MCP tool call is a content-block array instead: `[{ "type": "text", "text": "<json receipt>" }]` (live capture, claude 2.1.280, verbatim redacted fixture at `tests/fixtures/track-active-claim/real-posttooluse-task-finish-2.1.280.json`). `taskFinishResultingStatus` (`src/runtime/task-providers/agent-tasks.ts`) now also unwraps an MCP `CallToolResult` object, `{ content: [{ "type": "text", "text": "<json>" }] }` (plausible on Codex, handled defensively; **UNMEASURED** -- no Codex capture exists in this repo), and a bare JSON string. Anything else, or a `JSON.parse` failure at any step, resolves to "unreadable" and the release-on-unreadable fail-safe applies (see the classifier's own doc comment for what that trades off: a kept marker for an already-finished task would misroute the fail-closed completion gate that reads this file, `hook-solution-acceptance.ts`'s `readActiveClaim`, at the cost of a possible wedge if the release turns out to have been wrong). The hook's own stderr diagnostic names which shape it fell back from (`unreadable tool_response, shape=array|string|object|absent`) so this path is distinguishable from an intentional `done` release in the operator's own trail.
    - **A release verb only clears the marker for the task id it actually acted on (task c86e3c4a round 2).** `task_abandon`, `task_merge`, and a non-review `task_finish` all used to clear the marker unconditionally; a caller holding an active-claim WORK marker for task B who then merged, abandoned, or finished an UNRELATED task A lost marker B. The hook now reads the current marker before clearing and compares it to the acted-on task id (`tool_input.taskId`, falling back to the tool RESULT's `task.id` when the input carried none): a mismatch keeps the marker and logs the near-miss (`kept active-claim <B>: <verb> on <A>`) instead of silently dropping it. When no marker exists yet, or the acted-on id cannot be resolved at all, the old unconditional-release behaviour applies (fail-safe in the same direction as the unreadable-result case above).
...
`--task` is variadic. Passing several ids (`--task a b c`, or comma-joined `--task a,b,c`) writes one task-scoped marker per id in a single operator action. The understanding gate is task-scoped (the session approval covers only the task claimed when it was granted, and the `expire_on_tool_match` hook expires the approval when a task is finished to `done`, abandoned or merged), so without this a multi-task session needs one `harness approve understanding` per task. Pre-approving the batch up front means each `task_start` finds its marker already present. This does not weaken the gate: the operator's Understanding Report still has to enumerate every task it covers; only the round-trip count collapses. With no `--task` flag the active-claim file is auto-resolved as before (single task).
...
**Consumption is Claude Code only.** The Codex adapter has no delegation consumer: `harness pack hook codex-pre-tool-use` shares slice 2's `permission_mode`-based auto-approval with Claude Code, but carries no branch that reads or verifies a `.delegations/` marker, so a Codex session can never be the delegated CHILD of this verb. Issuing is not restricted the same way: `issueDelegation`'s parent-session resolution reads `$CODEX_SESSION_ID` like any other tier, and the parent-marker check that follows it verifies signature, structure and expiry only, never which harness minted the marker, so a Codex session holding a validly signed marker CAN be the delegating PARENT of a Claude `-p` child today, with no code change. Decided, not merely unimplemented: see the "Platform scope" amendment in `docs/decisions/2026-08-27-ug-auto-mode-approval.md`, which records this cross-runtime issuing surface as an accepted consequence of the shared marker contract rather than a defect, and gap 14 in `docs/okf/codex-adapter-parity-gaps.md`.
...
**Refusals.** Every failure is a distinct diagnostic, never a silent fall-through to allow: an invalid (non-UUID) `--child-session`, a delegation binding neither `--cwd` nor `--task`, an unresolved parent session id, a missing/forged/expired parent approval marker, an absent operator signing key (never created as a side effect of delegating), an unreadable `--report` file, or one that is readable but does not parse (`report-unparseable`, checked with the same parser `persistStdinReport` applies, before anything is signed or staged).

**`--report <path>` (fallback shape).** Under `-p`, the Stop hook fires once at the end of the run, and a report the child emits before its first tool call is not yet in the session transcript at the instant the child's PreToolUse hook fires; the hook's default recovery is a bounded transcript poll (`auto_approve.report_scan.max_wait`, see the config table above). If that lag is not reliably bounded (measured true for interactive sessions, see `docs/okf/understanding-gate-auto-mode-signals.md`), the launcher supplies the report directly: `--report <path>` reads that file and copies it (mode 0600) to the CONVENTIONAL location `harness.generated/.delegation-reports/<child-sid>.md`, refusing rather than silently overwriting a DIFFERENT file already staged there for the same child session; the delegation binds that conventional copy by BOTH its content hash and its path hash, so the child's hook checks content the parent fixed, at a path the parent fixed and the hook can independently derive from the child session id alone, not a same-content copy the child placed somewhere of its own choosing (`--report` never binds the operator's own argument path directly, since the child's hook has no channel to learn it). The child's PreToolUse hook reads this back: on a report-bound delegation it offers the conventional path to `verifyDelegation`, and on success persists the file's markdown `pending` for the child session, exactly like the transcript-scan channel, then adopts it once (keyed by the report's content hash, in the same once-per-session adoption ledger the transcript scan uses) so the same launcher report cannot re-mint a second marker once the auto-marker's own TTL ages out while the delegation stays valid. The transcript scan never runs for a report-bound delegation. This precedence sits above BOTH channels, not just the transcript scan: if the child session already has a pending persisted report from an earlier run of the hook (denied once, its report already captured), the hook mints from that pending report and never reads the launcher file at all, report-bound delegation or not. The conventional path is derived from the child session id on both sides, so nothing at that path (never written, or moved/renamed away) blocks as `report_missing`; a resolvable symlink sitting there (or a delegation that binds a report path but is offered none) blocks as `report_path_mismatch`; anything else present but wrong (bytes that do not match the bound hash, unreadable, a dangling symlink, a directory) blocks as `report_content_mismatch`. The existence check runs before the path hash, so a symlinked temp root cannot flip `report_missing` into `report_path_mismatch`. `--report <path>` refuses at STAGE time (`report-unparseable`) when the file itself does not parse under the pack's configured mode (the same `resolveMode`/`toPackageMode` resolution the child's hook applies at persist time), before anything is signed or written to the conventional path, so a file that does not parse under the pack's configured mode never gets staged and its content hash never spends the child session id's one-shot adoption; `harness delegate --report` also refuses (`report-conflict`) to silently overwrite a DIFFERENT file already staged at the conventional path for the same child session; re-arming the same child sid with different (parseable) content requires removing `harness.generated/.delegation-reports/<child-sid>.md` by hand first. See `docs/decisions/2026-08-27-ug-auto-mode-approval.md`, "Amendment: the launcher-report channel is consumed by the child hook", for the full design.
...
**Cleanup.** Neither `.delegations/` nor the once-per-session adoption ledger it leaves under `.delegation-adoptions/` ever shrinks on its own; `harness gc` sweeps both under its `delegation` category, deleting a delegation marker once its own `expires` binding is past the retention window and a ledger once its delegation has expired, or once the ledger itself is older than the retention window with no marker at all for its session (gated on the ledger's own mtime, not just marker absence, so a ledger written seconds ago is never swept), while a delegation file gc cannot parse is always left in place (see `docs/CLI.md`'s `gc` row). Known residue: the conventional launcher-report copies under `.delegation-reports/` (agent-tasks 49d1ee41) are not yet swept by this category and are folded into its own follow-up (task `3ece079d`).
