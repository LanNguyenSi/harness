...
**Claude Code adapter only.** The Codex adapter has no delegation consumer: `harness pack hook codex-pre-tool-use` shares slice 2's `permission_mode`-based auto-approval with Claude Code, but carries no branch that reads or verifies a `.delegations/` marker, so a Codex session can be neither the delegating parent nor the delegated child of this verb today. Decided, not merely unimplemented: see the "Platform scope" amendment in `docs/decisions/2026-08-27-ug-auto-mode-approval.md` and gap 14 in `docs/okf/codex-adapter-parity-gaps.md`.
