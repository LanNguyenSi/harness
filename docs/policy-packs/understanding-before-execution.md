...
#### Install default, upgrade path, and the bypass-without-opt-in doctor finding (task `8f637efd`)
...
`harness doctor` reports a warning when, among the newest `--recent-sessions` observations, the PreToolUse hook itself recorded `permission_mode: bypassPermissions` for some session AND `auto_approve` does not cover it (missing entirely, `bypassPermissions` absent from `when`, or `claude-code` absent from `harnesses`). The observation the finding reads comes from the hook process, not from anything the agent can write: the same PreToolUse hook that attempts the opt-in auto-approval already sees `permission_mode` on its payload, so it also persists a small per-session record (session id, last-seen mode, timestamp; no payload content) under `harness.generated/.permission-mode-observations/`, independent of whether `auto_approve` is even configured. This keeps the finding on the same evidence-ledger trust boundary the gate itself enforces: a fact the agent could write must never be what tells the operator they meant to opt in and didn't. The finding names the exact snippet to add and the upgrade command that inserts it. `harness gc` sweeps `harness.generated/.permission-mode-observations/` under its `permission-mode-observation` category, deleting a file once its own mtime is past the retention window, same as the approval-marker sweep (see `docs/CLI.md`'s `gc` row).
