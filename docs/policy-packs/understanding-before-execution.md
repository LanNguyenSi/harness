...
The approval marker has no TTL of its own: once `harness approve understanding` writes `harness.generated/.approvals/<sessionId>`, the gate is satisfied for the lifetime of that session id, outside `mode: session` only while the active claim names the task the approval was granted for (task binding, see "Approval state"), until a lifecycle trigger below re-arms it. The contract is "one Understanding Report approved per session and task (per session under `mode: session`), not per tool call"; a time-bounded shape similar to `within: 1h` on other policies is a possible v2 feature, not v1 behaviour.
...
5. **A different active claim** (task `5018c0c4`, not under `mode: session`): the session marker counts only while the active claim names the task it was granted for, so a `task_start` on another task stops it without deleting anything (see "Approval state"). A task-scoped marker for the new task, pre-approved with `--task`, still opens the gate.

`{ mode: "session" }` opts a session out of triggers 1 and 2 only, the tool/bash boundary expiry, reverting to the legacy "one approval per session for the session's lifetime" contract; it also opts out of the task binding (trigger 5), so a session approval keeps counting when the agent claims a different task. It does **not** opt out of `max_age`: a `{ mode: "session", max_age: "4h" }` config still expires the marker on a fixed TTL even though no tool or Bash call ever clears it early. Documented residual of that exemption: under `mode: session`, a task completed outside the session (merged in the UI, moved to `done` by a webhook) followed by a `task_start` on another task keeps the approval for the new task until `max_age`. `parseApprovalLifecycle` parses `max_age` on both branches through the same helper, so a malformed `max_age` warns identically either way (task `496660c5`, `CHANGELOG.md` and the ADR amendment "in-flight subagent records close the mid-session re-arm gap" in `docs/decisions/2026-08-27-ug-auto-mode-approval.md`).
...
`--task` is variadic. Passing several ids (`--task a b c`, or comma-joined `--task a,b,c`) writes one task-scoped marker per id in a single operator action. The understanding gate is task-scoped (outside `mode: session` the session approval covers only the task claimed when it was granted, and the `expire_on_tool_match` hook expires the approval when a task is finished to `done`, abandoned or merged), so without this a multi-task session needs one `harness approve understanding` per task. Pre-approving the batch up front means each `task_start` finds its marker already present. This does not weaken the gate: the operator's Understanding Report still has to enumerate every task it covers; only the round-trip count collapses. With no `--task` flag the active-claim file is auto-resolved as before (single task).
