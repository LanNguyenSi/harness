...
`harness dry-run <prompt> [--tool <name>] [--tool-args <json>]` statically predicts which hooks fire, which policies match, and which memory directories route for a prompt, "without ledger I/O" (docs/for-agents.md line 227). src/cli/dry-run.ts confirms: it never fetches the ledger; for each matching policy it computes the `ledgerQuery` string the runtime WOULD run (`staticLedgerQuery`) and reports it alongside `requires`/`enforcement`, plus `couldMatchPolicies` with a reason. **Trigger-matching parity (since task `ea8becf5`, extended by task `aabbad63`):** `policyMatchesTool`'s `bash_match` check (`src/cli/dry-run.ts`) now tests raw-OR-normalised-OR-amp-normalised via the same `normalizeCommand` AND `normalizeCommandAmpAware` (`src/runtime/command-normalize.ts`) the real `policyMatchesEvent` uses, so a wrapper-prefixed, extra-global-option, or bare-`&`-background-job command (`echo hi & nice git status`) predicts the same match/no-match as the runtime. It is NOT fully hermetic, but IS full parity on `${REPO}`/`${BRANCH}`: `builtinsFor` derives them from the cwd alone via `resolveGitContext`, and so does the runtime (`src/cli/policy/intercept.ts`) — a per-command target-directory resolution for these builtins was built and reviewed on the same task but removed before shipping (three consecutive review rounds each found a different way it regressed security; see `CHANGELOG.md`'s `ea8becf5` entry and `src/runtime/command-normalize.ts`'s module header), so no such divergence between `dry-run` and the runtime exists today. Prediction ≠ decision: dry-run tells you a policy WOULD match, not what outcome its `requires:` evaluation would produce, because that depends on ledger state it deliberately does not read.
...
- `harness test-risk <event.json>` classifies the envelope against `risk.classifiers[]` and prints the risk profile (severity, categories, reversibility, confidence, reasons). Fail-posture, stated in the command description and the source comment: "An action no pattern matches reports as **unclassified, not as safe**"; a manifest with no `risk.classifiers[]` is valid and then EVERY action is unclassified ("unknown is not safe", src/cli/test-risk.ts lines 39-40). It reports the same classification the runtime gate uses (docs/risk-gate.md line 140), including the built-in floors (harness's own read-only verbs classify `low`; `harness preflight && rm -rf /var` still classifies `critical` because highest-severity wins).
