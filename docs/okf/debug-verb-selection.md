...
- `harness test-risk <event.json>` classifies the envelope against `risk.classifiers[]` and prints the risk profile (severity, categories, reversibility, confidence, reasons). Fail-posture, stated in the command description and the source comment: "An action no pattern matches reports as **unclassified, not as safe**"; a manifest with no `risk.classifiers[]` is valid and then EVERY action is unclassified ("unknown is not safe", src/cli/test-risk.ts lines 39-40). It reports the same classification the runtime gate uses (docs/risk-gate.md line 145), including the built-in floors (harness's own read-only verbs classify `low`; `harness preflight && rm -rf /var` still classifies `critical` because highest-severity wins).
