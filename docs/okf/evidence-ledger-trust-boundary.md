...
- Making `checkApprovalMarker` invalidate on malformed marker JSON (hands a DoS vector to anyone who can append a byte) or letting a malformed sessionId throw out of the gate hook (the top-level handler would turn it into a non-blocking exit — fail-open).
