...
// Task 2dadec9d (batch-46 follow-up): the task f1eb1c5c divergence pin
// that used to live here, and needed a file-wide `vi.mock("node:fs",
// ...)` to spy on `existsSync`'s call history, moved to its own file
// (doctor-session-start-preflight-setup-version-divergence.test.ts) so
// that mock's scope no longer leaks into every OTHER case below. See
// that file for the pin itself and its negative control.
