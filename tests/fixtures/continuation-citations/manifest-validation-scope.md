...
`` `:110` `` backtick token continues the governing citation
`src/cli/add/index.ts:77-125` stated earlier in the same paragraph. This
file is a fixture only -- it is never read by the real docs/okf ratchet
describe block (which reads `docs/okf/` itself, not `tests/fixtures/`);
it exists solely so the negative-control test below can copy it into a
scratch docs/okf-shaped directory under its real historical filename and
assert the continuation extractor still finds and flags it.
-->
