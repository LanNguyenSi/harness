...
describe("main", () => {
  ...
  beforeEach(() => {
    ...
    for (const scanDir of SCAN_DIRS) mkdirSync(join(dir, scanDir), { recursive: true });
  ...
  it("the allowlisted generate-settings.ts path with its pinned NUL removed fails (expected 1, found 0)", () => {
    mkdirSync(join(dir, "src", "cli", "apply"), { recursive: true });
    writeFileSync(join(dir, "src", "cli", "apply", "generate-settings.ts"), Buffer.from("cmd|timeout"));

    main(dir);

    expect(process.exitCode).toBe(1);
    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining("expected 1 allowlisted raw control byte(s), found 0"),
    );
  });
  ...
  it("scans root-level *.ts/*.cjs/*.mjs files too (a BEL in a root .ts fails)", () => {
    writeFileSync(join(dir, "vitest.config.ts"), Buffer.from([0x2f, 0x2f, 0x07, 0x0a])); // "//" BEL "\n"

    main(dir);

    expect(process.exitCode).toBe(1);
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("vitest.config.ts"));
  });
  ...
  it("never descends into a nested node_modules/dist/coverage inside a scanned tree", () => {
    for (const skipped of ["node_modules", "dist", "coverage"]) {
      ...
      writeFileSync(join(dir, "tests", "probe-nm", skipped, "bad.md"), Buffer.from([0x23, 0x07, 0x0a]));
    }
  ...
  it("a missing scan directory is an IO error (exit 2 naming it), not an OK scan of what is left", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("check-no-raw-control-bytes: IO ERROR"));
  ...
  it.skipIf(process.getuid?.() === 0)(
    ...
    () => {
      ...
      try {
        ...
        expect(process.exitCode).toBe(EXIT_IO_ERROR);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("IO ERROR"));
...
describe("direct run (cwd independence)", () => {
  ...
  it("scans the same repo tree from an unrelated cwd as from the repo root", () => {
    ...
    const fromElsewhere = execFileSync(process.execPath, [SCRIPT_PATH], {
      cwd: mkdtempSync(join(tmpdir(), "harness-check-no-raw-control-bytes-cwd-")),
      encoding: "utf8",
    });
