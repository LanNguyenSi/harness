...
// literal (e.g. `/[\x00-\x1f]/` typed as the literal byte instead of the
// escaped backslash-u-0000 escape sequence) makes most tools stop seeing
// the file as source at all: git reports it as binary (`git diff
// --numstat` prints `-\t-` instead of a line count), `rg`/`grep` pattern
// matches against the file's own exported symbols silently return
// nothing, and the byte round-trips unnoticed into the built `dist/`
// output. None of the other checks in this file's CI job (typecheck,
// build, test) reject it: TypeScript compiles a raw control byte inside a
// regex or string literal without complaint, and a test asserting the
// resulting value's *behavior* can pass even though the byte itself is a
// maintainability and reviewability defect (unreadable in a diff,
// invisible to a codebase search). This script closes that gap directly:
// it reads each file as raw bytes (never through a text-decoding path
// that might itself alter or hide the byte) and fails on the first
// unallowlisted one found.
//
...
//
// SCOPE. Recursively scans src/, tests/, scripts/, docs/, .github/, plus
// the repo root's own *.md/*.yml/*.yaml/*.json files (root only, not
// recursive into other root directories like node_modules/dist/coverage,
// which this script never descends into). A file whose extension is in
// BINARY_EXTENSIONS is skipped regardless of directory, so a future
// binary asset (an image, a font) dropped under docs/ or .github/ is not
// scanned as text; this repo carries no such file today. Widened past
// src/+tests/ in round 3 because the round-2 incident (a raw control byte
// landing in a CHANGELOG/ci.yml edit) sat outside the original scope.
//
...
// count differs in either direction, not only when it's nonzero. A
...
import { extname, join, relative, sep as pathSep } from "node:path";
...
// directly in the repo root, never node_modules/dist/coverage/.git).
export const ROOT_FILE_EXTENSIONS = new Set([".md", ".yml", ".yaml", ".json"]);
...
// elsewhere. Replacing the delimiter with an escape-safe form (dropping
// this entry entirely) is a documented follow-up, not required by this
// task (CHANGELOG.md's round-3 sub-entry).
...
/** Recursively collects file paths under `dir`, skipping BINARY_EXTENSIONS. */
export function collectScanFiles(dir, out = []) {
  let entries;
  try {
    entries = readdirSync(dir, { withFileTypes: true });
  } catch {
    return out; // directory doesn't exist in this checkout; nothing to scan
  }
...
/** Collects root-level files (one directory deep) matching ROOT_FILE_EXTENSIONS. */
 ...
 *
 * - "clean": no control bytes found.
 * - "allowed": not in the allowlist, and none found (never reached with
 *   a non-empty `offsets`; kept distinct from "clean" only for symmetry).
 ...
 *   differs from the pinned `count` (in either direction).
...
export function main(rootDir = ".") {
  ...
  console.log(
    `check-no-raw-control-bytes: OK — scanned ${scanned} file(s) under ${SCAN_DIRS.join(", ")} ` +
      `plus root *.md/*.yml/*.yaml/*.json, no unallowlisted raw control bytes${allowedNote}`,
  );
}
