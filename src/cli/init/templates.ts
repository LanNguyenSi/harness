...
policy_packs:
  ...
  # recorded tip. The recovery vocabulary (git switch/checkout/pull/fetch,
  # git branch -d/-D, git stash list/show, any \`harness ...\` command)
  # always passes as its OWN command, classified before any manifest or
  # ledger access — the recovery path this gate itself recommends can never
  # be starved. Chaining one of those with a curated mutation does NOT
  # exempt the mutation (task 19356be7).
...
risk:
  ...
  # (the default, shown here for discoverability) block/require_approval
  # policies fail CLOSED (\`deny-degraded\`) while warn policies stay
  # non-blocking. Set \`fail_open\` to restore the pre-0.45
  # availability-first behaviour where EVERY degraded evaluation was a
  # non-blocking \`warn-degraded\`. See docs/okf/gate-fail-posture-matrix.md.
