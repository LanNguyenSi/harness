...
// A single-line TOML table header only: `[name]` or `[[name]]`, optionally
// followed by whitespace and a trailing comment. Deliberately excludes an
// array-continuation line such as `  [1, 2],` (no trailing comma allowed
// after the closing bracket) so a multi-line array/value sitting inside a
// still-owned harness hook table is never misread as a foreign table
// header ending the owned region early (task 6a037359). A quoted key may
// itself contain brackets/dots without tripping this up only when they
// sit OUTSIDE the header's own brackets;
// `[^[\]]+` intentionally still allows a dot or a quoted `"..."` segment
// inside the name (`[plugins."a@b"]`, `[hooks.state."/p:pre_tool_use:0:0"]`).
const NON_HOOK_TABLE_RE = /^\[(?!\[hooks\.)\[?[^[\]]+\]\]?\s*(#.*)?$/;
 ...
 * content; matching only whole trimmed lines rules that out. Lines inside
 * a TOML multi-line string (delimited by `"""` or `'''`) are skipped
 * regardless of content: a foreign table's own multi-line string value
 * may legitimately contain a line that reads exactly like the harness END
 * marker (task 6a037359) without that being a real stray END. The
 * triple-quote state is tracked only from `from`
 * onward (a string open before `from` is not detected), which is correct
 * for the caller's one use: `from` is always the start of the first
 * foreign table in the zone being searched, so any multi-line string that
 * matters here opens inside that zone, not before it.
 */
function findExactLine(text: string, target: string, from: number): number {
  let pos = from;
  let insideTripleQuoted = false;
  while (pos < text.length) {
    const lineEnd = lineEndAfter(text, pos);
    const rawLine = text.slice(pos, lineEnd);
    const trimmed = rawLine.trim();
    if (!insideTripleQuoted && trimmed === target) return pos;
    const delimiterCount = (rawLine.match(/"""|'''/g) ?? []).length;
    if (delimiterCount % 2 === 1) insideTripleQuoted = !insideTripleQuoted;
    pos = lineEnd;
  }
  return -1;
}
...
function assertNoSplitBlock(
  ...
  throw new CodexInstallRefusalError(
    `Codex config ${configPath} has a foreign table (${firstForeignHeader}) sitting before ` +
      ...
      "remain between them, then re-run `harness apply --runtime codex --install`. As a last " +
      `resort, restore ${configPath} from the most recent .harness-backup-* file, but that ` +
      "discards any Codex state (hook trust entries, marketplaces, plugins) written to the file " +
      "since that backup.",
    configPath,
  );
}
...
function collectForeignSectionHeaders(
  ...
  return headers.map((header) => {
    const count = counts.get(foreignSectionRootKey(header)) ?? 1;
    return count > 1 ? `${header} (${count} tables)` : header;
  });
}
...
function findManagedRange(text: string, configPath: string): ManagedRange | null {
  ...
  if (begin !== -1) {
    ...
    if (secondBegin !== -1) {
      ...
      throw new CodexInstallRefusalError(
        `Codex config ${configPath} has more than one '${CODEX_MANAGED_BEGIN}' marker ` +
          ...
          "guess which one is current. Delete the lines between whichever BEGIN/END pair you " +
          "do not want harness to manage, keeping exactly one, then re-run " +
          "`harness apply --runtime codex --install`. As a last resort, " +
          `restore ${configPath} from the most ` +
          "recent .harness-backup-* file, but that discards any Codex state (hook trust " +
          "entries, marketplaces, plugins) written to the file since that backup.",
        configPath,
      );
    }
    ...
    if (strayIdx === -1) {
      assertNoSplitBlock(text, end, text.length, configPath);
      return { start, end, foreignSectionsPreserved: [] };
    }

    const strayLineStart = lineStartAt(text, strayIdx);
    const strayLineEnd = lineEndAfter(text, strayIdx);
    assertNoSplitBlock(text, end, strayLineStart, configPath);
