...
//
// SCOPE: this key is HOST-GLOBAL, not per-repo. It is read from the base
// manifest plus whatever machine-override layers apply
// (`src/cli/loader.ts`'s `resolvePaths`/`applyLayers`), and the
// generated `git-preflight` SessionStart hook invokes `harness
// session-start preflight` with no `--project` flag
// (`src/cli/init/templates.ts`), so a single `setup: true` here applies
// to EVERY repository the operator opens a Claude Code session in from
// this host, not just the one it was enabled for. The only scoping
// available today is a project override layer that sets `setup: false`
// back for repositories that should not get it; there is no allowlist
// or per-repo opt-in mechanism yet.
