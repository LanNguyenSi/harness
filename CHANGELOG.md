...
- **The child's PreToolUse hook now consumes the launcher-supplied Understanding Report the `--report` fallback shape of `harness delegate` mints** (agent-tasks `49d1ee41`, follow-up to UG auto-mode slice 3, agent-tasks `37ad0b05`). Previously `harness delegate --report <path>` minted a fully signed, verifiable delegation, but the child's hook never passed `verifyDelegation` a `launcherReportPath`, so every report-bound delegation was refused (`report_path_mismatch`) unconditionally and the transcript scan was the only report channel this release actually acted on — documented in the pack doc as "issued and verifiable, not yet consumed".
  ...
  - **Adoption-once, verified for real.** Mutation probe 1: removing the content-hash comparison inside the report-bound capture branch (skip the `adopted.ids.has(entryId)` check) turns the new "(r2) ... cannot mint a second marker from the SAME launcher report once the first expired" test red; restoring the check turns it green again. Mutation probe 2: skipping the `reportRead.content` re-verification and persisting from the file unconditionally without checking `verified.ok`'s prior content match would let a modified conventional file's stale hash still mint — covered by the new "(w2) ... whose CONTENT was modified" test, which blocks on `report_content_mismatch` before any capture logic runs.
