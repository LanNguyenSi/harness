...
- **The Risk Gate's prefix parser mis-split ordinary quoted shell values, hiding production indicators from the environment resolver** (task `b093911d`). `src/runtime/bash-prefix-parse.ts` found the end of a quoted value or `cd` path with a naive `indexOf`, so `DATABASE_URL="postgres://say \"hi\"@prod-host/db" terraform destroy` and `DATABASE_URL='it'\''s prod-host/db' terraform destroy` were parsed into a truncated value AND lost their `cd` target; the same gap in `consumeLeadingCd` lost `cd /pro\ d` and `cd "/tmp/a\"b"`. That is the fail-open direction for this module: `src/cli/policy/intercept.ts` merges the result into the resolver inputs, where `env_var_patterns` substring-tests `inputs.env[name]` and `cdTarget` selects the git context for `branch_patterns`, so a truncated value misses its pattern and a lost `cd` target falls back to the hook's cwd. Fix: values and paths are scanned as shell WORDS — a concatenation of unquoted, single-quoted, double-quoted and ANSI-C `$'…'` runs with bash's per-run backslash rules. ANSI-C is covered deliberately: without it a run-based scanner would bail where the old code still returned a substring-bearing value, which would have been a new regression (the same divergence produced a CRITICAL in `13e55484`). Separately, `inlineEnv` now uses a null-prototype carrier, because `__proto__` is a grammatically valid POSIX variable name and `into["__proto__"] = v` on a plain object literal set the prototype instead of creating an own property, dropping the assignment from the resolver's view — signal loss, not a pollution vector. **Measured, not asserted (and twice corrected by measurement — see the losing-direction paragraph):** against real bash as referee through PATH shims, branch-correct went from 13/38 to 36/38 on values and 10/16 to 16/16 on `cd` targets, with **0 lost matches**, and **0 wrong `cd` targets** across 112 over-consumption probes — but that last figure clears only the shape it probed (`VAR=<value> <tail>` with the tail immediately following); separator-interleaved shapes are NOT clean and are covered below; end-to-end at the real hook entry point, two shapes that the shipped 0.42.0 lets through as "no policy matched" now block, while a staging-valued negative control still does not block on either side. **Measured in the LOSING direction too, because the first version of this entry reported only the gaining one:** the two outputs are not symmetric. `inlineEnv` is merged into the ambient env and matched by substring; that is not monotone either (the corrected value is a different string, not a superset — 10 of 15 corpus values are not supersets), but no realistic production indicator was lost in the measured corpus; `cdTarget` REPLACES the resolver's git context (`resolverGit`, `src/cli/policy/intercept.ts:515-524`), so more accuracy also points the resolver at a non-production repo the command genuinely `cd`s into. With cwd on `main` and a `cd` into a repo on a feature branch, 19 distinct spellings go from BLOCK on master to allow here (72 of 91 head/separator combinations at parse level; the enumerated examples are illustrative, not the population). **That losing side splits again, and only one half is benign.** Where bash genuinely enters the directory (`VAR=a\ b cd X`, `VAR="say \"hi\"" cd X`, `cd /esc\ aped`, …) the flip is honest: those blocks were an accident of the parse bug rather than a policy decision, three sibling spellings (`cd X`, `A=x cd X`, `cd 'X'`) already allowed on master, and the declassification lever itself is pre-existing and documented at `src/cli/policy/intercept.ts:492-507` with task `98ad072f` as the architectural fix. **Where bash never entered it, the flip was a real widening of a real bypass, and review round 2 caught it:** `VAR='';: cd DECOY ; terraform destroy` was allowed while a PATH shim showed terraform executing in the PRODUCTION repo, because the value scan concatenated runs past an unquoted metacharacter and made a later `cd` reachable that bash never performs. **CLOSED in round 3** (operator-authorised after the run's pre-declared halt criterion stopped the orchestrator from patching a security path on a third self-assessment): a value now ends at an unquoted shell metacharacter, which is where bash ends it. Phantom `cd` targets went 27 on master, 72 before the fix, **0 after**, and spellings master itself got wrong (`VAR=x||y cd DECOY ; …`) close too. **But round 3 traded that phantom for a fail-open, and the first write-up of this entry reported the trade as zero.** The "0 lost" figure came from a corpus that could not produce it: its unseparated arm rendered `A=xcd /tmp` with no space, it ran from a non-writable directory so every redirection shape was silently dropped, and its own output showed a positive control of ZERO honest `cd` targets in all three builds. Re-measured from a writable directory with the positive control asserted (84 honest shapes of 102): **8** honest `cd` targets lost against master, **16** against the pre-round-3 state. Mechanism: the value scan parks the cursor ON the metacharacter and `consumeLeadingCd`'s `skipWs` does not skip one, so a genuine leading `cd` after `A=x; ` was unreachable. **Closed in round 4**, also operator-authorised: a consumed `;` or `&&` is stepped over before the `cd` clause is retried. Every spelling round 3 let through blocks again, and the round-2 phantom bypasses stay blocked (0 phantoms remaining). **Round 4's own "0 lost against master" claim was wrong too, for the third time and by the same mechanism:** the corpus sampled the one spelling per separator where master is also null (`&` only unspaced, `>` in a shape that never ran), so those arms could not evidence a loss. Re-run with both spacings, real redirection targets, and a per-arm gate that refuses to fold an arm whose baseline never produced a target into a zero: **12** honest `cd` targets lost against master, 0 phantoms, and 11 of 16 arms explicitly reported as proving nothing. The separator matrix was wrong where it mattered. Against real bash, `;` and `&&` change the parent shell's directory and are stepped over; `||`, `|` and `(` do not and are correctly excluded; but `&`, `>` and `<` **do** change it — a redirection attaches to the same simple command rather than starting a shell, and `A=x&` backgrounds only the assignment — so those three are MISSES, not correct exclusions, and they are **not** parity with master, which captures the spaced spellings. Four of them are hook-confirmed fail-opens against master (`A=x& cd PROD …`, `A=x>o cd PROD …`, `A=x<in cd PROD …`, `A=x;y|| cd PROD …`). Pinned, not waived; the branch is held for an operator decision rather than patched a fifth time. **What this does NOT close:** ANSI-C escape DECODING (`$'a\nb'` yields the literal two characters, so an indicator that only exists after decoding — `$'pro\x64'` — is still unseen), `$` interpolation and command substitution, `pushd` / subshell `cd` / `bash -c`, and a `cd` behind `&`, `>` or `<`, which bash does run in the parent shell (12 measured losses against master, listed above). Also fixed on the way, since the two runs disagreed with each other: an unquoted `\<newline>` line continuation is now dropped the way `scanDoubleQuoted` already dropped it, so multi-line commands no longer parse differently depending on whether the value was quoted. `command-normalize.ts` is a SECOND consumer of this parser (`cd` half only, feeding its currently-unwired `targetDir`/`targetBase`), so this change moves that module's behaviour without editing it. The fall-through contract is unchanged: input with no determinable word boundary is still left unconsumed. `command-normalize.ts` keeps its own separate quote model; unifying the two remains task `d977ad58`.
