...
- **The PR #522 migration note's SYMLINK and CASING rules for `session_start_preflight`'s per-repo project layer are now pinned by automated tests, not only by the hand-run CLI matrix behind that note's CHANGELOG entry and `docs/CLI.md`'s PER-REPO SCOPING notes** (task `6c8c1bae`). A loader-level fixture in `tests/cli/loader-project-layer.test.ts` drives `explainPolicy` (its own exported `session_start_preflight.source` field, the field the hand-run matrix read) with a scratch git repository reached through a symlink: a project layer directory keyed on the symlink's own name never resolves (`source: "base"`), the one keyed on the real directory's basename resolves (`source: "project"`), from the identical symlink cwd both times, matching the SYMLINK rule. Two producer-level fixtures at the end of `tests/cli/session-start/preflight.test.ts` drive `runSessionStartPreflight` itself with an `event.cwd` that is a symlink path and a differently-cased path to the same repository, asserting the `setup` value the producer feeds to its `preflight` child comes from the project layer named after the real, on-disk basename in both cases. The CASING fixture corrects a premise in this task's own brief, measured directly against the built module before the assertion was written (`node -e` against `dist/runtime/git-context.js` and `dist/cli/loader.js`): `deriveProjectName`'s `fs.realpathSync` step does NOT canonicalize a plain (non-symlink) differently-cased path segment (the same finding `tests/runtime/git-context.test.ts`'s own case-differing test already pins), so the derived project name carries the ACCESS path's own casing, not the real basename's; what actually resolves the layer on a case-insensitive `HARNESS_HOME` (this suite's own filesystem, APFS default) is a second, independent case-insensitivity in the layer FILE lookup itself (`fs.existsSync`/`fs.readFileSync`, `src/cli/loader.ts`), which finds a directory physically spelled with the real basename regardless of the differently-cased string used to look it up. The case-sensitive-`HARNESS_HOME` half stays documented, environment-dependent, and untested in CI, per this task's own scope cut: both new producer test comments and this entry name the reason (`tests/runtime/git-context.test.ts`'s existing case-differing test already self-skips there via `ctx.skip`).
