...
- **`substituteProject` (`src/probes/memory.ts`) now applies the exported `isValidProjectName` guard, one of the sinks task `1c4eb3ea` left unguarded (agent-tasks `e904f25a`).** An invalid `--project` value (`".."`, a name containing a path separator, etc.) degrades to the "no project supplied" branch instead of being interpolated into the `{project}` placeholder: the literal survives substitution and is reported as an `unresolved: true` pattern (informational, not "missing"), never a concrete path resolved outside the directory the placeholder is meant to occupy. Two of the four sinks remain unguarded after this change, `generate-memory-index.ts`'s own `{project}` substitution and `buildLockEntries` (`src/io/harness-lock.ts`): the former is reserved for a Codex slice per the pandora handoffs (so the split, this sink guarded now, that one left for Codex, is recorded rather than worked around), the latter is a follow-up to file. Both are reached only through an explicit `--project` flag, never through a cwd-derived name. `docs/CLI.md`'s SINK GUARD sentence and the `isValidProjectName` doc comment (`src/runtime/git-context.ts`) are updated to name the new state. Pinned by a new test in `tests/probes/memory.test.ts` ("rejects an invalid opts.project" with a `".."` value); a mutation probe removing the new guard (`agent-primitives probe --pre 'npm run build'`) killed.
  ...
  - **A `--project` value is stripped of control characters before it reaches any rendered line.** `isValidProjectName` screens only for the shapes that escape a path segment (empty, `.`, `..`, `/`, `\`, NUL), so a `--project` value can carry a newline or an ANSI escape and still be accepted, or be rejected and echoed back: `harness doctor` rendered it verbatim on its header line, on the rejected-`--project` warning, on the per-directory note and on the `session_start_preflight.setup` finding's `(project: X)` suffix, and `harness list memories` carried it verbatim in the `project_rejected` row field. A crafted value therefore forged an extra, diagnostic-looking doctor line and wrote raw terminal escapes into the operator's terminal. All five rendering sites now go through one shared helper, `sanitizeProjectForDisplay` (`src/runtime/git-context.ts`, beside `isValidProjectName`); report DATA (`DoctorReport.project`, `MemoryReport.projectRejected`) keeps the raw value. Pinned by `tests/runtime/git-context.test.ts` (C0, DEL and C1 stripped; ordinary names byte-identical), `tests/cli/doctor.test.ts` (a newline-bearing and an ANSI-escape-bearing `--project`: no forged line anywhere in the rendered report, no raw escape) and `tests/cli/list.test.ts` (the row field, the `--json` surface and the text table). A second new `doctor` case pins a mixed manifest (one `{project}`-templated directory plus one concrete missing directory): both warnings appear and only the rejection is the extra one in `warningCount`. `format.ts`'s dead `projectRejected === null` re-test is now a plain `else` carrying the reason.
