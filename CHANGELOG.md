...
- **A git-verb `bash_match` policy trigger could be silently defeated by wrapping the command, and the bypass left NO audit trail at all — no ledger query, no decision, no `harness audit` row** (task `ea8becf5`). `preflight-before-investigation`'s trigger was a raw regex requiring the literal token `git` at a shell boundary, optionally preceded by `VAR=value` tokens; `env`/`nice`/`command`/`sudo`/`doas`/`time`/`timeout`/`stdbuf`/`setsid` wrappers, any git global option other than `-C`, a path-qualified `git` binary, or even a doubled space between `git` and its subcommand all defeated it. Reproduced live: `env -C <repo> git status` reached an un-preflighted repository while `cd <repo>; git status` was correctly blocked. A second, independent defect meant `${REPO}`/`${BRANCH}` never consulted what a command actually targeted: with a `preflight:harness` fact on record, `git -C <other-repo> status` run from inside `harness` was allowed — one repository's preflight authorising the investigation of another. Fix: a new pure normaliser, `src/runtime/command-normalize.ts`, peels the wrapper prefixes above (matching the git token by basename, e.g. `/usr/bin/git`) and canonicalises a git invocation's own global options (`-C`, `-c`, `--git-dir`, `--work-tree`, `--no-pager`, `--exec-path`, `--namespace`, …) down to `git <subcommand>`; `policyMatchesEvent` (`src/runtime/intercept.ts`) now tests each `bash_match` regex against the raw command and, only on a raw miss, against the normalised one — additive by construction, so nothing gated before this can become ungated, pinned by a superset test. `${REPO}`/`${BRANCH}` (`src/cli/policy/intercept.ts`) now resolve from the same normaliser's extracted target directory (`git -C`, `env -C`/`--chdir`, `--work-tree`/`--git-dir`, a leading `cd`) whenever it resolves to an actual git work tree and every git invocation in the command agrees on it; an unparseable, missing, non-repo, or internally-disagreeing target falls back to the hook's own cwd, so no command can lock an agent out of a repo it never named. Both changes live entirely in the engine, so an already-installed manifest is fixed on upgrade — no `harness apply` or manifest edit required. **What this does NOT close, so no doc or commit claims it does:** only a `bash_match` whose peeled head token is literally `git` benefits at all. `review-before-merge-bash`'s `gh pr merge` trigger, `review-subagent-before-pr-create-bash`'s `gh pr create` trigger, `dogfood-before-release`'s `npm publish` half (its `git tag v` half IS covered), and all three `deny-*` operator-only kill switches (`harness pause`/`resume`/`gate`, the session-env-strip pattern, the pause-sentinel-write pattern) remain exactly as wrapper-bypassable as before this change — do not read it as having hardened those. Also still open: `xargs git status` (deliberately excluded — `xargs` can invoke its wrapped command zero, one, or many times, unlike a transparent wrapper), a quoted subcommand (`git "status"`), backtick command substitution (`` echo `env -C /tmp git status` ``), `sh -c`, heredocs, `eval`, base64-decoded payloads, and quoted paths containing whitespace. **Correction (review round 2): `$(...)` command substitution is NOT open, unlike the backtick case right before it** — `echo $(env -C /tmp git status)` demonstrably blocks today, ACCIDENTALLY: `$(...)`'s opening paren happens to be the exact `(` character this module already treats as a shell boundary for the `(cd X && ...)` leading-subshell case, so it falls out of that, not a deliberate feature, and is now pinned by a dedicated test so an unrelated future edit can't silently drop the coincidence. Ten more measured bypasses found in the same review round (G2 finding, 2026-07-27), none of them peeled wrapper prefixes or a recognised `git` spelling: `exec`/`nohup`/`ionice`/`flock`/`script`/`chrt`/`taskset` wrapping `git status`, a backslash-escaped `git` token (`\git status`), a quoted git BINARY name (`"git" status` — the mirror image of the quoted-subcommand gap above), and `env -S "git status"` (the split-string flag hands off to a fresh, deliberately-opaque re-parse). An eleventh, `nice -10 git status` (the bare-dash form `nice(1)` documents as its PRIMARY spelling), was measured as a bypass in the same pass but is now CLOSED (G3 fix, review round 2): `nice` stays in the peeled-wrapper list, this just widens which of its own spellings are recognised. And an `env`-argument spelling like `--default-signal=INT`, which only one of the two independent env-peelers previously recognised, is ALSO now closed (G6 fix, review round 2): the earlier fix here (sharing the flag-NAME enumerations via exported constants in `src/runtime/read-only-bash.ts`) had only closed the flag-name-SET drift, not this specific bypass, which needed its own generic glued-`--long=value` catch-all mirrored into this module too; any OTHER unrecognised glued env flag is skipped the same way, though none of them carry `-C`/`--chdir`-style `targetDir` semantics to begin with. Separately (G4 finding, review round 2): above `MAX_NORMALIZE_LENGTH` (100,000 chars), normalisation used to skip SILENTLY — no stderr line, no audit row; `harness policy intercept` now emits one stderr line reporting the skip, and `docs/okf/gate-fail-posture-matrix.md` gained a matching row. A hook cwd outside any git repository still collapses the ledger tag to the bare prefix `preflight:`, which any repository's preflight satisfies (deferred, tracked separately). `${BRANCH}` is still a bare branch name with no repository qualifier. Target-directory resolution is scoped to `tool_name === "Bash"` and does not extend to Codex's shell-tool aliases (`shell`, `exec_command`). **Newly widened false positives** (pre-existing class, task `dbc6d303`): quoted or heredoc text that merely mentions a wrapped git verb can now trigger where it previously did not — hit twice live during this fix's own development, once in an Understanding Report heredoc and once in a commit message containing an `&& git push` example. An intermediate version of this fix also let the command's target-repo resolution leak into `environments.resolvers[]` risk classification (a targeted read against a repo on a non-`main` branch, chained ahead of a destructive command, silently declassified the environment) and let the first of several git invocations in one command decide `${REPO}`/`${BRANCH}` for the whole event (a targeted read chained ahead of an untargeted `git push` could make the push gate check the read's repo instead of the push's) — both closed before release, with regression tests, and neither ever shipped. **Correction (second, independent review round over this SAME task, before any of it ever shipped):** the second of those two fixes was only complete for GIT-vs-GIT disagreement — a git invocation naming an explicit target still leaked into a DIFFERENT, non-git gated verb sharing the same chain (`git -C <B> rev-parse HEAD && gh pr merge` still resolved `${BRANCH}` from B for the merge, even though `-C` scopes only that one git call and `gh pr merge` genuinely runs at the real, unaffected cwd — the review-before-merge-bash / review-subagent-before-pr-create-bash gates specifically). Closed in the same review round that also added `nice`'s bare `-<n>`/`+<n>` forms, an `env --long=value` generic catch-all mirroring `read-only-bash.ts`'s own, and a stderr line when `MAX_NORMALIZE_LENGTH` skips normalisation — see `command-normalize.ts`'s own header for the full account. `harness dry-run`'s trigger prediction now matches raw-or-normalised the same way `policy intercept` does; its `${REPO}`/`${BRANCH}` prediction still derives from cwd only, so dry-run and the runtime can still disagree when a command names a different repository. New tests: `tests/runtime/command-normalize.test.ts`, new suites in `tests/runtime/intercept-cli.test.ts` and `tests/cli/dry-run.test.ts` (review round 2 additions: the G1 git-vs-non-git-verb ambiguity cases through the real `review-before-merge-bash`/`review-subagent-before-pr-create-bash`/`dogfood-before-release` triggers, the G2 ten-spelling ceiling plus the `$(...)` accidental-coverage pin, the G3 `nice` bare-form cases, the G6 `env --default-signal=INT` case, and a `policy intercept` end-to-end pin on the G4 stderr line). Docs: `docs/okf/gate-fail-posture-matrix.md`, `docs/okf/debug-verb-selection.md`, `docs/okf/policy-engine-producer-wiring.md`, `docs/okf/codex-adapter-parity-gaps.md`.
