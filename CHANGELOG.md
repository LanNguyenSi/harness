...
- **`harness apply --runtime codex --install` no longer deletes foreign (non harness-owned) tables when a previous install's END marker had drifted past them** (task `6a037359`). `findManagedRange` used to trust `text.indexOf(CODEX_MANAGED_END, begin)` unconditionally: a dogfood run on 2026-09-23 upgrading 0.56.0 to 0.58.0 found the marker sitting after Codex's own `[hooks.state]` trust table, `[marketplaces.*]`, `[plugins.*]` and `[tui]` tables, so the install deleted all of them. The region end is now computed independently of the END marker's position (a new `scanOwnedContentEnd` stops at the first foreign table, the same `NON_HOOK_TABLE_RE` the legacy no-END path already used), a stray END found beyond that point is excised and a fresh one is re-stamped right after the regenerated block, and a comment block Codex (or an operator) attaches to the next foreign table is never folded into the replaced region. A foreign table wedged between two harness hook tables, or a moved/duplicated BEGIN marker, now refuses loudly (`assertNoSplitBlock`) instead of guessing. `harness apply --dry-run --runtime codex --install` and the post-install summary now also name every hook id being removed (a hook id present in the old block absent from the new one) and every foreign section preserved, by its table header. Pinned in `tests/cli/apply/apply-codex-runtime.test.ts` with a fixture shaped like the real evidence file (neutral placeholder paths): the fixture fails against the pre-fix implementation (every foreign table and the trailing operator content are deleted) and passes at head; a dry-run test asserts the named removed-hook-id and preserved-foreign-section lists; a split-block fixture asserts the loud refusal. Mutation probes (`agent-primitives probe --pre 'npm run build'`) restoring blind END trust and removing the stray-END cleanup both killed by the fixture.
  ...
  **Round-1 review fix** (adversarial review, task `6a037359`): the surviving mutation probes from that review pointed at guards with no fixture exercising them, not at missing logic, so each got a dedicated fixture in `tests/cli/apply/apply-codex-runtime.test.ts`: an operator comment sitting directly above the first foreign table (stays attached to that table, not folded into the replaced block, and appears exactly once); two harness-managed BEGIN markers (refused, file untouched); a BEGIN with no END where a foreign table is followed further down by a real `# harness hook:` comment (refused, since the block boundaries can no longer be read as a plain line scan); a managed block followed by a trailing operator comment at EOF (kept, not silently replaced away); exact (not `arrayContaining`) equality on `foreignSectionsPreserved` and `removedHookIds`, plus one exact byte-for-byte equality on the whole installed output for the drifted fixture. All ten of the review's surviving/replayed mutants now kill under `agent-primitives probe`. A CLI-level test (through the built program, not only the library function) now asserts `--dry-run`'s and the applied install's own stdout for the removed-hook-block and preserved-foreign-section lines (task `6a037359`'s AC-004). The refusal itself is more actionable: it now names the RESOLVED config path (never a hard-coded `~/.codex`), the first foreign table and the specific line that still looks like harness content, tells the operator to either move that table below the END marker or delete the lines between BEGIN and END, and offers the `.harness-backup-*` restore only as a last resort with an explicit warning that it discards any Codex state (hook trust, marketplaces, plugins) written since that backup; it now throws a dedicated `CodexInstallRefusalError` (a `HarnessExitError` at `EX_FAIL`) instead of a plain `Error`, and `--json` emits a structured `{ outcome: "codex-install-refuse", configPath, error }` object on stdout the same way `target-exists-refuse`/`drift-refuse` already do, instead of only the plain-text message reaching stderr. The install's own permissions no longer widen a secret-bearing config: it now preserves the EXISTING file's mode for both the rewritten config and its `.harness-backup-*` copy, defaulting to `0600` (not `atomicWriteFile`'s own `0644`) only when creating a config that did not exist before. `foreignSectionsPreserved` now prints a namespace's table COUNT when more than one table collapses into it (e.g. `[hooks.state] (3 tables)`), instead of silently representing three preserved tables as one indistinguishable header. Table-header recognition is now a full single-line TOML header shape, so a multi-line array's own continuation line inside a still-owned hook table (e.g. `  [1, 2],`) is never misread as a foreign table header ending the region early; the stray-END search now matches a whole trimmed line, not a raw substring, and skips a line sitting inside a foreign table's own multi-line TOML string, so a foreign string value that happens to contain a line reading exactly like the harness END marker is preserved untouched instead of being excised as a false stray END.
