...
- **Cross-build measurements of the Bash prefix parser now have a permanent, self-testing instrument: `scripts/measure-bash-prefix-parse.mjs` (`npm run measure:bash-prefix-parse`)** (task `47297478`). The `b093911d` run produced three consecutive measurement corpora that reported "0 lost cd targets" while being structurally incapable of reporting a loss (round 3: the separator-less arm rendered `A=xcd /tmp` and a non-writable cwd silently killed every redirect shape; round 4: exactly the one spelling per separator where the baseline is also null), each time turning into a false safety claim that cost a full review round. The tool hardens that run's `final-audit.mjs` into repo infrastructure: a candidate build of `src/runtime/bash-prefix-parse.ts` is measured against real bash (PATH-shim referee in a throwaway writable workdir) and at least two baseline builds, with a PER-ARM GATE — an arm whose shapes never ran, never entered the target, or whose baseline never hit is reported as proving nothing and is never folded into a total zero; a total that excludes such arms says so instead of printing a bare 0. Both residual weaknesses the run's reviewer named in `final-audit.mjs` are fixed: every baseline keeps independent counters (a loss against two baselines counts against both, no `else if` folding), and a non-null-but-WRONG target is its own failure class for candidate and baselines alike, never a hit — so a wrong-target baseline cannot satisfy the gate either. The instrument proves it can fail before it is believed: a self-test (always run first; `--self-test` runs it alone) rebuilds the round-3 defect on purpose and requires the gate to flag the sabotaged arm as evidence-free, and requires the healthy corpus to produce a measured separator-less arm and executed redirect shapes, so a broken environment fails loudly. Measurement runs refuse to start with fewer than two `--baseline name=path` builds. Corpus commands only ever execute the probe shim, nothing real. New tests: `tests/scripts/measure-bash-prefix-parse.test.ts` (hermetic audit-core suite with injected referee/parsers, plus a real-bash CLI self-test e2e via the INFRA-allowlisted `node`). `bash-prefix-parse.ts` and `command-normalize.ts` carry a MEASUREMENT RULE header pointer so nobody measuring those modules can miss the tool. The parser itself is untouched.
