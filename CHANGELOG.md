...
- **`harness doctor` warns on a live Codex `approval_policy = "never"` or
  ...
  path consumes, measured to always resolve to `bypassPermissions`
  (`dogfood/ug-auto-mode-signals/README.md`, sections (k) and (l)), the
  Codex counterpart of the existing `permissions.defaultMode`
  settings-drift warning for Claude Code. Gated on
  `auto_approve.harnesses` listing `codex`, so a repo that never opted
  Codex into the auto path sees no line. Warning only, never an
  enforcement; new module `src/cli/doctor/codex-config-drift.ts`.
