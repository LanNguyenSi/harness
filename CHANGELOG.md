...
- **`understanding-before-execution`'s `config.mode` (harness.yaml) was config-drift: it drove prose only, never enforcement** (task `5d73d78d`). Live repro in an isolated home: `config.mode: grill_me`, an agent report with no `mode:` metadata field, `harness approve understanding` (stdin heredoc) → output read `validation: ✓ fast_confirm report passed structural checks` and persisted `"mode": "fast_confirm"`, silently skipping the `derivedTodos` / `acceptanceCriteria` / `priorArt` sections `grill_me` requires. Root cause, two independent sites: (1) the Claude-runtime `UserPromptSubmit`/`Stop` hooks are bins shipped by `@lannguyensi/understanding-gate` (`understanding-gate-claude-hook` / `understanding-gate-claude-stop`), invoked bare — no `UNDERSTANDING_GATE_MODE` env var — so the package's own env-then-marker-then-`fast_confirm`-default resolver always fell through to its default; (2) `persistStdinReport`'s gap-fill for a report with no declared mode hardcoded the literal `"fast_confirm"`. Fix: `resolveMode()` (`src/policy-packs/builtin/understanding-before-execution.ts`) now resolves **`UNDERSTANDING_GATE_MODE` env > `config.mode` > default (`grill_me`)**, unifying what was previously two independent, disagreeing resolvers into one; `buildHooks` forwards the resolved value to both npm-backed Claude bins via the same env var (`wrapMode`, mirroring the existing `UNDERSTANDING_GATE_REPORT_DIR` wrap); `approve/understanding.ts` resolves the same way against the loaded manifest and passes it into `persistStdinReport` instead of the hardcoded default. `strict` has no upstream equivalent (the package's own mode type is two-valued); `toPackageMode()` coerces it to `grill_me` — the closest available rigor — only at that interop boundary, not inside harness's own `strict` semantics. **Behavior change for any host with `config.mode` explicitly set to anything other than `fast_confirm`: that mode is now genuinely enforced** (previously-accepted reports missing `derivedTodos` / `acceptanceCriteria` / `priorArt` are now rejected until the agent supplies them). New coverage: `tests/policy-packs/resolve-mode.test.ts` (env/YAML/default priority, invalid-value warnings, `toPackageMode`) and new cases in `tests/policy-packs/expand.test.ts` (the `UNDERSTANDING_GATE_MODE`-wrapped hook commands, config-driven, env-override, and `strict`-coercion).
