...
- **`tests/scripts/memory-frontmatter-contract.test.ts`'s "uses the script location and keeps sync transactional through recovery faults" test now carries an explicit 20000ms per-test timeout** (task `1353a96e`). The test drives roughly 90+ synchronous `execFileSync("git", ...)`/`spawnSync(node, ...)` calls across its pinnedUpdatedFixture() passes, CLI invocations and fault-injection runs; measured 2.8-4.1s in an ordinary checkout but repeatedly up to ~6.2s (1/10 local repeats timing out at vitest's default 5000ms `testTimeout`) from a detached git worktree placed under `os.tmpdir()`, the same environment `agent-primitives probe`'s default isolation copy uses. This is the cold-subprocess-spawn-latency-vs-fixed-budget race already documented in `vitest.config.ts` for `tests/policies/ledger-client.test.ts`, `tests/probes/mcp.test.ts`, `tests/runtime/ledger-add.test.ts`, `tests/io/ledger-record.test.ts` and `tests/cli/doctor-codex.test.ts`, not the `os.tmpdir()`-exemption-swallow mechanism task `9a4a417b` (PR #527) fixed in `tests/runtime/hermetic-spawn-allowlist.test.ts`: this test derives no expected path from the script's own location or from `os.tmpdir()`, so there was no location assumption to correct. Fixed the same way `tests/cli/smoke/smoke.test.ts`'s "escalates to SIGKILL" test already is: a generous explicit timeout, no assertion weakened or removed.
