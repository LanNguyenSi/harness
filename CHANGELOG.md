...
- **SECURITY: a quoted env-assignment value with embedded whitespace (`VAR='hello world' git push`) bypassed every `bash_match` gate on BOTH matching layers — closed by continuing the assignment's value across tokens in the normaliser** (task `13e55484`, found by a read-only explorer during `d977ad58` and verified against the shipped binary, not derived). Measured at the real hook entry point with shipped 0.42.0 as control: `VAR='hello world' git push origin master` and `VAR='hello world' gh pr merge 4242 --squash` both produced "no policy matched" while their plain spellings blocked. The two layers failed on the same character: the raw regex's `(\w+=\S+\s+)*` cannot span an embedded space, and `command-normalize.ts`'s whitespace tokenizer split the assignment into `VAR='hello` + `world'`, the second token aborting the wrapper-peel loop so the segment came back byte-identical. Fix: a `consumeAssignment` helper continues an assignment's VALUE across whitespace-split tokens while an opening quote from that assignment stays unbalanced at a token's end AND a matching close exists in a later token, used by both the top-level peel loop and `peelEnv`'s own assignment scan (`env VAR='a b' git push` was the same bypass one wrapper deep) — deliberately NOT a general quote-aware tokenizer, and raw-first matching is untouched, so the change is additive by construction. Never-unmatch is pinned two ways: an UNTERMINATED quote (`VAR='a git push`) keeps the pre-task one-token consume because that spelling normalised to a match before and must keep doing so, and a corpus property test carries an ENGAGEMENT assurance (≥8 corpus entries must match ONLY via normalisation) so it cannot rot into an all-raw vacuous corpus — the dbc6d303 lesson applied. Closure pinned per variant: double quotes, multiple mixed-quoting assignments, assignment+wrapper, env-internal assignment, tab-in-value, chained quote runs (`'a b'"c d"`), and the second gated verb; mutation-probed (reverting the two call sites turns exactly the 8 closure pins + corpus red). **What this does NOT close, pinned as still-open rather than implied:** backslash-escaped whitespace without quotes (`VAR=a\ b git push`) is the escape-handling class of task `b093911d` and remains a measured bypass; the RAW regex half is deliberately unchanged (the engine-level fix heals installed manifests on upgrade, per this module's contract). Latency: micro-benched sub-microsecond per call, intercept wall time indistinguishable from shipped.
