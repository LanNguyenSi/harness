...
- **An unclassified action no longer satisfies `risk.severity_at_least: critical` on its own** (agent-tasks `2929c5b7`). It still satisfies `low`/`medium`/`high` (`risk.category_in` and `action.reversible` keep the full "unknown is not safe" fallback unchanged), but the top tier now requires a real classification — `evaluateWhen` (`src/runtime/when-eval.ts`) treats an unclassified profile as sitting at the "high" rung, one below `critical`, for `severity_at_least` comparisons only. No manifest opt-in restores the old blanket behavior; see `docs/risk-gate.md`'s "Unclassified actions and the fail-close rule".
  - **Paired classification changes, so the loosened fallback opens no bypass.** The Risk Classifier's read-only floor (`src/runtime/read-only-bash.ts`) gained per-bin write-flag guards for `sed` (in-place edit) and `curl` (data/upload/form flags, non-GET/HEAD methods). The shipped `dangerous-shell` classifier (`docs/examples/full-manifest.yaml` / `FULL_TEMPLATE`, kept in lockstep by `tests/cli/init-full-template-parity.test.ts`) gained explicit patterns, at the severity the existing patterns already use for a comparable action: `critical` for `dd` (with an `of=` target), `truncate` (`-s`/`--size`), `shred`, `mkfs`/`mkfs.*`, `find ... -delete`, `find ... -exec(dir) rm`; `high` for `git reset --hard`, `git push --force`/`--force-with-lease`/`-f`, `git clean -f`, `git checkout -- .`, `git restore .`, `chmod -R`/`chown -R`, `curl -X POST|PUT|PATCH|DELETE`, `curl -d`/`--data*`/`--upload-file`, `sed -i`/`--in-place`. A bare truncating redirection (`> file`) is a documented, NOT-closed gap: the classifier is regex-over-raw-string with no redirection model, and a heuristic for it would over-match `2>&1`/heredocs/comparison operators.
  ...
  - **The `ux:`-declared deny envelope now names a fallback-caused deny**, not just the neutral non-`ux:` one. When a policy's `when:` block matched via the fallback (not a real classification), a sentence, "This is an unclassified action in a production context: no risk classifier pattern recognized it, so the fail-closed severity fallback applied rather than a genuine critical-severity match.", is prepended to the operator's own `cannot:` text (`src/runtime/intercept.ts`); the operator's text itself is unchanged, and a genuine classification hit renders it with no prefix, as before.
  - Docs: `docs/risk-gate.md` ("Unclassified actions and the fail-close rule" and "Built-in read-only commands").
