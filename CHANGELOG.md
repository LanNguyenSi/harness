...
- **`harness pack hook track-active-claim` no longer clears `harness.generated/active-claim` on a `task_finish` that lands the task in `review`** (task `c86e3c4a`). The hook used to treat every `task_finish` as a release, disagreeing with agent-tasks's own documented v2 semantics (the work claim is cleared going to `done`, kept going to `review`); the two disagreeing left a session wedged (`task_start` on another task returns `already_claimed`, `task_abandon` on the review-state task returns `bad_state`, and the completion gate refuses every push/`task_merge` with "no active-claim task id recorded"). The hook now reads the tool RESULT, not the verb: `task_finish`'s receipt (mcp-server receipt v1, `{ ok, task: { id, status }, deviations? }`) carries the resulting `task.status`, and only a resulting status of `review` keeps the marker; `done` and any other resolvable status still release it, matching the prior behavior. An absent or malformed result (no `tool_response`, or `task.status` not a string) fails safe by releasing rather than risking a marker that never clears: the active-claim file is an ergonomic auto-resolve shortcut, not a security-relevant signal, so losing the shortcut is the safer failure than wedging the gate. `task_merge` is now also recognized as a release (previously untracked by this hook's own classification, falling through as "not tracked"), alongside the existing `task_abandon` release path. The recovery path (`task_start` on the SAME review-state task) needed no code change: `writeActiveClaim` already overwrites unconditionally, so it rewrites the marker on the next `task_start` the same way it always has; this is now named explicitly in `docs/policy-packs/understanding-before-execution.md`. This intentionally bypasses `claimEffectForAgentTasksTool`'s own `task_finish` classification (`src/runtime/task-providers/agent-tasks.ts`), which still treats every `task_finish` as an unconditional release and is out of this task's allowed-changes scope to touch; the same applies to the generated PostToolUse matcher (`understanding-before-execution.ts`'s `TRACK_ACTIVE_CLAIM_MATCH`/`_CODEX`), which is still built from `ACTIVE_CLAIM_TOOL_NAMES` and therefore does not yet dispatch a real `task_merge` tool call to this hook (a wiring gap, filed as a risk below, not fixed here). Pinned by new `tests/cli/pack-hook-track-active-claim.test.ts` cases: finish-to-done clears, finish-to-review keeps, finish-with-no-result and finish-with-malformed-status both fail safe (clear), `task_merge` clears, and a review-then-restart sequence pins the recovery path end to end. Mutation probes (`agent-primitives probe --pre 'npm run build'`) reverting to clear-on-every-finish and to keep-the-marker-on-done both killed by the new fixtures.
