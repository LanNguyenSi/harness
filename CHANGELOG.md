...
- **`harness doctor`: warn when `npm prefix -g`/bin is not on PATH** (harness/4ddd78ed). Surfaces the nvm-drift footgun where `harness init --interactive` runs `npm i -g` against the active Node's prefix but the operator's shell PATH points at a different one, so installed binaries are silently invisible to subsequent doctor probes. Doctor now resolves the bin dir via `npm prefix -g` (the modern replacement for the removed `npm bin -g`) and renders an `Environment` section with the actionable PATH-patch suggestion when the bin dir is not in `process.env.PATH`. The section stays absent when the bin IS on PATH (no noise) and when npm itself is unreachable (the other dep checks already fail loudly enough). Skipped under `--shallow`, which stays in its real-spawn-free contract. Implementation in a new `src/cli/doctor/npm-bin-path.ts` module with discriminated-union return shape, test-injectable spawn. Covered by `tests/cli/doctor-npm-bin-path.test.ts` (6 unit cases pinning each branch + edge cases like empty PATH segment + whitespace trimming) plus 4 integration tests in `tests/cli/doctor.test.ts` exercising the warn / ok / unknown / count-rollup paths through the doctor pipeline.
