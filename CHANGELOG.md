...
- **SECURITY: post-merge-gate blocker precedence reversed — deny now wins over the escape allowlist, closing the chained-escape bypass where `harness preflight && git push origin master` (the documented preflight-then-push workflow) skipped the gate entirely** (task `19356be7`). `isEscapeCommand` was an OR over the WHOLE command string and `hook-post-merge-gate.ts` checked it first and unconditionally, so any chained command containing an escape verb anywhere (`git switch`, `git stash list`, any `harness ...`) fell out of the gate including its mutation half. The blocker now classifies via the new `isGateEligibleCommand` (post-merge-gate-runtime.ts): gate-eligible iff the curated mutation matcher fires after quoted-heredoc bodies attached to a harness invocation are stripped. **Measured before choosing (62-row corpus, bash ground truth via PATH stubs; `.ai` run `2026-08-02-post-merge-gate-escape-precedence`):** the task's three candidate shapes (per-segment, deny-wins, escape-at-start-only) are 0-divergent — a consequence of the pinned escape/deny verb disjointness — so the simplest (deny-wins) shipped; 0 false-frees, 0 recovery-chain lockouts (`git switch <default> && git pull --ff-only` stays free structurally, allowed before any manifest/ledger access), 16 bash-confirmed real mutations newly gate-eligible, 4 accepted quoted-text false positives (the existing `echo "a&git push"` class, no longer rescued by a chained escape verb). The new `stripHarnessHeredocBodies` keeps `harness approve understanding <<'…'` report bodies (stdin data, never executed) from becoming gate-eligible — without it, deny-wins would deadlock the understanding gate against the post-merge gate; scoped to harness consumers because `bash <<'EOF' … EOF` really executes its body (measured; a general strip would have freed it). Escape regexes stay exported as deny-message vocabulary and diagnostics. Pack instructions, `docs/policy-packs/post-merge-gate.md` (Chained-escape bypass gap marked CLOSED with its measured residual), and the deny message's escape-hatch wording updated to match.
