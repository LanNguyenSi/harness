...
- `harness apply --runtime codex --install`'s split-block refusal now always names the actual foreign table header, including when the table is wedged directly between a `# harness hook:` comment and the `[[hooks.*]]` table that comment belongs to (previously printed `(unknown)` in that shape, because the owned-region scan had already backed the zone start off to only blank or comment lines preceding the found marker, so the pre-marker header lookup found no content line there; it now also looks just past the marker's own line when that happens). Pinned with two fixtures: the table wedged after a lone `# harness hook:` comment, and the tracker's own multi-hook shape where a second hook's comment directly follows the first hook's content line, so nothing at all (not even a blank/comment run) precedes the marker in the zone. A mutant that only applies the fallback when something other than the marker itself precedes it survives the first fixture but is killed by the second. Safety was unaffected in every case (refusal still fires, exit `EX_FAIL`, file left untouched, no secret value ever echoed, now also pinned for this wedged-after-comment shape in the no-echo matrix): only the naming in the message text was wrong. (task `25a4c53f`)
