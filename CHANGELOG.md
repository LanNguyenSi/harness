...
- **Codex sessions now expire the understanding-gate approval marker on task-completion boundaries, closing the last runtime-parity gap the pack's `approval_lifecycle.expire_on_tool_match` / `expire_on_bash_match` config left open on Codex** (task a1348c89, follow-up to the `[0.39.0]` task e7c2ec3c TTL/task-marker parity fix, whose entry named this exact residual). Before this, the Codex branch of `buildHooks` emitted only three hooks (UserPromptSubmit, Stop, PreToolUse); a Codex approval marker survived every `task_finish` / `task_abandon` / `pull_requests_merge` and only died via `approval_lifecycle.max_age` or a manual `rm`, so a long Codex session kept re-approving stale interpretations across unrelated tasks. `harness apply --runtime codex` now also emits a `[[hooks.PostToolUse]]` group running the new `harness pack hook codex-post-tool-use` (`src/cli/pack/hook-codex-post-tool-use.ts`), which shares its boundary-match and marker/report-clearing logic with the existing Claude `post-tool-use` hook through three newly-extracted functions in `understanding-before-execution-runtime.ts` (`matchPostToolUseBoundary`, `applyPostToolUseExpiry`, `describePostToolUseExpiry`) so the two runtimes cannot silently drift on which tools trigger expiry — the same drift class task e7c2ec3c fixed on the PreToolUse side. Codex's shell-tool aliases (`Bash`/`shell`/`exec_command`/`functions.exec_command`) all satisfy `expire_on_bash_match`, matching the PreToolUse blocker's own alias set; the hook resolves `session_id` from `$CODEX_SESSION_ID` ahead of the Claude env fallbacks and prefers the real Codex `tool_input` field over the portable `raw_input` shim field. Advisory only (never blocks; every error path is exit 0 + stderr). The Codex `PostToolUse` group schema support (`[[hooks.PostToolUse]]`) already existed in the generator from PR #211 against the real Codex 0.131.0 hook schema — this task contributes the pack-side hook and the CLI verb that consume it, not new generator plumbing. `docs/policy-packs/understanding-before-execution.md`, `docs/CLI.md`, and `docs/okf/codex-adapter-parity-gaps.md` updated; the active-claim tracker and stay-in-scope reminder remain Claude-only (tracked separately). Pinned by 24 new tests (`tests/cli/pack-hook-codex-post-tool-use.test.ts`) mirroring the Claude suite's coverage plus Codex-specific cases (tool/tool_name and tool_input/raw_input synonyms, all four shell aliases, the `$CODEX_SESSION_ID` fallback) and an integration must-pass control proving the expiry genuinely re-locks the PreToolUse blocker rather than just deleting an unread file.
