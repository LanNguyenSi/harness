...
- **SECURITY: post-merge-gate blocker precedence reversed — deny now wins over the escape allowlist, closing the chained-escape bypass where `harness preflight && git push origin master` (the documented preflight-then-push workflow) skipped the gate entirely** (task `19356be7`). `isEscapeCommand` was an OR over the WHOLE command string and `hook-post-merge-gate.ts` checked it first and unconditionally, so any chained command containing an escape verb anywhere (`git switch`, `git stash list`, any `harness ...`) fell out of the gate including its mutation half. The blocker now classifies via the new `isGateEligibleCommand` (post-merge-gate-runtime.ts): gate-eligible iff the curated mutation matcher fires after quoted-heredoc bodies attached to a harness invocation are stripped. **Measured before choosing (69-row corpus, bash ground truth via PATH stubs; `.ai` run `2026-08-02-post-merge-gate-escape-precedence`):** per-segment evaluation and deny-wins are 0-divergent — a consequence of the pinned escape/deny verb disjointness — so the simplest (deny-wins) shipped. The third candidate, escape-at-start-only, was measured and REJECTED: it diverges on 26 rows and still exempts three of the four lines this task exists to close. Against the shipped classifier: 0 false-frees, 0 recovery-chain lockouts (`git switch <default> && git pull --ff-only` stays free structurally, allowed before any manifest/ledger access), 22 bash-confirmed real mutations newly gate-eligible, 4 accepted quoted-text false positives (the existing `echo "a&git push"` class, no longer rescued by a chained escape verb), and exactly one shape more permissive than before (`sleep 0 & harness note <<'EOF'`, where bash runs no mutation — pinned). The new `stripHarnessHeredocBodies` keeps `harness approve understanding <<'…'` report bodies (stdin data, never executed) from becoming gate-eligible — without it, deny-wins would deadlock the understanding gate against the post-merge gate; scoped to harness consumers because `bash <<'EOF' … EOF` really executes its body (measured; a general strip would have freed it). It is a quote-aware scanner rather than a regex: review found six shapes where a raw-line regex mistook quoted TEXT (`"see <<'EOF' syntax"`) or mis-ordered paired `<<A <<'B'` operators for real heredoc framing and swallowed lines bash genuinely executes — all six are closed and pinned, and anything the scanner cannot resolve now strips nothing at all. Escape regexes stay exported as deny-message vocabulary and diagnostics. Pack instructions, `docs/policy-packs/post-merge-gate.md` (Chained-escape bypass gap marked CLOSED with its measured residual), the blocker hook's `description`, `harness pack hook post-merge-gate --help`, the generated-manifest comment in `harness init --template full`, and `docs/examples/full-manifest.yaml` all updated to stop teaching escape-first, with drift pins on the operator-visible surfaces.
