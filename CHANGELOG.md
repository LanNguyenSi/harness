...
- **`verifyDelegation` now distinguishes an absent conventional launcher report from a moved or content-modified one, with a dedicated `report_missing` reason** (agent-tasks `204efc56`, follow-up to `49d1ee41`). Previously an absent `harness.generated/.delegation-reports/<child-sid>.md` surfaced as `report_content_mismatch` (or, non-deterministically under a symlinked temp root such as macOS `os.tmpdir()`, `report_path_mismatch`), which told the operator the report was wrong rather than that it did not exist.
  - `DelegationRefusalReason` (`src/policy-packs/builtin/understanding-before-execution/delegation-markers.ts`) gains `report_missing`. A stat-only existence probe (`probeRegularFilePresence`, `src/io/read-regular-file.ts`) runs BEFORE the path hash: existence is decided independently of `hashDelegationCwd`'s missing-path fallback (`path.resolve`), so a symlinked temp root can no longer flip the reason. This is a bare `lstat`, not a read: a resolvable symlink or a directory answers "present" and falls through, so a resolvable symlink is still caught one step later at the path-hash comparison (`report_path_mismatch`), and only a dangling/looping symlink or a directory reaches the single full read (`readRegularFileRejectingSymlink`, still after the path hash and the content-hash presence check pass) and is refused there (`report_content_mismatch`). The ok path still hashes and returns one set of bytes exactly as before; the only change is one extra `lstat` call ahead of it, not an extra read.
  ...
  - Docs: `docs/decisions/2026-08-27-ug-auto-mode-approval.md` (the launcher-report-channel paragraph now names all three distinct reasons and their check order), `docs/policy-packs/understanding-before-execution.md` (the `--report` fallback-shape paragraph names the same three reasons and check order).
  - **Tests**: `tests/policy-packs/understanding-before-execution-delegation.test.ts` pins `report_missing` on a never-written report (both a non-realpathed and an explicitly realpathed temp root) and on a report STAGED then deleted before verify (a symlinked temp root, where the bind-time and verify-time path hashes would otherwise disagree); a SYMLINK and a DIRECTORY at the conventional path each pin the negative (`result.reason).not.toBe("report_missing")`, landing on `report_path_mismatch` and `report_content_mismatch` respectively; a hand-signed half-binding (report path bound, no `reportContentHash`, a shape `writeDelegationMarker` itself refuses to produce) with an absent file pins `report_missing` over `report_content_mismatch`. `tests/cli/pack-hook-pre-tool-use-delegate.test.ts` case (w) (moved/absent conventional report) now expects `report_missing` instead of the previous `report_content_mismatch`. Mutation probe: removing the existence check (restoring the fall-through straight into the path-hash comparison) turns the new report_missing-pinned tests red; restoring the check turns them green again.
